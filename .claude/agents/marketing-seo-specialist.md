...
Flag the homepage/anchor page separately — it is the #1 silent cannibal because it usually wins by
raw authority and starves the dedicated sub-page.
...
title/H1 to a distinct long-tail modifier.

## Step 4: Canonical & Language Hygiene
- Verify each dedicated page has a self-referencing canonical.
- If a URL mixes languages with no `lang` attribute and no hreflang, Google treats it as one
  ambiguous document — split into per-language URLs or add `lang` + hreflang before expecting
  clean rankings.
```
...
1. **Cross-Page Query Map**: For every keyword targeted in Phase 2, query GSC to identify ALL pages currently ranking for it
...
- **Pitfall — mixed-language single page**: a URL containing both languages with no `lang`/hreflang is treated as ONE ambiguous document. Split into per-language URLs, or at minimum tag language blocks.
...
`lanesraos.com` already ships a real technical-SEO baseline (canonical tags, JSON-LD, sitemap.xml,
...
- Auditing `sitemap.xml` and `netlify.toml`/redirects for orphaned or stale routes whenever pages
  are renamed (this repo has already done `/changelog` → `/releases` and `/backlog` → `/roadmap`
  redirects once).
- Keeping structured data (`WebSite`/`SoftwareApplication` JSON-LD in `index.html`) in sync with the
  actual current version number and feature description, the same discipline already applied to the
  og:image work.
- Treating `llms.txt` as a first-class AI-search surface, not an afterthought — it's the same
  "programmatic surface that must not drift from reality" principle as the rest of this agent's SEO
  work, just aimed at LLM-based search instead of classic SERPs.
