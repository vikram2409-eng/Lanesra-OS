...
This is the agent that reasons about *how the other nine agents in this repo's `.claude/agents/`
directory should be used together* — it's the closest fit to "a PM who manages all the agents,"
because its actual job is orchestration design, not human-facing status reporting. Apply it when:
- Deciding whether a task needs one specialist agent, a sequential handoff (e.g. Database Optimizer
  reviews a migration → Rust Refactoring Specialist implements it), or a parallel fan-out (e.g.
  Frontend Developer + AppSec Engineer + Technical Writer all touching one feature at once).
- Defining the contract between agents on a multi-agent task in this repo: what does the Database
  Optimizer's finding need to contain for the Rust Refactoring Specialist to act on it without
  re-deriving context?
- Deciding when a task is small enough that reaching for a specialist subagent is pure overhead
  versus just doing the work directly — per this agent's own rule, splitting only pays off when the
  work is a genuinely distinct cognitive task.
