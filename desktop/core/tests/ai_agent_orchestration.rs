...
async fn pipeline_crud_is_administrator_gated_and_validates_its_steps() {
    ...
    let input = AiAgentPipelineInput {
        name: "Two Step".into(), description: Some("A demo pipeline".into()),
        steps: vec![
    ...
    let bad = AiAgentPipelineInput { name: "Bad".into(), description: None, steps: vec![PipelineStepInput { agent_id: "not-a-real-id".into(), input_template: "hi".into() }] };
...
async fn a_manual_pipeline_run_feeds_step_ones_real_output_into_step_two() {
    ...
    let pipeline = ai_orchestration_service::create_pipeline(
        ...
        &AiAgentPipelineInput {
            name: "Summarize then translate".into(), description: None,
            steps: vec![
