...
function landing(){
 ...
 ${publicNav()}
 <main>
 <section class="hero"><div class="container hero-grid"><div><div class="eyebrow">The open-source, AI-native business operating system</div><h1>The AI-native business operating system.</h1><p style="font-size:20px;font-weight:650;margin:0 0 18px">Sovereign data. Declarative agents. Zero lock-in.</p><p>Combine an extensible relational data model with a unified Agent Foundry. Every agent runs against your real Custom Objects and business rules — not a bolted-on chatbot — and the built-in AI Gateway routes each one to a cloud provider or a fully local, air-gapped model, with automatic failover and per-agent token budgets. <a href="/platform" style="color:inherit;text-decoration:underline">Read the platform architecture →</a></p><div class="hero-actions"><a class="btn btn-primary" href="/demo">Try the live demo →</a><a class="btn btn-secondary" href="https://github.com/vikram2409-eng/Lanesra-OS" target="_blank" rel="noopener">View on GitHub →</a></div><div class="trust-row"><span>✓ Free to use</span><span>✓ No licence key</span><span>✓ Self-hosted &amp; offline-first</span><span>✓ Bring your own AI key</span><span>✓ Own your data</span></div></div><div class="terminal-window"><div class="terminal-top"><span class="dot"></span><span class="dot"></span><span class="dot"></span></div><div class="terminal-body"><div><span class="prompt">$</span> lanesra agent run claims_auditor</div><div class="muted-line">→ resolving model routing for "claims_auditor"...</div><div class="muted-line">→ primary (Claude) unreachable — failing over</div><div class="muted-line">→ local_fallback (Ollama, air-gapped): dispatching</div><div class="ok">✓ run complete — served_by: local_fallback</div><div><span class="prompt">$</span> <span class="cursor"></span></div></div></div></div></section>
 ...
 <section id="open-source" class="section"><div class="container cta"><div class="eyebrow" style="color:var(--brand)">Your data. Your models. Your OS.</div><h2>Inspect it. Run it. Improve it.</h2><p style="color:#cbd5e1;max-width:700px;margin:0 auto 24px">An open-source, self-hosted business platform and AI Agent Foundry — no subscription, no per-seat pricing, no vendor lock-in, and nothing here calls a model or ships a record anywhere until you configure it to.</p><div class="hero-actions" style="justify-content:center"><a class="btn btn-primary" href="/demo">Try the live demo →</a><a class="btn btn-secondary" href="https://github.com/vikram2409-eng/Lanesra-OS" target="_blank" rel="noopener">View GitHub repository</a></div></div></section>
 </main>${publicFooter()}
 </div>`;
 bindPublicNav();
 bindHomeCodeTabs();
...
function roadmapPage(){
 ...
 const shippedPillars=[
  ...
  ['⬢','Apps, Packaging & Connections','Install a ready-made vertical app, package your own customizations like a real software vendor, and connect the workspace to everything else it needs to talk to.',[
   ...
   ['⬡','Integration Hub',[["Connections, Connectors, API Access, Webhooks","A real, encrypted-at-rest Connection layer (generic REST, SFTP, Postgres, OData, SMTP, OAuth2), OpenAPI-imported Connectors that become reusable Workflow Automation actions, inbound API clients (hashed <code>{client_id}.{secret}</code> keys, scoped, revocable) against a generic <code>/api/v1/objects/...</code> REST API, and outbound Webhooks with HMAC-SHA256 signing, retry/backoff and delivery history - each backed by real tests (a real local TCP listener stands in for the external system, not a mock)."],["Data Exchange, External Objects, Integration Jobs","The existing Companies/Contacts-only CSV import is now a generalized, object-agnostic wizard (any built-in or Custom Object, column mapping with transforms, duplicate/upsert handling, dry-run preview) plus CSV export and reusable Mappings - all routed through the same generic record-write path the REST API uses, so permissions and business rules apply identically. External Objects surface read-only, live records from a Connection; Integration Jobs add a recurring pull-sync with a checkpoint cursor, run by a real background scheduler on the Team Workspace server (desktop keeps a manual Run Now, the same client-poll-only pattern Workflow Automation's own scheduling already has)."],["Logs & Monitoring, Settings, event streaming","A unified execution log across API calls, webhook deliveries and import/export runs, a real KPI-driven Overview, workspace-level rate-limit/retention Settings, and a Server-Sent-Events endpoint streaming executions live. Best-effort, explicitly-labeled coverage beyond the core v1 scope: SFTP and PostgreSQL connectors (proven against a real in-process SFTP server and a real local Postgres), OAuth2 client-credentials and authorization-code token exchange, a minimal SMTP action, and an OData v4 query/response layer. mTLS is the one deliberate gap, left for later as the spec itself marks it."],["Connector Template Library, Phase 1: five curated pre-built connectors","Import connector now opens on a template gallery, not just a blank paste box - five hand-curated, hand-trimmed OpenAPI specs (OpenAI, Cohere, Slack, GitHub, SendGrid) an admin can start from instead of writing their own. Every template was chosen for a confirmed <code>Authorization: Bearer</code> + JSON-body fit with this codebase's existing Connection auth model, and every request-body schema is fully inline (no <code>$ref</code>), so every imported action is immediately eligible for the Integration Hub Tool Bridge, not just Workflow Automation - verified by a new automated test asserting a zero-warning parse and a non-empty schema for every template, not just claimed in a doc comment. Deliberately not a larger first batch: Stripe/Twilio's form-encoded bodies and Google Gemini's query-parameter API key don't fit any of the 7 existing auth modes yet, and data warehouses need their own per-vendor auth check first - see Planned, below, for the honest remainder."],["Connector Template Library, Phase 2: data warehouses, form-encoded bodies &amp; query-param auth","Eight more templates - Snowflake, Databricks (both use a Personal/Programmatic Access Token, not the JWT/key-pair auth originally assumed), HubSpot, Airtable, Notion, Stripe, Twilio, Google Gemini - closing out every gap Phase 1 named. Reading <code>connection_service::apply_auth</code> found <code>oauth2_client_credentials</code>/<code>oauth2_authorization_code</code> are, in this codebase, purely descriptive labels for <code>bearer</code> - no token exchange or refresh loop exists anywhere - so HubSpot/Airtable/Notion qualified because each also offers a long-lived static token, not because of the auth_mode's name; BigQuery, Salesforce and Google Sheets stay excluded, honestly, since all three require a genuinely short-lived OAuth2 access token with mandatory refresh this codebase doesn't implement. Stripe/Twilio needed a real, bounded engine change: a new <code>is_flat_object_schema</code> check plus a <code>request_content_type</code> column let the parser recognize a flat <code>application/x-www-form-urlencoded</code> body as first-class (zero warnings, a real <code>.form()</code> send) while a nested one still falls back to the pre-existing opaque-with-warning path, unchanged. Gemini needed a new <code>\"query_param\"</code> auth_mode for its <code>?key=</code> API key. Thirteen templates total."],["Connector Template Library, Phase 3: six more SaaS templates &amp; real MySQL/SQL Server connectivity","Zendesk, Jira Cloud, PagerDuty, Datadog, Mailchimp, Shopify - each verified against the vendor's current docs or published OpenAPI spec. Linear turned out to be GraphQL-only (confirmed - no REST resource endpoints exist), a real architectural mismatch with this gallery's OpenAPI parser, not a curation gap - excluded. Datadog needs a second credential (<code>DD-APPLICATION-KEY</code>) for most endpoints, confirmed against its own OpenAPI spec; since this Connection model has one secret slot, its template is scoped to the two operations (submit event, submit metrics) confirmed to need only <code>DD-API-KEY</code>. Nineteen templates total. Separately: real MySQL/MariaDB and SQL Server <code>connection_type</code>s, both pure-Rust wire-protocol drivers (<code>mysql_async</code>/<code>tiberius</code>, both on <code>rustls</code> - no new OpenSSL dependency). MySQL's test_connection is proven against a real local MariaDB server, gated <code>#[ignore]</code> like Postgres's own; SQL Server's success path against a live instance is a stated, known gap - no SQL Server build exists in this sandbox to prove it against, matching this codebase's existing honesty about SFTP's server-side."],["Connector Template Library, Phase 4: seven more SaaS templates &amp; real MongoDB/Redis connectivity","GitLab, Confluence Cloud, Asana, Freshdesk, Segment, Calendly, CircleCI - each verified against the vendor's own current docs or published OpenAPI spec. monday.com turned out to be GraphQL-only (confirmed - the only endpoint is <code>POST /v2</code> with a GraphQL query body), the same class of exclusion as Linear - excluded. Segment's own docs pages return HTTP 403 to automated doc fetches; its Basic-auth-with-blank-password shape was instead confirmed with a real live request against the production <code>api.segment.io/v1/track</code> endpoint - stronger evidence than a doc page. Twenty-six templates total. Separately: real MongoDB and Redis <code>connection_type</code>s - the official <code>mongodb</code> driver (defaulting to <code>rustls-tls</code>) and the <code>redis</code> crate (no TLS feature needed for a plain connection) - both proven against real local servers stood up for this pass (a real MongoDB 7.0 downloaded from MongoDB's own official tarball distribution, since no MongoDB package exists in this sandbox's apt repos; Redis was already installed and just needed starting), unlike SQL Server's still-stated live-server gap."]]],
   ]],
 ...
 const planned=[
  ...
  ['Connector Template Library: real OAuth2 token refresh (BigQuery, Salesforce, Google Sheets)','Phases 1 and 2 shipped thirteen curated templates - see Shipped, below - closing every gap that turned out to be a curation problem. What\'s left is a real infrastructure gap, not more curation: <code>connection_service::apply_auth</code> today treats <code>oauth2_client_credentials</code>/<code>oauth2_authorization_code</code> identically to <code>bearer</code> - a static secret, no token exchange, no refresh loop anywhere in this codebase - which is a fine fit for a vendor that also offers a long-lived static token (as Phase 2\'s HubSpot/Airtable/Notion do), but BigQuery, Salesforce and Google Sheets all confirmed to require a genuinely short-lived OAuth2 access token with mandatory refresh and no static alternative. Adding them for real needs actual token-exchange-and-refresh infrastructure on the Connection model - persisted refresh tokens, expiry tracking, a refresh-before-expiry call before every outbound request - before a template for any of the three is worth curating.','Integration Hub','M'],
