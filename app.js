...
function roadmapPage(){
 ...
 const shippedPillars=[
  ['▣','Core Product','Data model, deployment and document workflow — the foundation everything else is built on.',[
   ['🔐','Organization & Access Control — Phase 1 of 6',[['Organization, Organization Units & Work Teams','One Organization per workspace containing Organization Units in a materialized-path hierarchy (regions, divisions, teams) — create, edit, and move a unit with an impact preview showing exactly how many descendant units and owned records the move affects before you confirm it. Work Teams sit alongside the hierarchy with time-bounded memberships; ending a membership never reassigns anything the member owned — a deliberate, tested guarantee.'],['Record ownership — Owner & Owning Organization Unit on every record','Every Company, Contact, Opportunity, Product, Quote, Order, Invoice, Contract, Task and Custom Object record now carries a structured Owner (a user or a team, not free text) and an Owning Organization Unit, defaulted to the creating user and their org unit on create. Change it on an individual record or reassign in bulk across a multi-select; an ownership-version counter and timestamp advance only on a real transfer, never a no-op save. Scoped to exactly Phase 1 of a six-phase Enterprise Access Foundation — capability-scoped Access Roles, Field Access Policies, Record Grants, AI Action Levels and Enterprise Identity/SSO are next (see Planned, below).']]],
 ...
 const planned=[
  ...
  ['Access Control v1: capability-scoped Access Roles','Phase 2 of the six-phase Enterprise Access Foundation, right behind Phase 1 (Organization/Org Units/Work Teams/record ownership - see Shipped, above). Replaces today\'s placeholder "Assign requires the Administrator role" gate with a real Access Role engine: named, composable roles carrying a capability set (per-object create/read/update/delete/assign) and a Record Scope (Owner-only, Owner\'s Team, Owner\'s Org Unit and below, or Organization-wide) - the actual mechanism <code>ownership_service::require_assign_capability</code>\'s own doc comment already names as the seam this phase replaces. Ships with an Access Inspector - "why can/can\'t this user see this record" - answerable from a real evaluation trace, not guesswork. See <a href="https://github.com/vikram2409-eng/Lanesra-OS/issues/141">Issue #141</a>.','Organization & Access Control','L'],
 ];
 const proposed=[
  ...
  ['Access Control v1.1: Field Access Policies, Capability Packs &amp; Record Grants','Phase 3 of the Enterprise Access Foundation, once Phase 2\'s Access Role engine exists to hang these off of. Field-level read/write policies beyond today\'s all-or-nothing record access; reusable Capability Packs/Bundles so an admin assigns a named bundle instead of hand-picking every capability per role; Access Groups (a named set of users, independent of the Org Unit hierarchy, for cross-cutting access like "everyone on the Q3 launch"); and Record Grants — a one-off, time-bounded share of a specific record to a specific user or team outside their normal scope, the exception valve every scope-based system eventually needs.','Needs Phase 2\'s Access Role/capability model to exist first — Field Access Policies in particular need a decision on evaluation order against Business Rules\' own field-level show/hide/lock effects, so the two systems don\'t silently fight each other. See <a href="https://github.com/vikram2409-eng/Lanesra-OS/issues/142">Issue #142</a>.',null,'L'],
...
function platformPage(){
 ...
 const layers=[
  ...
  {icon:'🔐',eyebrow:'Admin → Access · Phase 1 of 6',name:'Organization & Access Control',blurb:'An Organization Unit hierarchy (regions, divisions, teams — move one with an impact preview before it commits) and Work Teams with time-bounded memberships that never touch record ownership on their own. Every record across the CRM and your Custom Objects carries a structured Owner (a user or a team) and an Owning Organization Unit, individually or bulk-reassignable, with a full transfer history. Phase 1 of a six-phase Access Foundation — capability-scoped Access Roles, Field Access Policies and Enterprise Identity/SSO are next. Shipped on both the desktop app and the online demo.'},
 ];
...
const HELP_CATEGORIES=[
 ...
 {key:'integration-hub',label:'Integration Hub',icon:'⬡',
  ...
  {key:'organization-access-control',label:'Organization & Access Control',icon:'🔐',
   blurb:'Who reports to whom, and who owns what. Organizations, an Organization Unit hierarchy, Work Teams, and a structured Owner + Owning Organization Unit on every record - Phase 1 of a six-phase Access Foundation.',
   topics:[
    ...
    {slug:'access-foundation-whats-next',title:"What's next: Access Control, AI Access & Enterprise Identity",
     summary:'Organization/Org Units/Work Teams/ownership is Phase 1 of six. Phases 2-6 - capability-scoped Access Roles, Field Access Policies, AI Action Levels and SSO - are roadmapped, not built yet.',
     sections:[
      {heading:'Why this category stops here',bodyHtml:`<p>What's documented above - Organizations, Organization Units, Work Teams and record ownership - is deliberately scoped to <b>Phase 1 (Access Foundation)</b> of a six-phase Enterprise Requirements Specification, matching exactly what this phase of work set out to build rather than attempting all six phases in one pass.</p>`},
      {heading:'The placeholder gate this replaces',bodyHtml:`<p>Today, changing a record's owner or moving an Organization Unit requires the existing Administrator role - a deliberate placeholder, not a real capability-and-scope engine. The next phase replaces it for real.</p>`},
      {heading:'Phases 2 through 6',bodyHtml:`<ul><li><b>Access Control v1</b> - capability-scoped Access Roles (per-object create/read/update/delete/assign), Record Scopes (Owner-only, Owner's Team, Owner's Org Unit and below, or Organization-wide), and an Access Inspector answering "why can/can't this user see this record."</li><li><b>v1.1</b> - Field Access Policies, reusable Capability Packs/Bundles, Access Groups, and Record Grants for one-off, time-bounded exceptions.</li><li><b>v1.2</b> - declarative Record Access Policies, and a stronger dedicated audit view over bulk reassignment specifically.</li><li><b>AI Access v1</b> - graded AI Action Levels per tool call, and a real Agent/Integration Identity distinct from the human who configured it.</li><li><b>Enterprise Identity v2</b> - OIDC/SAML single sign-on, SCIM-provisioned user lifecycle, and a first-class External User concept.</li></ul>`},
