...
function roadmapPage(){
 ...
 const proposed=[
  ...
  ['Local MCP access without a Team Workspace server','The native MCP server (<code>POST /mcp</code>) only exists on the Team Workspace HTTP server - a solo desktop install, with no listening socket at all, has no MCP endpoint, so pointing Claude Desktop or an IDE agent at a personal workspace\'s real records is impossible today unless that person stands up a whole Team Workspace server just to get one feature. A lightweight local bridge - the desktop app\'s own process exposing MCP over stdio (the transport most desktop MCP clients actually expect, distinct from the existing stateless HTTP JSON-RPC endpoint), reusing the identical tool dispatcher and scoped API-client auth the Team Workspace HTTP endpoint already uses - would close this for the majority of users who never run a server at all.','Needs a decision on the transport shape: a stdio bridge process spawned by the desktop app itself (simplest single-entry Claude Desktop config, but a new process-management surface on every OS) vs. teaching the desktop app to also open a local-only HTTP listener (reuses the existing server code wholesale, but adds a listening socket to a product whose whole pitch has been local-first with no open ports by default).','MCP','M'],
  ['Integration Hub Tool Bridge: connectors & workflow actions as agent tools','An AI Agent\'s Actions checklist is a fixed, hand-registered catalog (8 record tools, 37 admin action pairs) - nothing today lets an agent call a Connector Action built from an OpenAPI import, or fire an existing Workflow Automation action, as a tool of its own. Since Integration Hub already validates and executes these against real external systems, translating each configured Connector Action\'s own request/response schema into a standard LLM tool-call definition (name, JSON-schema parameters) at agent-config time would give an agent real-world write access to whatever a workspace has already connected - a shipping, invoice or fulfillment system, say - without a line of custom integration code. Likely the single highest-leverage item here: it reuses infrastructure (Connectors, OpenAPI import, scoped auth) that already exists end to end.','Needs a decision on scope and safety: every Connector Action would need its own generated tool schema plus a runtime authorization check equivalent to the Actions checklist\'s own Administrator-only gate, and a mis-described OpenAPI schema could hand an agent a "write" tool that reads as read-only from its description alone - the translation layer needs to fail closed on an ambiguous schema, not guess.','Integration Hub','L'],
  ['Agent-as-code: git-trackable agent definitions','Every AI Agent today lives only as a database row, edited through Admin → AI Agents - reviewable and versioned the same way as a Business Rule, but not as a file a developer can diff, PR-review or track in the same git history as the rest of a team\'s configuration. A parallel file-based format - a directory per agent combining a persona/system-prompt file, a memory seed and a guardrails file, the same three concepts the DB-backed agent already has - imported into (and exportable back out to) the existing Solution export/import pipeline, would let a technical team manage agent behavior as code alongside everything else they already git-track, without changing how a non-technical admin edits that same agent from the UI.','The real risk is two sources of truth: once an agent can be edited from a file AND from the Admin UI, something has to own conflict resolution (last-write-wins on import? lock a file-sourced agent against UI edits entirely?) - needs that decision made explicitly up front, not settled implicitly by whichever import path gets built first.','AI Agent Foundry','L'],
  ['Evaluation Harness: a CLI and a release gate, not just an Admin screen','The Evaluation Harness already runs a named Suite of golden Cases against an Agent or Pipeline, graded by an LLM-as-judge, from Admin → Evaluations - but only interactively, one click at a time. A <code>lanesra agent eval &lt;suite&gt;</code> CLI command producing a machine-readable score report, plus a documented way to fail a CI job or a Solution release when a score regresses past a defined threshold, would turn the harness a maintainer already has into an actual pre-deploy safety net for agent changes - the same promote-after-tests-pass discipline Deployment Management already gives configuration changes, extended to agent behavior.','Needs the CLI\'s own auth story worked out first - today every write path assumes an interactive signed-in session or a scoped API-client key; a CI runner is neither, so this either mints its own service-account concept or reuses existing API-client keys in a mode the CLI has to document clearly.','AI Agent Foundry','M'],
 ];
