...
function exportLocalWorkspace(){
 ...
 const objects=(data.customObjects||[]).filter(o=>localIds.has(o.id)).map(o=>({key:o.key,singular_label:o.label,plural_label:o.labelPlural,icon:o.icon,prefix:o.prefix,digits:o.digits}));
 const fields=(data.customFields||[]).filter(f=>localIds.has(f.id)).map(f=>({key:f.key,entity_type:f.entity,label:f.label,field_type:f.type,options:f.options?f.options.split('|'):[],required:!!f.required}));
 const relationships=(data.relationshipDefinitions||[]).filter(r=>localIds.has(r.id)).map(r=>({key:r.key,source_entity_type:r.sourceEntity,target_entity_type:r.targetEntity,relationship_type:r.relType,forward_label:r.forwardLabel,reverse_label:r.reverseLabel}));
 const businessRules=(data.fieldRules||[]).filter(r=>localIds.has(r.id)).map(r=>({entity_type:r.entity,match_type:r.matchType,conditions:r.conditions,actions:r.actions}));
 const workflows=(data.workflowRules||[]).filter(w=>localIds.has(w.id)).map(w=>({entity_type:w.entity,match_type:w.matchType,conditions:w.conditions,actions:w.actions}));
 const reports=(data.customReports||[]).filter(r=>localIds.has(r.id)).map(r=>({name:r.name,entity_type:r.entityKey,group_by_source:r.groupBySource,group_by_field:r.groupByField,aggregate:r.aggregate,sum_field_key:r.sumFieldKey||null}));
 const manifest={format_version:1,package_id:'local.workspace_export',name:'Local Workspace Export',industry:'Custom',version:`1.0.${Date.now()}`,min_lanesra_version:'0.1.0',objects,fields,relationships,business_rules:businessRules,workflows,reports};
