...
function roadmapPage(){
 ...
 const sequence=[
  ['1 · Decide: Screen Designer, dashboard customization, Google sign-in, richer report builder, code signing','Both scoped items from this round have shipped (Global search & list-view filtering, Self-hosted internet deployment). Everything left in the near-term backlog needs a product/architecture decision first — several independent scope/budget calls, worth a deliberate conversation each rather than defaulting into months of work.',true],
 ];
 $('#app').innerHTML=`${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Built from the working codebase</div><h1>Roadmap & backlog.</h1><p>Where Lanesra OS stands today, what's being built next, and everything queued up after that — compiled from the actual code (core/server/src-tauri/frontend) plus the online demo, not a wishlist. Every "Shipped" line below is running code with tests.</p><div class="status-row"><span class="status-chip">Early Access v0.25.0</span><span class="muted">Last updated August 2026</span></div>
