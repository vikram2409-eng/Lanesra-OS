...
function roadmapPage(){
 ...
 <section class="rm-section" id="building-next"><div class="rm-section-head"><div class="eyebrow">In progress · up next</div><h2>What's being built next.</h2><p>Scoped and ready to build — no open product question left to resolve first. The agentic-infrastructure items below came out of a full backlog review against the working codebase — see the <a href="https://claude.ai/code/artifact/5d446d58-ec3f-4906-97b0-9fcaf5b6a404" target="_blank" rel="noopener">full backlog analysis</a> for the pros/cons and rationale behind each decision.</p></div>${planned.length>0?planned.map(p=>`<div class="rm-hero-card"><div class="rm-kicker"><span class="dot"></span>Next up</div><h3>${p[0]}</h3><p>${p[1]}</p><div class="rm-tags"><span class="backlog-tag planned-tag">planned</span><span class="backlog-tag">size: ${p[3]}</span></div></div>`).join(''):`<div class="rm-empty">Nothing scoped and ready right now — every remaining item needs a product/architecture decision first. See "Deciding" below.</div>`}</section>
...
else if(path==='/demo')appShell();else if(path==='/roadmap')roadmapPage();else if(path==='/releases')releasesPage();else if(path==='/platform'||path==='/build')platformPage();else if(path==='/principles'||path==='/journey'||path==='/our-story'||path==='/about')principlesPage();else if(path==='/compare')comparePage();else if(path==='/download')downloadPage();else if(path==='/help')helpHubPage();else if(path.startsWith('/help/'))helpTopicPage(path.slice(6));else landing();
