...
const HOME_FACTS=[['10','ready-made industry apps in the App Catalog'],['7','AI & Agentic Layer phases already shipped'],['$0','licence keys or per-seat fees, ever'],['100%','of the source, public on GitHub']];
const HOME_PERSONAS=[
 ['📈','Outgrowing spreadsheets','Your pipeline, quotes and invoices finally live in one connected place instead of three tabs and a shared spreadsheet nobody fully trusts.'],
 ['🧩','Agencies & consultants','Configure a client\'s workspace once, package it as a versioned Solution, and promote or resell it — the build-in-test/promote pattern enterprise platforms charge extra for.'],
 ['🛠️','Technical & ops teams','A real extensible platform, not a rigid SaaS CRM — Custom Objects, a REST API, webhooks and scheduled sync mean you\'re never blocked waiting on a vendor\'s roadmap.'],
 ['🔒','Data-sovereign organizations','Run the desktop edition fully offline, self-host Team Workspace on your own infrastructure, and bring your own AI key — nothing routes through a Lanesra-run server, because there isn\'t one.'],
];
const HOME_CRM_LIST=['Companies & Contacts — profiles, people, notes and activity in one place','Sales Pipeline — move opportunities visually from lead to won','Products & Services — reusable pricing, categories and tax settings','Quotes → Orders → Invoices, one connected flow with acceptance and paid/open/overdue tracking','Contracts — agreement values, dates, files and renewals','Tasks & Activities — calls, meetings, follow-ups and priorities','Sales Dashboard — pipeline, revenue and next actions at a glance'];
const HOME_PLATFORM_LIST=['Custom Objects & Relationships — model anything, not just what shipped in the box','Screen/App Builder — role-based layouts, Draft → Publish, no developer required','Business Rules & Workflow Automation — validation and triggers from a settings screen','Custom Fields, Reports & Dashboards — group-and-sum reporting with no separate tool','App Builder + a 10-app Industry Catalog — install a ready-made business instead of building one','Deployment Management — versioned Solutions, promoted between workspaces you own','Integration Hub — REST API, webhooks, OpenAPI connectors, scheduled sync'];
const HOME_AI_LIST=['Bring your own Anthropic, OpenAI-compatible or fully offline key — nothing calls a model until you do','MCP Server & CLI for Claude Desktop, an IDE agent, or a shell script','A Chat Assistant with real tool access to your records, or your admin config','AI Agent Foundry — named agents with a persona, Actions, persistent Memory and Skills','Delegation & deterministic Pipelines, fired manually, on a schedule, or by a webhook'];
function landing(){
 document.title='Lanesra OS — Open-source CRM, app platform & integration hub';
 $('#app').innerHTML=`
 ${publicNav()}
 <main>
 <section class="hero"><div class="container hero-grid"><div><div class="eyebrow">The open-source business application platform</div><h1>The CRM, the platform to build on it, and the AI layer on top — one workspace, not three subscriptions.</h1><p>Lanesra OS ships a complete CRM for customers, pipeline, quotes, orders, invoices and contracts — built from the same no-code platform you get to use yourself: custom objects, relationships, screens, business rules, workflows and dashboards. Package what you build into a Solution and promote it, install a ready-made industry app instead, or hand it to an AI agent running on a key you control. <a href="/platform" style="color:inherit;text-decoration:underline">See what else you can build with it →</a></p><div class="hero-actions"><a class="btn btn-primary" href="/demo">Try the live demo →</a><a class="btn btn-secondary" href="/download">Desktop edition — Windows installer available</a></div><div class="trust-row"><span>✓ Free to use</span><span>✓ No licence key</span><span>✓ No-code customization</span><span>✓ Bring your own AI key</span><span>✓ Own your data</span></div></div><div class="mock"><div class="mock-top"><span class="dot"></span><span class="dot"></span><span class="dot"></span></div><div class="mock-body"><div class="mock-grid"><div class="mock-card"><small class="muted">Pipeline</small><br><strong>$192K</strong></div><div class="mock-card"><small class="muted">Revenue</small><br><strong>$84K</strong></div><div class="mock-card mock-chart">${[40,70,52,88,62,100].map(h=>`<div class="bar" style="height:${h}%"></div>`).join('')}</div></div></div></div></div></section>
 <div class="container"><div class="fact-strip">${HOME_FACTS.map(f=>`<div class="fact-tile"><span class="n">${f[0]}</span><span class="l">${f[1]}</span></div>`).join('')}</div></div>
 <section class="section"><div class="container"><div class="section-head" style="margin:0 auto 30px;text-align:center;max-width:680px"><div class="eyebrow">Built for</div><h2>Who actually reaches for this.</h2><p class="muted">Not a persona deck — four real situations the platform underneath already handles.</p></div><div class="persona-grid">${HOME_PERSONAS.map(p=>`<article class="persona-card"><div class="persona-icon">${p[0]}</div><h3>${p[1]}</h3><p>${p[2]}</p></article>`).join('')}</div></div></section>
 <section id="features" class="section" style="background:var(--surface-alt)"><div class="container"><div class="section-head"><div class="eyebrow">Three layers, one workspace</div><h2>Everything connected, and nothing hardcoded.</h2><p class="muted">The sales system you get on day one is itself just one app built from the same no-code platform and the same AI layer available to you underneath.</p></div><div class="bento">
 <article class="bento-tile lead"><div><div class="bento-icon">◎</div><div class="eyebrow">Complete sales journey</div><h3 style="font-size:22px">Everything connected from first conversation to invoice</h3><p>No maze of modules, no enterprise setup project — just the essentials a sales team uses every day.</p></div><ul class="bento-list">${HOME_CRM_LIST.map(l=>`<li>${l}</li>`).join('')}</ul></article>
 <article class="bento-tile wide"><div class="bento-icon">⬡</div><h3>Make it yours — no code required</h3><p>Every business outgrows a fixed data model. An Administrator reshapes the workspace itself from a settings screen — not a developer, not a support ticket.</p><ul class="bento-list" style="margin-top:14px">${HOME_PLATFORM_LIST.map(l=>`<li>${l}</li>`).join('')}</ul></article>
 <article class="bento-tile wide on-dark"><div class="bento-icon">🏭</div><h3>An AI &amp; Agentic Layer, not a bolted-on chatbot</h3><p>Every layer here runs on the LLM key you supply — Lanesra never resells or proxies inference, the same "own your data" posture the rest of the product already takes.</p><ul class="bento-list" style="margin-top:14px">${HOME_AI_LIST.map(l=>`<li>${l}</li>`).join('')}</ul><div style="margin-top:16px"><a class="btn btn-secondary" href="/platform">See how it fits the platform →</a></div></article>
 </div></div></section>
