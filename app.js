...
function landing(){
 ...
 <section id="features" class="section" style="background:var(--surface-alt)"><div class="container"><div class="section-head" style="margin:0 auto 34px;text-align:center;max-width:720px"><div class="eyebrow">The platform primitives</div><h2>What actually runs underneath.</h2><p class="muted">Four real primitives, not a feature-name wordlist — every one of these is shipped code you can read on GitHub today.</p></div><div class="bento">${HOME_PRIMITIVES.map(p=>`<article class="bento-tile wide"><div class="bento-icon">${p[0]}</div><h3>${p[1]}</h3><p>${p[2]}</p></article>`).join('')}</div></div></section>
...
function platformPage(){
 ...
 const layers=[
  {icon:'⬢',eyebrow:'Admin → Apps → App Catalog',name:'App Catalog',blurb:'10 complete, install-ready industry apps — Field Service, Property Management, Construction & Contractors, Professional Services, Practice Administration, Recruitment & Staffing, Real Estate Brokerage, Legal Practice, Nonprofit & Association Management and Auto Repair & Service Garage. A validated, backed-up, transactional install runs straight into your workspace, reusing your existing Companies and Contacts rather than creating a parallel database.'},
 ...
 <section class="section"><div class="container narrow"><div class="section-head" style="text-align:center;margin:0 auto 8px"><div class="eyebrow">The building blocks</div><h2>Seven primitives. One workspace.</h2><p class="muted">Every example below — and every tool an agent above can call — is assembled from these seven primitives. Nothing industry-specific is baked into the product itself.</p></div><div class="platform-primitives">${primitives.map((p,i)=>`<span class="platform-primitive"><span class="n">${i+1}</span>${p}</span>${i<primitives.length-1?'<span class="platform-arrow">→</span>':''}`).join('')}</div></div></section>
...
function comparePage(){document.title='Compare — Lanesra OS';
 const groups=[
  ['Ownership & deployment',[
   ['Runs without internet','Partial','No','No','Yes'],
   ['Self-owned business data','File-based','Cloud-hosted','Cloud-hosted','Yes'],
   ['Local database','No','No','No','Yes (desktop)'],
   ['Open source','No','No','No','Yes'],
   ['Mandatory subscription','No','Yes','Yes','No'],
  ]],
  ['Sales workflow',[
   ['Connected sales workflow','Manual','Limited','Advanced','Yes'],
   ['Designed for small business','General','Yes','Enterprise','Yes'],
  ]],
  ['No-code customization',[
   ['Custom record types, no code','No','Limited, paid tiers','Yes, complex/paid','Yes, built in'],
   ['Custom screens & layouts by role','No','Limited','Yes, complex/paid','Yes, built in'],
   ['Custom business rules & workflow automation','No','Paid tiers','Yes, needs admin training','Yes, built in'],
   ['Build a whole custom app on the platform','No','No','Yes, paid tiers','Yes, built in'],
  ]],
  ['Apps, packaging & connections',[
   ['Ready-made industry apps to install, not just build','No','No','AppExchange, paid','Yes, 10 built in'],
   ['Package & promote customizations, test → production','No','No','Yes, paid tiers (Change Sets/DevOps Center)','Yes, built in'],
   ['Native REST API, webhooks & scheduled sync','No','Paid tiers','Yes, paid tiers/limits','Yes, built in'],
  ]],
  ['AI & Agentic Layer',[
   ['Bring your own LLM key, no markup or reselling','No','No, bundled AI credits','No, bundled Einstein/Agentforce credits','Yes, built in'],
   ['MCP server & CLI for outside AI agents','No','No','No','Yes, built in'],
   ['Conversational AI assistant with real tool access','No','Limited, paid tiers','Yes, paid add-on (Agentforce)','Yes, built in'],
   ['Custom AI agents with memory, skills & delegation','No','No','Yes, paid add-on (Agentforce)','Yes, built in'],
   ['Multi-agent orchestration & workflow-triggered runs','No','No','Yes, paid add-on','Yes, built in'],
  ]],
 ];
 $('#app').innerHTML=`<div class="site-dark">${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Choose with context</div><h1>Where Lanesra fits.</h1><p>A factual comparison for small businesses deciding between spreadsheets, cloud CRMs and a local-first open-source system.</p></div></section><section class="section"><div class="container compare-wrap"><table class="compare-table"><thead><tr><th>Capability</th><th>Excel</th><th>HubSpot</th><th>Salesforce</th><th class="lanesra-col">Lanesra OS</th></tr></thead><tbody>${groups.map(g=>`<tr class="compare-group-row"><td colspan="5">${g[0]}</td></tr>${g[1].map(r=>`<tr>${r.map((x,i)=>`<td class="${i===4?'lanesra-col':''}">${x}</td>`).join('')}</tr>`).join('')}`).join('')}</tbody></table><p class="compare-note">Comparisons are intentionally high-level. Product capabilities and commercial terms can change; review each vendor's current documentation before making a purchase decision.</p></div></section></main>${publicFooter()}</div>`;bindPublicNav()}
