...
function ensureAdminData(){
 ...
 // Deployment Management Phase 3 mirror: component-tagging's registry -
 // see tagLocalComponent/retagComponent's own comment.
 if(!data.components)data.components=[];
 // Deployment Management Phase 4 mirror: named, scoped Solutions - see
 // createSolution's own comment.
 if(!data.solutions)data.solutions=[];
 // Deployment Management Phase 2 mirror: a real Publisher registry, same
...
// Dark Mode Redesign v2: the home page's hero terminal mock shows a real
// Phase 7a Gateway failover sequence (primary -> local_fallback,
// served_by) - not a fabricated CLI session - and the bento grid below
// names the four real platform primitives (Gateway, SQLite-first data,
// MCP, Integration Hub), corrected against what's actually shipped (see
// this branch's own plan/PR body for the full fact-check).
const HOME_PRIMITIVES=[
 ['◈','Unified AI Gateway','Route every agent to Anthropic, OpenAI-compatible or Google Gemini — or a fully local, air-gapped model. Automatic primary → fallback → local-fallback failover, per-agent and per-workspace token budgets, and forced air-gapping the moment a request touches sensitive data.'],
 ['⌁','Native Model Context Protocol','Lanesra OS natively implements MCP over JSON-RPC 2.0 — securely exposing your Custom Objects, records and business logic to Claude Desktop, an IDE agent, or any MCP-capable client, authenticated with the same scoped API keys the REST API already uses.'],
 ['◫','Sovereign, SQLite-First Data','Runs entirely on SQLite, on your own machine or your own server — offline-first by default, no managed database, no data pipeline shipping records anywhere else. Every Custom Object and Relationship you define is queried directly by your agents\' tools, not a separate context pipeline to keep in sync.'],
 ['⇄','Integration Hub, Agent-Aware','Encrypted Connections, OpenAPI-imported Connectors, a generic REST API and HMAC-signed webhooks — and now a real Tool Bridge: an admin opts a connector\'s read-only actions into an AI Agent\'s own tool list, with a separate, Administrator-only opt-in before any write-capable action joins them.'],
];
const HOME_CODE_TABS=[
 ['Persona & Actions',`<span class="c"># claims_auditor — system_prompt</span>
You are a claims auditor. Verify the claimant ID,
policy ID and loss date before approving any payout.

<span class="c"># action_names — granted tools</span>
<span class="k">get_record</span>(Policy)
<span class="k">list_records</span>(Invoice)
<span class="k">update_record</span>(Claim)`],
 ['Memory',`<span class="c"># memory.md — a living document this agent</span>
<span class="c"># revises itself via its own update_memory tool</span>

Prior audits show claims over $10k need a second
reviewer's signature before payout is approved.`],
 ['Delegation',`<span class="k">$</span> claims_auditor → <span class="k">delegate_to_agent</span>("fraud_review")
<span class="c">→ depth 1/4 — guarded against runaway recursion</span>
<span class="c">→ fraud_review invoked with the same claim context</span>
<span class="k">← returns: "no fraud indicators found"</span>`],
];
function codeTabsHtml(tabs){
 return `<div class="code-tabs"><div class="code-tabs-nav">${tabs.map((t,i)=>`<button type="button" class="code-tab-btn${i===0?' active':''}" data-home-code-tab="${i}">${t[0]}</button>`).join('')}</div>${tabs.map((t,i)=>`<pre class="code-tab-body" id="homeCodeTab${i}" ${i===0?'':'hidden'}>${t[1]}</pre>`).join('')}</div>`;
}
function bindHomeCodeTabs(){
 document.querySelectorAll('[data-home-code-tab]').forEach(btn=>{
  btn.onclick=()=>{
   document.querySelectorAll('[data-home-code-tab]').forEach(b=>b.classList.remove('active'));
   btn.classList.add('active');
   HOME_CODE_TABS.forEach((_,i)=>{const el=document.getElementById('homeCodeTab'+i);if(el)el.hidden=String(i)!==btn.dataset.homeCodeTab});
  };
 });
}
function landing(){
 document.title='Lanesra OS — The AI-native business operating system';
 setPageMeta('Run your business and AI agents on data you own. Lanesra OS combines a complete CRM, no-code app builder, AI Agent Foundry and Integration Hub in one open-source, self-hosted platform - 11 installable industry apps, offline-first, bring your own AI model, no licence key or per-seat pricing. Try the live demo or download the Windows Early Access build.');
 $('#app').innerHTML=`<div class="site-dark">
 ${publicNav()}
 <main>
 ...
 <section class="section"><div class="container narrow"><div class="section-head" style="text-align:center;margin:0 auto 22px"><div class="eyebrow">90 seconds, no account needed</div><h2>See it in action.</h2><p class="muted">A real walkthrough of the live demo below - the pipeline, the Organization Unit hierarchy, an Integration Hub connector, the AI Agent Foundry and the chat Assistant, all captured from the actual product, not a mockup.</p></div><video class="hero-screenshot" poster="/media/product-demo-poster.jpg" width="1280" height="800" autoplay muted loop playsinline controls preload="metadata"><source src="/media/product-demo.webm" type="video/webm"><source src="/media/product-demo.mp4" type="video/mp4"></video></div></section>
 ...
 <section class="section" style="background:var(--surface-alt)"><div class="container split"><div class="choice-card"><div class="eyebrow">Package it, promote it</div><h2>Deployment Management</h2><p class="muted">Curate a named, versioned Solution from anything you've built — objects, fields, rules, workflows, screens, agents — export it, and import it into another workspace. The build-in-test, promote-to-production pattern enterprise platforms sell as a premium feature, running here as ordinary export/import between two self-hosted instances you already own.</p><ul><li>Publisher registry, no namespace collisions</li><li>Component-tagged, so nothing's a mystery</li><li>Update-with-diff before you apply a new version</li></ul><a class="btn btn-secondary" href="/platform#examples">See the platform →</a></div><div class="choice-card"><div class="eyebrow">Connect it</div><h2>Integration Hub</h2><p class="muted">Encrypted Connections to REST, SFTP, PostgreSQL, MySQL, SQL Server, MongoDB, Redis, OData and SMTP systems. OpenAPI-imported Connectors your workflows can call, from a curated template gallery or a hand-pasted spec. A generic REST API secured by scoped, hashed keys. HMAC-signed webhooks with retry.</p><ul><li>No integration platform subscription required</li><li>Every delivery signed and retried, not fire-and-forget</li><li>A unified log across every API call and delivery</li></ul><a class="btn btn-secondary" href="/roadmap#shipped">See what's shipped →</a></div></div></section>
 <section class="section"><div class="container"><div class="section-head" style="margin:0 auto 30px;text-align:center;max-width:720px"><div class="eyebrow">Docs-as-code, for agents</div><h2>Engineered for real agent lifecycle management.</h2><p class="muted">Build, delegate, and promote — with developer velocity.</p></div>${codeTabsHtml(HOME_CODE_TABS)}<div class="bento" style="margin-top:22px"><article class="bento-tile wide"><div class="bento-icon">📦</div><h3>Solution-Packaged</h3><p>Export any agent — persona, Actions, Memory, Skills and delegation — as part of a versioned Solution, and import it into another workspace. The same build-in-test, promote-to-production pattern Deployment Management already ships.</p></article><article class="bento-tile wide"><div class="bento-icon">🔀</div><h3>Delegation &amp; Pipelines</h3><p>A supervisor agent delegates to named sub-agents (depth-guarded against runaway recursion), or chain agents into a deterministic, repeatable Pipeline — fired manually, on a schedule, or from a webhook.</p></article><article class="bento-tile lead on-dark"><div><div class="bento-icon">🗺️</div><h3 style="font-size:20px">Coming next</h3><p>A formal 5-layer extension model for packaged customizations, an Approval Engine for records, and an Admin panel UX/UI consistency pass are next up for a decision — see exactly what's shipped vs. proposed, compiled from the working codebase, not a wishlist.</p></div><div style="align-self:center"><a class="btn btn-secondary" href="/roadmap">View the roadmap →</a></div></article></div></div></section>
 <section id="open-source" class="section"><div class="container cta"><div class="eyebrow" style="color:var(--brand)">Your data. Your models. Your OS.</div><h2>Inspect it. Run it. Improve it.</h2><p style="color:#cbd5e1;max-width:700px;margin:0 auto 24px">An open-source, self-hosted business platform and AI Agent Foundry — no subscription, no per-seat pricing, no vendor lock-in, and nothing here calls a model or ships a record anywhere until you configure it to.</p><div class="trust-row" style="justify-content:center;margin-bottom:6px"><span>✓ MIT licensed</span><span>✓ Public vulnerability disclosure policy</span><span>✓ Single local SQLite file, no telemetry in the product</span></div><div class="hero-actions" style="justify-content:center"><a class="btn btn-primary" href="/demo">Try the live demo →</a><a class="btn btn-secondary" href="https://github.com/vikram2409-eng/Lanesra-OS" target="_blank" rel="noopener">View GitHub repository</a></div></div></section>
 </main>${publicFooter()}
 </div>`;
 bindPublicNav();
 bindHomeCodeTabs();
...
function appShell(){
 ...
 $('#helpButton').onclick=()=>modal('Help & product links',`<div class="help-list"><a href="/compare">Compare Lanesra</a><a href="/roadmap">Roadmap & backlog</a><a href="/releases">Releases</a><a href="/">Product website</a><button class="btn btn-secondary" onclick="document.getElementById('modal').remove()">Close</button></div>`);
...
// Read-only rollup: Organization -> Organization Units -> Work Teams -> Users,
// in one place - the Organization/Organization Units/Work Teams/Users tabs
// above each manage one layer of this same hierarchy but never show how they
// actually connect. Purely a view over data already on the other four tabs -
// no new state, so it can't drift out of sync with what they show.
function orgHierarchyTab(body){
 const units=(data.orgUnits||[]).slice().sort((a,b)=>a.path.localeCompare(b.path));
 const teams=data.workTeams||[];
 const users=data.users||[];
 const activeMemberships=(data.teamMemberships||[]).filter(m=>!m.effectiveTo);
 const unitRow=u=>{
  const unitUsers=users.filter(x=>x.primaryOrgUnitId===u.id);
  const unitTeams=teams.filter(t=>t.primaryOrgUnitId===u.id);
  const usersHtml=unitUsers.length?`<ul class="org-hierarchy-list">${unitUsers.map(x=>`<li>👤 ${x.name}${x.role?` <span class="muted">(${x.role})</span>`:''}</li>`).join('')}</ul>`:'';
  const teamsHtml=unitTeams.length?unitTeams.map(t=>{
   const members=activeMemberships.filter(m=>m.teamId===t.id).map(m=>userName(m.userId));
   return `<div class="org-hierarchy-team"><div>🧑‍🤝‍🧑 <b>${t.name}</b> <span class="muted">${t.teamType}${t.status==='Inactive'?' · Inactive':''}</span></div>${members.length?`<ul class="org-hierarchy-list">${members.map(n=>`<li>👤 ${n}</li>`).join('')}</ul>`:'<p class="muted" style="margin:2px 0 0 20px;font-size:13px">No active members</p>'}</div>`;
  }).join(''):'';
  const empty=!unitUsers.length&&!unitTeams.length;
  return `<div class="org-hierarchy-unit" style="margin-left:${u.depth*24}px">
   <div class="org-hierarchy-unit-head">${u.depth>0?'<span class="muted">└ </span>':'🏢 '}<b>${u.name}</b> <span class="muted">${u.unitType}${u.status==='Inactive'?' · Inactive':''}</span></div>
   <div style="margin-left:20px">${usersHtml}${teamsHtml}${empty?'<p class="muted" style="font-size:13px;margin:2px 0">No users or teams directly in this unit</p>':''}</div>
  </div>`;
 };
 body.innerHTML=`<div class="panel"><h3 style="margin-top:0">Hierarchy</h3><p class="muted">Organization → Organization Units → Work Teams → Users, read-only - a rollup of the other four Access tabs, not a separate source of truth. Manage the pieces from Organization, Organization Units, Work Teams and Users &amp; roles.</p>
 <div style="margin-top:8px"><div class="org-hierarchy-org">🏛 <b>${data.workspace.name}</b> <span class="muted">Organization${data.workspace.orgCode?` · ${data.workspace.orgCode}`:''}</span></div>
 <div style="margin-top:10px;display:flex;flex-direction:column;gap:14px">${units.map(unitRow).join('')}</div></div>
 </div>`;
}
...
const REFERENCE_PACKAGES={
 ...
 // Eleventh package, added after the original ten on direct product
 // request - reduced mirror of the desktop edition's own
 // policy_admin_claims_manifest_json (reference_packages.rs), same
 // package_id/industry/object-and-field vocabulary, trimmed to this demo's
 // simpler engine the same way the eight packages above are: no
 // record_created trigger (desktop's "Claim intake" workflow is left out,
 // same reasoning as Real Estate's "Showing scheduled" etc. above) and no
 // date-to-date comparison operator (desktop's "Policy date validation"
 // business rule - expiration_date after effective_date - is left out,
 // same reasoning as Nonprofit's "Renewal integrity" above). policy_stage,
 // not status: every custom object here already has a fixed built-in
 // Active/Inactive/Archived status, the same collision desktop's own doc
 // comment on this package warns about.
 policy_admin_claims:{
  packageId:'lanesra.policy_admin_claims',name:'Policy Administration & Claims Management',industry:'Insurance',version:'1.0.0',
  ...
  appIcon:'🛡',
  objects:[
   {key:'policy',label:'Policy',labelPlural:'Policies',icon:'📋',prefix:'POL',digits:6},
   {key:'coverage',label:'Coverage',labelPlural:'Coverages',icon:'🛡',prefix:'COV',digits:5},
   {key:'insured_risk',label:'Insured Risk',labelPlural:'Insured Risks',icon:'📦',prefix:'RISK',digits:5},
   {key:'endorsement',label:'Endorsement',labelPlural:'Endorsements',icon:'✏',prefix:'END',digits:5},
   {key:'claim',label:'Claim',labelPlural:'Claims',icon:'🧾',prefix:'CLM',digits:6},
   {key:'claimant',label:'Claimant',labelPlural:'Claimants',icon:'🧑',prefix:'CLT',digits:5},
   {key:'loss_reserve',label:'Loss Reserve',labelPlural:'Loss Reserves',icon:'💰',prefix:'RES',digits:6},
   {key:'claim_payment',label:'Claim Payment',labelPlural:'Claim Payments',icon:'💳',prefix:'PAY',digits:6},
   {key:'adjuster_assignment',label:'Adjuster Assignment',labelPlural:'Adjuster Assignments',icon:'🕵',prefix:'ADJ',digits:5},
   {key:'subrogation',label:'Subrogation Case',labelPlural:'Subrogation Cases',icon:'🔁',prefix:'SUB',digits:5},
  ],
  fields:[
   ['policy','policy_stage','Status','select','Quote|Bound|In Force|Renewed|Non-Renewed|Cancelled|Expired|Lapsed'],
   ['policy','effective_date','Effective Date','date'],['policy','expiration_date','Expiration Date','date'],
   ['coverage','coverage_type','Coverage Type','select','Bodily Injury Liability|Property Damage Liability|Collision|Comprehensive|General Liability|Property|Workers Compensation|Umbrella|Other'],
   ['coverage','premium_amount','Premium','number'],
   ['insured_risk','risk_type','Risk Type','select','Vehicle|Real Property|Location|Equipment/Contents|Other'],['insured_risk','description','Description','text'],
   ['endorsement','endorsement_status','Status','select','Draft|Pending Approval|Approved|Processed|Rejected'],['endorsement','premium_change_amount','Premium Change','number'],
   ['claim','claim_status','Status','select','New|Under Investigation|Pending Documentation|Approved|Denied|In Litigation|Closed - Paid|Closed - Denied|Reopened'],
   ['claim','date_of_loss','Date of Loss','date'],['claim','loss_type','Loss Type','select','Collision|Fire|Theft|Water Damage|Wind/Hail|Liability - Bodily Injury|Liability - Property Damage|Vandalism|Weather/Catastrophe|Workers Comp Injury|Other'],
   ['claim','reserve_amount','Total Reserve','number'],['claim','paid_amount','Total Paid','number'],
   ['claim','fraud_referral','Fraud Referral','select','No|Under Review|Confirmed - SIU'],
   ['claimant','claimant_type','Claimant Type','select','First Party - Named Insured|First Party - Additional Insured|Third Party'],
   ['claimant','represented_by_attorney','Represented by Attorney','select','No|Yes'],['claimant','attorney_name','Attorney Name','text'],
   ['loss_reserve','reserve_type','Reserve Type','select','Indemnity|Expense|Legal/Defense'],['loss_reserve','reserve_amount','Reserve Amount','number'],
   ['claim_payment','payment_type','Payment Type','select','Indemnity|Expense|Deductible Recovery|Subrogation Recovery|Salvage Recovery'],
   ['claim_payment','payee_type','Payee','select','Insured|Claimant|Third Party|Vendor/Repair Shop|Attorney'],['claim_payment','payment_amount','Payment Amount','number'],
   ['adjuster_assignment','adjuster_role','Adjuster Role','select','Desk Adjuster|Field Adjuster|Independent Adjuster|SIU Investigator|Appraiser'],
   ['adjuster_assignment','assignment_status','Status','select','Assigned|Active|Completed|Reassigned'],
   ['subrogation','subrogation_status','Status','select','Identified|Demand Sent|In Negotiation|Recovered|Closed - No Recovery'],['subrogation','recovered_amount','Recovered Amount','number'],
  ],
  relationships:[
   {source:'policy',target:'Contact',relType:'many_to_one',forwardLabel:'Named Insured',reverseLabel:'Policies'},
   {source:'coverage',target:'policy',relType:'many_to_one',forwardLabel:'Policy',reverseLabel:'Coverages'},
   {source:'insured_risk',target:'policy',relType:'many_to_one',forwardLabel:'Policy',reverseLabel:'Insured Risks'},
   {source:'coverage',target:'insured_risk',relType:'many_to_one',forwardLabel:'Insured Risk',reverseLabel:'Coverages'},
   {source:'endorsement',target:'policy',relType:'many_to_one',forwardLabel:'Policy',reverseLabel:'Endorsements'},
   {source:'claim',target:'policy',relType:'many_to_one',forwardLabel:'Policy',reverseLabel:'Claims'},
   {source:'claim',target:'insured_risk',relType:'many_to_one',forwardLabel:'Insured Risk',reverseLabel:'Claims'},
   {source:'claimant',target:'claim',relType:'many_to_one',forwardLabel:'Claim',reverseLabel:'Claimants'},
   {source:'loss_reserve',target:'claim',relType:'many_to_one',forwardLabel:'Claim',reverseLabel:'Loss Reserves'},
   {source:'claim_payment',target:'claim',relType:'many_to_one',forwardLabel:'Claim',reverseLabel:'Payments'},
   {source:'adjuster_assignment',target:'claim',relType:'many_to_one',forwardLabel:'Claim',reverseLabel:'Adjuster Assignments'},
   {source:'adjuster_assignment',target:'Company',relType:'many_to_one',forwardLabel:'Adjusting Firm',reverseLabel:'Adjuster Assignments'},
   {source:'subrogation',target:'claim',relType:'many_to_one',forwardLabel:'Claim',reverseLabel:'Subrogation'},
   {source:'subrogation',target:'Company',relType:'many_to_one',forwardLabel:'Responsible Party Insurer',reverseLabel:'Subrogation Cases'},
  ],
  rules:[
   {entity:'claim',matchType:'all',
    conditions:[{fieldKey:'claim_status',operator:'equals',value:'Closed - Paid',compareField:null,groupId:null}],
    actions:[{type:'require',targetField:'paid_amount',value:'',message:''}]},
   {entity:'claimant',matchType:'all',
    conditions:[{fieldKey:'represented_by_attorney',operator:'equals',value:'Yes',compareField:null,groupId:null}],
    actions:[{type:'require',targetField:'attorney_name',value:'',message:''}]},
   {entity:'subrogation',matchType:'all',
    conditions:[{fieldKey:'subrogation_status',operator:'equals',value:'Recovered',compareField:null,groupId:null}],
    actions:[{type:'require',targetField:'recovered_amount',value:'',message:''}]},
  ],
  workflows:[
   {entity:'policy',matchType:'all',notify:false,
    conditions:[{fieldKey:'policy_stage',operator:'equals',value:'Bound',compareField:null,groupId:null}],
    actions:[{type:'create_task',taskTitle:'Issue policy documents to the named insured',daysOffset:1}]},
   {entity:'claim',matchType:'all',notify:true,
    conditions:[{fieldKey:'fraud_referral',operator:'equals',value:'Confirmed - SIU',compareField:null,groupId:null}],
    actions:[{type:'create_task',taskTitle:'Notify SIU and legal',daysOffset:1}]},
   {entity:'claim',matchType:'all',notify:false,
    conditions:[{fieldKey:'claim_status',operator:'in_list',value:'Closed - Paid|Closed - Denied',compareField:null,groupId:null}],
    actions:[{type:'create_task',taskTitle:'Send closure letter to claimant',daysOffset:2}]},
   {entity:'endorsement',matchType:'all',notify:false,
    conditions:[{fieldKey:'endorsement_status',operator:'equals',value:'Processed',compareField:null,groupId:null}],
    actions:[{type:'create_task',taskTitle:'Update billing for the premium change',daysOffset:1}]},
  ],
 },
};
...
// ---- Deployment Management Phase 2 mirror: Publishers -----------------------
...
// ---- Deployment Management Phase 3 mirror: component-tagging, Local
// Workspace, export ---------------------------------------------------------
...
// The Packaged/Custom distinction's Custom half: everything still
...
// ---- Deployment Management Phase 4 mirror: named, scoped Solutions --------
...
function importPackage(key){
 ...
 if(!publisher){
  ...
  return alert(`'${wantKey}' isn't a registered publisher in this workspace yet - register it under Admin → Deployment Management → Publishers before importing a package under that namespace.`);
 }
...
function installReferencePackage(key){
 ...
 // Deployment Management's Components tab mirror: every id created below is
...
function packagesTab(body){
 ...
 <p class="muted">Bundled reference packages - each creates a small, working set of Custom Objects, fields, relationships, business rules and workflow automation for a specific industry, plus an App grouping them together. Click Details to preview what one contains, or Import it straight away.</p>
 ${catalogKeys.length?`<div style="display:flex;gap:12px;flex-wrap:wrap">${catalogKeys.map(k=>{const p=REFERENCE_PACKAGES[k];const open=previewPackageKey===k;return `<div class="panel" style="flex:${open?'1 1 100%':'1'};min-width:240px;background:var(--surface-alt,#f7f8fc)"><div style="font-weight:700">${p.appIcon} ${p.name}</div><p class="muted" style="font-size:12px">${p.description}</p><p class="muted" style="font-size:12px">${p.objects.length} objects · ${p.fields.length} fields · ${p.relationships.length} relationships · ${p.rules.length} rules · ${p.workflows.length} workflows</p><div class="actions"><button class="btn btn-secondary" data-import="${k}">Import</button><button class="icon-btn" data-preview="${k}">${open?'Hide details':'Details'}</button></div>${open?packageDetailsHtml(p):''}</div>`}).join('')}</div>`:'<div class="empty">Every starter package has been imported or installed.</div>'}
 </div>
...
// ---- Deployment Management (Admin IA design spec) ----------------------------
// Mirrors the desktop edition's DeploymentManagementAdmin.tsx: a read-only
...
// built here either, matching desktop: a real Packaged/Custom
...
function solutionsTab(body){
 ...
 body.innerHTML=`<div class="panel">
 <h3 style="margin-top:0">Deployment Management</h3>
...
function roadmapPage(){
 ...
 // from one flat list of 7 categories to 4 pillars is the actual
 // navigability fix requested here - someone scanning for "how does this
 // connect to other systems" now finds one pillar heading instead of
 // hunting through 7 same-weight accordions for the right one.
 const shippedPillars=[
  ['▣','Core Product','Data model, deployment and document workflow — the foundation everything else is built on.',[
   ['🔐','Organization & Access Control — Phases 1 &amp; 2 of 6',[['Organization, Organization Units & Work Teams','One Organization per workspace containing Organization Units in a materialized-path hierarchy (regions, divisions, teams) — create, edit, and move a unit with an impact preview showing exactly how many descendant units and owned records the move affects before you confirm it. Work Teams sit alongside the hierarchy with time-bounded memberships; ending a membership never reassigns anything the member owned — a deliberate, tested guarantee.'],['Record ownership — Owner & Owning Organization Unit on every record','Every Company, Contact, Opportunity, Product, Quote, Order, Invoice, Contract, Task and Custom Object record now carries a structured Owner (a user or a team, not free text) and an Owning Organization Unit, defaulted to the creating user and their org unit on create. Change it on an individual record or reassign in bulk across a multi-select; an ownership-version counter and timestamp advance only on a real transfer, never a no-op save. Phase 1 of a six-phase Enterprise Access Foundation — Phase 2\'s capability-scoped Access Roles ship right below.'],['Access Control v1 — capability-scoped Access Roles, Record Scopes &amp; an Access Inspector','Replaces the Phase 1 placeholder — "Assign requires the Administrator role" — with a real engine: named, composable Access Roles carrying a capability set (per-object create/read/update/delete/assign) and a Record Scope (Owner-only, Owner\'s Team, Owner\'s Org Unit and below, or Organization-wide). Every workspace lazily bootstraps Full Access (for existing Administrators) and Standard User (for everyone else, seeded at Organization scope so today\'s open CRUD behavior never regresses) the first time any check runs. An Access Inspector answers "why can/can\'t this user see this record" from the exact same evaluation trace enforcement uses, not a second guess.'],['Access Control v1, extended — CRUD gating &amp; an additive legacy-check migration','Record-level Create/Read/Update/Delete gating now runs on all 10 owned business objects and Custom Object records, checked one record at a time (create/update/delete/single-record-read). The dozen existing <code>require_admin</code>-style checks in other subsystems (Organization Units, Custom Objects, Workflow Automation, Business Rules, Custom Fields, Custom Reports, Status Transitions, Numbering, the Workspace profile, Backup/Restore, Work Teams, Relationships, App Builder) were migrated onto the new engine additively — the legacy Administrator role still always passes unchanged, and a non-Administrator can only additionally act with an explicit Access Role grant on that exact object, never via a wildcard fallback. Zero behavior change for any workspace that hasn\'t deliberately granted a new capability.'],['Access Control v1, extended further — list views, search, dashboards &amp; reports','Closes the one gap the single-record-only pass above deliberately left open: list screens, global search, dashboard record-list widgets and custom report counts/sums are now narrowed to each viewer\'s Access Role Read scope too, reusing the identical evaluator (<code>access_service::filter_visible</code>) single-record enforcement already calls rather than a second check. Filtering happens before truncation, not after, so a dashboard widget or report total is never silently under-filled or wrong by counting a record the viewer can\'t see. Related-record lists and aggregate KPI dashboard tiles are the next, narrower remainder — not done here, named honestly rather than implied.']]],
  ...
  ['✦','AI & Agentic Layer','Bring your own LLM key, then build on it — an MCP server for outside agents, a conversational assistant, a full AI Agent Foundry with memory, skills, actions, guardrails, delegation and orchestration (sequential, consensus or peer-review, with human-in-the-loop approval gates and a real Evaluation Harness), a Unified AI Gateway for multi-provider routing/failover/budgets, and a Context Layer for memory history, ranked search, an agent context primer and embeddings-based Vector Search.',[
   ['🔑','Bring-your-own LLM key, MCP Server & CLI',[['Bring-your-own LLM key','A workspace supplies its own Anthropic or OpenAI-compatible API key (openai_compatible also covers a self-hosted, fully offline model server) - Lanesra never resells or proxies inference, the same "own your data" posture the rest of the product already takes. The key is stored exactly like any other secret (AES-256-GCM, never returned by any read command), with a one-click Test key connectivity check, and nothing else in the product calls an LLM until this is configured.'],['MCP Server & CLI','A stateless <code>POST /mcp</code> Model Context Protocol server (Team Workspace mode) exposes 7 tools - list/get objects and records, create/update/archive a record - to any MCP-speaking external agent (Claude Desktop, an IDE agent, a custom script), authenticated with the same scoped API-client bearer keys and rate-limited/logged the same way the generic REST API already is. A companion <code>lanesra</code> CLI gives humans and shell scripts the same access without an agent in the loop.']]],
   ['🕒','Unified Activity Timeline & Agent Actions',[['Unified Activity Timeline','A generic activities log - one entry per email, call or message, tied to a Company, Contact or Opportunity - the foundation later agent features read from. Distinct from the audit trail (which records what a user changed, not what happened around a record from outside it); ships with manual logging first, with an email/call/Slack auto-capture connector named as a deliberate follow-on, not silently skipped.'],['Agent Actions: natural-language reporting','Ask a plain-English question ("revenue by month this quarter") and the workspace\'s configured LLM translates it into the exact shape the existing Custom Report builder already understands - one object, one group-by field, count or sum - validated through the identical path a human\'s manual report creation goes through, then rendered live with a one-click Save as report. The first of four proposed Agent Actions; meeting-prep briefings, commitment capture and record-hygiene suggestions stay proposed until a live Activity channel exists for them to draw on.']]],
   ['💬','LLM Chat Assistant',[['Conversational assistant — records & admin, full tool coverage','A chat assistant for every user (a "records" mode with the same 7 read/write tools MCP exposes) and, for Administrators, an "admin" mode with ~17 tool pairs spanning Business Rules, Workflow Automation, Custom Objects/Fields/Relationships, Integration Hub, Dashboards, Apps, Reports, Saved Views, Users and more - each a thin wrapper over that subsystem\'s own already-validated function, so the assistant genuinely builds a workflow or a business rule for you, not just describes how. One non-negotiable boundary: a Connection\'s or API client\'s secret is never read from a tool call and is redacted a second time before it ever reaches chat history.']]],
   ['🏭','AI Agent Foundry & Orchestration',[['AI Agent Foundry — Agents, Memory, Skills, Actions, Delegation','A dedicated admin section for named, reusable AI Agents: a persona layered over a per-tool Actions checklist (granting any admin-catalog tool makes an agent Administrator-only, computed at runtime), a persistent Memory document the agent reads every run and can revise itself, a shared Skills library (name + description shown up front, full instructions loaded only when the agent decides to use one), and delegation to other agents - a supervisor agent calling named sub-agents and getting their answer back, depth-guarded against runaway recursion. Chat with any agent you\'re allowed to use from the main Assistant page.'],['Orchestration — Pipelines, Triggers & workflow-fired agent runs','Chain agents into a deterministic, admin-authored Pipeline - each step\'s input can reference the previous step\'s real output or the run\'s own trigger input - complementary to an agent\'s own dynamic delegation above, not redundant with it. Trigger a Pipeline or a lone Agent manually, on a schedule, from an inbound authenticated webhook, or as a new "Run AI agent" Workflow Automation action - the same enqueue-now/drain-later pattern already proven for Connector Actions, so a workflow firing inside a record save never blocks on a live LLM call. Every run - manual, scheduled, webhook or workflow-triggered - writes to one unified run history, step by step.'],['Guardrails & declarative export/import','An agent\'s own <code>guardrails_md</code> is injected into its system prompt as an explicit operational-boundary statement the model is instructed to respect - advisory, the same real/advisory split Business Rules and an agent\'s own persona text already draw - alongside one genuinely code-enforced guard: the tool-calling loop aborts with a clear error the moment the same tool call with the same arguments repeats 3 times in a row. An agent or Skill (persona, Memory seed, guardrails, Actions, Skills and delegates) can be added to a Solution, exported, and imported into another workspace exactly like a Custom Object or Business Rule already can be - its Gateway routing policy and live token-usage history don\'t travel with it, since those are workspace-specific runtime state, not portable definition.'],['Consensus & Peer-Review orchestration, with a real Evaluation Harness','Two more Pipeline topologies alongside the original "sequential": "consensus" runs every step but the last independently against the same trigger input, then a synthesizer step combines every candidate\'s real answer; "peer_review" loops a drafter and a reviewer (draft, critique, revise) until the reviewer approves, or 3 rounds pass without approval, failing clearly rather than looping forever. A new Evaluation Harness runs a named Suite of golden test Cases - an input plus a plain-English success criteria - against one Agent or Pipeline, each graded by an LLM-as-judge call rather than a brittle exact-match comparison.'],['Human-in-the-loop approval gates & OTLP tracing','Any Pipeline topology can pause for an Administrator\'s review at the one step it has a well-defined resume point for - any step of a sequential chain, a consensus pipeline\'s synthesizer, or a peer-review pipeline\'s reviewer (every round) - who then approves (optionally editing the paused step\'s own output before it feeds the rest of the run) or rejects it outright. Real per-step timing renders a finished run as a standard OTLP <code>resourceSpans</code> trace, exportable as JSON or pushed on demand to a configured collector endpoint.'],['Integration Hub Tool Bridge — Connector Actions as agent tools','An agent\'s Actions checklist could already list/manage Connectors but never call one - closed with a real, typed bridge from an imported OpenAPI operation straight to an LLM tool-call definition. Fails closed on an ambiguous schema by construction: a Connector Action with a request body only becomes a tool once its body schema is confidently typed (a real <code>properties</code> map, no unresolved <code>$ref</code>/<code>oneOf</code>/<code>anyOf</code>/<code>allOf</code>) - a vague or unresolvable body schema is never turned into a guessable tool. Phased exactly as scoped: read-only actions (GET/HEAD/OPTIONS) need only a per-connector opt-in plus one Connection Reference the tool executes against; write-capable actions need a second, separate opt-in and make the whole agent Administrator-only the moment one is granted - the identical "one admin-catalog action gates the agent" rule the fixed record/admin tool catalogs already enforced, now covering connector-derived tools too.']]],
   ['🌐','Unified AI Gateway',[['Multi-provider routing, failover & budgets','Named, admin-managed AI Providers beyond a single workspace default - Anthropic, an OpenAI-compatible endpoint, or Google Gemini - with per-agent primary/fallback/local-fallback routing: a request automatically fails over on an error or rate limit, all the way down to a fully local, air-gapped model if configured. Real per-(workspace, agent, user, day) token-usage accounting is enforced against a daily budget at both the System and Agent tier, with a Gateway health view showing today\'s usage and every failover event and its reason.'],['DLP-driven forced air-gapping','An agent can name sensitive data classes - SSN, credit card number, bank account, phone, email - that route its request straight to the local-fallback tier the moment a scan detects one, regardless of the normal routing order. An agent with no routing policy configured keeps its exact pre-Gateway behavior - purely additive, not a breaking change to the Agent Foundry or Orchestration above.']]],
   ['🔎','Context Layer',[['Real memory-change history','The prior value of an agent\'s persistent Memory is snapshotted before every overwrite - whether from the agent\'s own <code>update_memory</code> tool or an admin\'s direct edit - so what an agent has revised about itself over time is auditable and reviewable from a "Show memory history" panel, most-recent-first. No snapshot for a genuine no-op (empty or unchanged).'],['Ranked full-text search over Custom Object records','A new <code>search_records</code> tool gives agents ranked full-text search over Custom Object records via a SQLite FTS5 index kept in sync by triggers - <code>bm25()</code> lexical relevance, a fast, always-on complement to Vector Search\'s newer, meaning-based ranking below. Scoped to Custom Objects for this pass; built-in entities and MCP exposure are named next steps, not silently skipped.'],['Agent context primer — relationships & a platform overview','<code>get_object_metadata</code> includes every Custom Relationship an object participates in, from either direction, with the correct label and related object already resolved; <code>get_related_records</code> follows one for a specific record, the same lookup the desktop UI\'s own "Related records" panel uses. A new <code>get_platform_overview</code> tool explains how Lanesra OS\'s own primitives compose (Custom Object → Fields → Relationships → Business Rules → Workflow Automation → App → Dashboard) plus a live count of this workspace\'s own objects, relationships and apps - orientation an agent can call before proposing or building anything unfamiliar.'],['Vector Search — semantic search over records','Real embeddings from the workspace\'s own already-configured provider (OpenAI-compatible or Google Gemini - Anthropic has no embeddings API) power <code>semantic_search_records</code>, a meaning-based ranking over Custom Object records that finds a relevant record even when it shares none of a query\'s exact words. No vector database or native extension: vectors are plain BLOBs in SQLite, ranked by cosine similarity in Rust; a record\'s own create/update/archive enqueues a reindex rather than blocking on a live provider call. Desktop only, opt-in (Admin → LLM & MCP → Vector Search).']]],
   ]],
 ];
 // Flatten pillars → categories for indices, search, filter chips and
 // JSON-LD - the pillar is a rendering/navigation grouping, not a new
 // identity, so every category keeps exactly the id/behavior it had
 // before this restructure (`cat-${flatIndex}`), just nested one level
 // deeper in the DOM.
 const flatCats=[];
 shippedPillars.forEach((pl,pIdx)=>pl[3].forEach(c=>flatCats.push({pillarIdx:pIdx,icon:c[0],title:c[1],items:c[2]})));
 const shippedTotal=flatCats.reduce((n,c)=>n+c.items.length,0);
 // "Just shipped" spotlight points at the category holding the newest
 ...
 // itself and longer titles still fit a pill - derived from the title
 // itself rather than a second parallel array, so it can never drift out
 // of sync with it.
 ...
 const planned=[
  ...
  ['Agent-as-code: exportable, git-trackable agent definitions','Every AI Agent today lives only as a database row, edited through Admin → AI Agents - not as a file a developer can diff or PR-review alongside the rest of a team\'s git-tracked configuration. Scoped as a <b>one-way, generated export</b>: Admin → AI Agents stays the sole source of truth; an "Export as files" action writes a persona/instructions file, a memory seed and a guardrails file derived from that row, for git history and review only. Re-importing goes through the existing Solution export/import pipeline exactly like any other component - never a background file-watcher feeding changes back into the DB - so there\'s no dual-source-of-truth conflict to resolve.','AI Agent Foundry','L'],
  ['Evaluation Harness: a CLI and a release gate','The Evaluation Harness already runs a named Suite of golden Cases against an Agent or Pipeline, graded by an LLM-as-judge, from Admin → Evaluations - but only interactively, one click at a time. A <code>lanesra agent eval &lt;suite&gt;</code> CLI command producing a machine-readable score report, plus a documented way to fail a CI job or a Solution release when a score regresses past a defined threshold, turns the harness a maintainer already has into an actual pre-deploy safety net - the same promote-after-tests-pass discipline Deployment Management already gives configuration changes, extended to agent behavior. Shares its underlying "agents as reviewable files" investment with Agent-as-code above rather than solving that problem twice.','AI Agent Foundry','M'],
  ['Connector Template Library: data warehouses &amp; the OAuth/form-body remainder','Phase 1 shipped five curated templates - see Shipped, below. Still open: data warehouses (Snowflake, BigQuery, Databricks - Redshift needs no new work at all, since it\'s wire-compatible with the existing PostgreSQL connection type already) need their own per-vendor auth-mode check before being added, since both Snowflake and BigQuery commonly use JWT/key-pair auth that doesn\'t map onto the existing <code>auth_mode</code> enum the way Phase 1\'s Bearer-token services did. Also deferred: Stripe/Twilio, whose classic APIs use <code>application/x-www-form-urlencoded</code> bodies rather than JSON (this parser degrades a non-JSON body to "opaque" with a warning, so those actions wouldn\'t come out Tool-Bridge-eligible today); Google Gemini, whose API key is a <code>?key=</code> query parameter that none of the 7 existing <code>auth_mode</code>s can inject; and the OAuth2-authorization-code group (HubSpot, Salesforce, Google Sheets, Notion, Airtable) - mechanically supported by Connections already, just not yet curated into templates.','Integration Hub','M'],
  ['PostgreSQL for Team Workspace & Cloud SaaS','SQLite\'s single-writer lock is a real ceiling for a multi-user Team Workspace and a hard blocker for a genuine multi-tenant SaaS tier - both move to PostgreSQL; Desktop keeps SQLite unchanged, since a single local user has no concurrency problem to solve. Team Workspace runs vanilla, self-hosted Postgres, bundled into Lanesra\'s own setup (one <code>docker compose up</code>, no separate DB install the operator has to perform by hand). The (future) SaaS tier uses <b>one Postgres database per tenant</b> - the strongest tenant-isolation guarantee available, and it keeps the server\'s existing one-workspace-per-connection assumption completely unchanged - hosted on a serverless-native Postgres platform rather than vanilla self-managed instances, since Lanesra would be operating many small, mostly-idle tenant databases provisioned the instant someone signs up. See the <a href="/help/multi-target-architecture">Architecture &amp; Extensibility</a> documentation category for the full reasoning and the principles this is built against.','Architecture','XL'],
 ];
 const proposed=[
  ['The 5-layer extension model & destructive uninstall','The remaining scope from the Solution Packages design spec, now that component-tagging, the Local Workspace/Custom grouping, export and update-with-diff have all shipped (see Shipped, below): a formal 5-layer extensibility model (Core → Packaged → Packaged Extension → Custom Extension → Local Override) so a packaged component can be deliberately, safely extended without editing it destructively - today any admin can already add a field to a package-installed object with no restriction, which covers the common case but has no concept of "this field belongs to a different layer than the object it\'s on." Also still open: destructive uninstall with a dependency/data-disposition review (only non-destructive deactivate/reactivate exist), and <code>is_locally_customized</code> drift detection (package_artifacts has had the column since the beginning; nothing sets it yet - update-with-diff\'s new Modified classification is the natural signal to wire it from).','Needs its own design pass on what a "layer" concretely changes about how a component behaves or renders, beyond the ownership tagging that already exists - the risk is building a taxonomy with no visible difference from what component-tagging (just shipped) already answers.','App Catalog','M'],
  ...
  ['Access Control v1.1: Field Access Policies, Capability Packs &amp; Record Grants','Phase 3 of the Enterprise Access Foundation, now that Phase 2\'s Access Role/Record Scope engine and Access Inspector have shipped (see Shipped, above) for these to hang off of. Field-level read/write policies beyond today\'s all-or-nothing record access; reusable Capability Packs/Bundles so an admin assigns a named bundle instead of hand-picking every capability per role; Access Groups (a named set of users, independent of the Org Unit hierarchy, for cross-cutting access like "everyone on the Q3 launch"); and Record Grants — a one-off, time-bounded share of a specific record to a specific user or team outside their normal scope, the exception valve every scope-based system eventually needs.','Field Access Policies in particular need a decision on evaluation order against Business Rules\' own field-level show/hide/lock effects, so the two systems don\'t silently fight each other. See <a href="https://github.com/vikram2409-eng/Lanesra-OS/issues/142">Issue #142</a>.',null,'L'],
  ['Access Control v1.2: Record Access Policies &amp; stronger bulk-audit','Phase 4. Declarative Record Access Policies - admin-defined rules computing a record\'s effective access without hand-wiring it into every path the way Phase 1\'s owner/org-unit fields do today - plus a materially stronger audit trail for bulk reassignment specifically: today\'s bulk transfer already writes one <code>audit_events</code> row per record (see Shipped, above), but the spec\'s v1.2 scope asks for a dedicated before/after diff view over a whole bulk operation, not just per-record rows a maintainer has to reassemble by timestamp.','Needs real usage of Phase 1\'s bulk transfer and Phase 2/3\'s role and grant system first, so the policy language is designed against actual access patterns rather than guessed ahead of them. See <a href="https://github.com/vikram2409-eng/Lanesra-OS/issues/143">Issue #143</a>.',null,'M'],
  ['AI Access v1: AI Action Levels &amp; Agent/Integration Identities','Phase 5. Today an AI Agent\'s capability gate is binary — granting any admin-catalog tool makes the whole agent Administrator-only (see AI Agent Foundry, above). AI Access v1 replaces that with graded AI Action Levels (read-only, propose-a-change-for-approval, autonomous-write) evaluated per tool call, plus a real Agent/Integration Identity distinct from the human user who configured it, so an agent\'s own actions carry their own attributable identity in the audit trail and in Phase 2\'s Access Role/scope evaluation (shipped - see Shipped, above), not the configuring admin\'s.','Phase 2\'s Access Role engine now exists as an evaluation substrate to build against; still needs a decision on how an AI Action Level composes with the existing per-connector read/write Tool Bridge opt-in (Integration Hub, above) rather than duplicating it. See <a href="https://github.com/vikram2409-eng/Lanesra-OS/issues/144">Issue #144</a>.','AI Agent Foundry','L'],
  ['Enterprise Identity v2: OIDC/SAML/SCIM &amp; external users','Phase 6, the largest remaining phase. Real enterprise identity federation — OIDC/SAML single sign-on and SCIM-provisioned user lifecycle — beyond the simpler "Optional Google/Microsoft sign-in" proposed above (which is a per-workspace convenience toggle, not directory-driven provisioning), plus a first-class External User concept (a customer or partner with narrowly scoped access to their own records only) that today\'s all-internal user model has no place for.','The self-hosted, no-shared-SaaS deployment model (see PostgreSQL item, above) means every organization registers its own IdP app — real setup friction to design around rather than assume away. External Users also need Phase 2\'s Record Scope model finished first, so "see only your own records" has a real enforcement mechanism to plug into. See <a href="https://github.com/vikram2409-eng/Lanesra-OS/issues/145">Issue #145</a>.',null,'XL'],
 ];
 const futureIdeas=['Projects and milestones','Inventory and suppliers','Recurring invoices','Customer portal','Plugin architecture','A pluggable vector backend (e.g. pgvector) for large Team Workspaces'];

 // The agentic-infrastructure backlog review (2026-09-12): all 7 epics that
 // came out of checking a proposed "agentic OS" feature list against the
 // working codebase, decided one by one. This used to live only in an
 // external artifact linked from "Now building" below - it's inlined here
 // instead so the reasoning behind every planned/skipped/held item is on
 // this page itself, not one click away on a third-party host.
 // Shape: [title, status('skip'|'hold'|'proceed'), verdict, forList[], againstList[], decision, links[[label,href],...]]
 const backlogEpics=[
  ['Unified AI Gateway','skip',
   'Already fully built. Named Providers, per-agent primary → fallback → local-fallback routing, System + Agent daily token budgets, and DLP-driven forced air-gapping across 5 sensitive-data classes are all live in Admin → LLM &amp; MCP → Gateway.',
   ['Diffable, reviewable in a PR (a manifest.yaml-style config)'],
   ['A second source of truth for a setting the database already stores correctly — can silently drift from what\'s actually loaded'],
   'Configuration stays UI-only. No independent file source — a YAML export could be generated from it later, never read back as an input.',
   []],
  ['Agent Foundry &amp; Orchestration','proceed',
   'The "Invoke Agent" workflow node is already live as the <code>run_ai_agent</code> Workflow Automation action. What was actually still missing: <b>Agent-as-code</b> — agents exist only as database rows today, edited in Admin → AI Agents, with no git-trackable file format.',
   ['Diffable, PR-reviewable agent behavior','Fits a technical team\'s existing git workflow'],
   ['A live, two-way file sync would create a second source of truth with no clean conflict owner'],
   'Scoped as a one-way, generated export — the Admin UI stays the sole source of truth. "Export as files" writes persona/memory/guardrails files for git history only; re-importing goes through the existing Solution pipeline, never a background sync.',
   [['Issue #118','https://github.com/vikram2409-eng/Lanesra-OS/issues/118']]],
  ['Vector RAG &amp; Context Layer','hold',
   'Already works. Vector Search auto-enqueues a reindex on every record write once a workspace opts in — this isn\'t a missing pipeline.',
   ['Real ANN search once records reach real scale (sqlite-vss / pgvector)'],
   ['sqlite-vss needs a compiled native extension per OS','No confirmed workspace has hit the brute-force scan\'s ceiling yet'],
   'Don\'t build ahead of the problem. Left as an unscoped future idea (see Ideas below) — the right trigger is a real latency complaint, not a hypothetical one.',
   []],
  ['Integration Hub Tool Bridge','proceed',
   'An agent\'s Actions checklist is a fixed, hand-registered catalog (8 record tools, 37 admin pairs) — nothing exposes a configured Connector Action or Workflow action as an agent tool. <b>The single highest-leverage item in this review</b> — pure translation over infrastructure that already exists end to end.',
   ['Real-world reach into whatever a workspace has already connected','Zero custom integration code'],
   ['A vague OpenAPI schema risks a hallucinated tool call','Write access means real-world blast radius, not just this workspace\'s records'],
   'Phased. Read-only Connector Actions exposed as tools first; write-capable actions gated behind a separate, explicit per-connector opt-in, Administrator-only.',
   [['Issue #117','https://github.com/vikram2409-eng/Lanesra-OS/issues/117']]],
  ['Model Context Protocol (MCP)','proceed',
   'MCP (<code>POST /mcp</code>) exists only on the Team Workspace HTTP server — a solo desktop install, the majority of this product\'s actual users, has zero MCP access today. (Already shipped alongside this review: OpenAI &amp; Google Gemini support for the Chat Assistant, identical to Anthropic.)',
   ['Closes MCP access for most of the install base','stdio is what real desktop MCP clients expect'],
   ['New process-lifecycle surface across 3 platforms'],
   'Build a local stdio bridge, reusing the existing tool dispatcher and auth. Deliberately stdio-only — SSE is being phased out of the MCP spec, and a new local HTTP listener would break the local-first, no-open-ports posture.',
   [['Issue #116','https://github.com/vikram2409-eng/Lanesra-OS/issues/116']]],
  ['Evaluation Harness &amp; Guardrails','proceed',
   'The harness already grades a Suite of golden Cases via LLM-as-judge from Admin → Evaluations — but only interactively, one click at a time.',
   ['Turns an existing tool into a real pre-deploy safety net','Matches Deployment Management\'s promote-after-tests-pass discipline'],
   ['A CI runner is neither an interactive session nor an obvious API-client holder — needs its own auth story'],
   'A <code>lanesra agent eval &lt;suite&gt;</code> CLI plus a documented CI/release regression gate. Bundled with Agent-as-code above — both need the same "agents as reviewable files" investment.',
   [['Issue #119','https://github.com/vikram2409-eng/Lanesra-OS/issues/119']]],
  ['PostgreSQL for Team Workspace &amp; Cloud SaaS','proceed',
   'Grew out of a separate architecture discussion, not the original six-epic list — SQLite\'s single-writer lock is a real ceiling for a multi-user Team Workspace and a hard blocker for a genuine multi-tenant Cloud SaaS tier. <b>The largest single item in this review.</b>',
   ['Real concurrent multi-user writes, which SQLite structurally can\'t give either target','Database-per-tenant gives SaaS the strongest tenant-isolation guarantee available'],
   ['Distributed SQL (Yugabyte, CockroachDB) was considered and set aside — the actual ask was ordinary SaaS remote access, which database-per-tenant Postgres already serves','Real migration work porting SQLite-specific SQL to Postgres for the Team Workspace build'],
   'Desktop unchanged (SQLite). Team Workspace: vanilla self-hosted Postgres, bundled into setup (one <code>docker compose up</code>, no separate DB install). Cloud SaaS (future): one Postgres database per tenant, hosted on a serverless-native platform for fast provisioning and idle-cost scaling.',
   [['Issue #120','https://github.com/vikram2409-eng/Lanesra-OS/issues/120'],['Architecture documentation','/help/multi-target-architecture']]],
 ];
 const backlogOrder=[
  ['MCP stdio bridge &amp; Tool Bridge (read-only)','Tied for first — together they close "agents have no real reach outside this workspace."'],
  ['Tool Bridge, write-access phase','A deliberate, separately-gated second phase once read-only is proven out.'],
  ['Agent-as-code + Evaluation Harness CLI','Bundled — same underlying file-based, git-trackable investment.'],
  ['PostgreSQL for Team Workspace &amp; Cloud SaaS','Tracked as issue #120 — the largest single lift on this list, so sequenced last despite being fully decided.'],
 ];
 const backlogStatusLabel={skip:'Skip',hold:'On hold',proceed:'Proceed'};
 ...
 const shippedListItems=flatCats.map((c,i)=>({'@type':'ListItem',position:i+1,name:plainText(c.title),description:`${c.items.length} shipped item${c.items.length===1?'':'s'}, including ${truncate(c.items[0][0],80)}`}));
 setPageJsonLd('roadmap-jsonld',{
  ...
  description:'Everything shipped, being built next, and still proposed for Lanesra OS - CRM, no-code platform, App Catalog, Deployment Management, Integration Hub - compiled from the working codebase.',
 ...
 $('#app').innerHTML=`<div class="site-dark">${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Built from the working codebase</div><h1>Roadmap & backlog.</h1><p>What's being built next, what's still waiting on a decision, and everything already shipped — compiled from the actual code (core/server/src-tauri/frontend) plus the online demo, not a wishlist. Every "Shipped" entry below is running code with tests.</p><div class="status-row"><span class="status-chip">Early Access v0.60.0</span><span class="muted">Last updated September 2026</span></div>
 <a class="rm-spotlight" href="#cat-${latestCatIndex}"><span class="rm-spotlight-eyebrow"><span class="rm-spotlight-dot"></span>Just shipped</span><span class="rm-spotlight-title"><span class="rm-cat-icon">${latestCat.icon}</span>${latestCat.title}</span><span class="rm-spotlight-arrow">See what's in it →</span></a>
 <div class="backlog-callout" id="desktop"><h3>Release status</h3><p><b>desktop-v0.13.0 is the latest tagged release</b> (installers attached, Early Access/prerelease as intended), cut with everything from Saved Views & Bulk Actions (v0.36.0) through the Context Layer (v0.45.0) — the whole no-code platform, App Catalog, Deployment Management, Integration Hub, and the AI & Agentic Layer through Phase 7b (Unified AI Gateway, Agent Foundry, Orchestration, memory history + ranked search). Everything since - agent guardrails + declarative export/import, consensus/peer-review orchestration with a real Evaluation Harness, human-in-the-loop approval gates extended to every topology, OTLP tracing + collector push, an agent context primer (relationship metadata, <code>get_related_records</code>, <code>get_platform_overview</code>), Vector Search, an eleventh industry package (Policy Administration & Claims Management), Field Service deepened past its original scope, a twelfth shared cross-industry package (Lanesra Industry Foundation) with Property Management retrofitted onto it, engine hardening (self-referential &amp; polymorphic relationships, cross-record conditions, effective-dating), and the Integration Hub Tool Bridge (Connector Actions as AI Agent tools), the Connector Template Library, and the Enterprise Access Foundation (Organizations, Organization Units, Work Teams &amp; Record Ownership) - all the way through v0.60.0 below - is merged to <code>main</code> but not yet cut into a tagged installer. Everything below reflects the working codebase on <code>main</code>, regardless of installer/tag status. Full desktop feature list → <a href="/download">/download</a>, full release history → <a href="/releases">/releases</a>.</p><p class="muted">Repo hygiene: a real MIT <code>LICENSE</code>, <code>CONTRIBUTING.md</code>, <code>CODE_OF_CONDUCT.md</code>, <code>SECURITY.md</code>, issue/PR templates, and a root README written for someone landing on the repo, not a deploy runbook.</p></div>
 </div></section>
 <nav class="rm-subnav" aria-label="Roadmap sections"><div class="container narrow rm-subnav-inner"><a href="#building-next"><span class="rm-subnav-n">${planned.length}</span>Now building</a><a href="#backlog-analysis"><span class="rm-subnav-n">${backlogEpics.length}</span>Backlog analysis</a><a href="#deciding"><span class="rm-subnav-n">${proposed.length}</span>Deciding</a><a href="#future-ideas"><span class="rm-subnav-n">${futureIdeas.length}</span>Ideas</a><a href="#shipped" class="done"><span class="rm-subnav-n">${shippedTotal}</span>Shipped</a></div></nav>
 <section class="section"><div class="container narrow">

 <section class="rm-section" id="building-next"><div class="rm-section-head"><div class="eyebrow">In progress · up next</div><h2>What's being built next.</h2><p>Scoped and ready to build — no open product question left to resolve first. Five of these six items came out of a full agentic-infrastructure backlog review against the working codebase — see <a href="#backlog-analysis">Backlog analysis</a> below for the pros/cons and rationale behind every decision, including the ones that didn't make the cut.</p></div>${planned.length>0?planned.map(p=>`<div class="rm-hero-card"><div class="rm-kicker"><span class="dot"></span>Next up</div><h3>${p[0]}</h3><p>${p[1]}</p><div class="rm-tags"><span class="backlog-tag planned-tag">planned</span><span class="backlog-tag">size: ${p[3]}</span></div></div>`).join(''):`<div class="rm-empty">Nothing scoped and ready right now — every remaining item needs a product/architecture decision first. See "Deciding" below.</div>`}</section>

 <section class="rm-section" id="backlog-analysis"><div class="rm-section-head"><div class="eyebrow">How we got here</div><h2>The agentic-infrastructure backlog review.</h2><p>Seven epics for turning Lanesra OS into an AI-native operating system, checked one by one against the working codebase — what was already shipped, what was genuinely missing, and what we decided to build. Reviewed 2026-09-12.</p></div>
  <div class="backlog-legend"><span><i class="dot proceed-dot"></i>Proceed — now building</span><span><i class="dot hold-dot"></i>On hold</span><span><i class="dot skip-dot"></i>Skip</span></div>
  <div class="backlog-epics">${backlogEpics.map((e,i)=>`<article class="backlog-epic ${e[1]}"><div class="backlog-epic-head"><span class="backlog-epic-num">0${i+1}</span><h3>${e[0]}</h3><span class="backlog-tag ${e[1]}-tag">${backlogStatusLabel[e[1]]}</span></div><p class="ask">${e[2]}</p><div class="backlog-pc"><div class="backlog-solution"><div class="sol-label">For</div><ul>${e[3].map(x=>`<li>${x}</li>`).join('')}</ul></div><div class="backlog-solution"><div class="sol-label">Against</div><ul>${e[4].map(x=>`<li>${x}</li>`).join('')}</ul></div></div><div class="backlog-solution decision"><div class="sol-label">Decision</div><p>${e[5]}</p></div>${e[6].length?`<div class="backlog-epic-links">${e[6].map(l=>`<a href="${l[1]}"${l[1].startsWith('http')?' target="_blank" rel="noopener"':''}>${l[0]} →</a>`).join('')}</div>`:''}</article>`).join('')}</div>
  <div class="backlog-order"><h3>Build order</h3><ol class="order">${backlogOrder.map((o,i)=>`<li><strong>${o[0]}</strong><span>${o[1]}</span></li>`).join('')}</ol></div>
 </section>

 <section class="rm-section" id="deciding"><div class="rm-section-head"><div class="eyebrow">Proposed</div><h2>Big calls, waiting on a decision.</h2><p>Explicitly deferred, not forgotten — each needs a product or architecture decision before it's scoped enough to build. <b>The 5-layer extension model</b> is the one worth deciding first: Deployment Management's foundation (component-tagging, Local Workspace, export, update-with-diff) has already shipped — see <a href="#shipped">Shipped</a> below — so this is the natural next layer on top of it.</p></div><div class="rm-card-grid">${proposed.map(p=>`<div class="backlog-card"><div class="backlog-card-head"><h3>${p[0]}</h3><div class="backlog-card-tags"><span class="backlog-tag proposed-tag">proposed</span>${p[3]?`<span class="backlog-tag">${p[3]}</span>`:''}<span class="backlog-tag">size: ${p[4]}</span></div></div><p class="ask">${p[1]}</p><div class="backlog-solution"><div class="sol-label">Why it's still just proposed</div><ul><li>${p[2]}</li></ul></div></div>`).join('')}</div></section>
 ...
 <section class="rm-section" id="shipped"><div class="rm-section-head"><div class="eyebrow">Shipped</div><h2>Everything already running in production.</h2><p>${shippedTotal} epics across ${shippedPillars.length} pillars, all merged to <code>main</code> — every line below is running code with tests, not a changelog promise.</p></div>
 <div class="rm-pillar-nav">${shippedPillars.map((pl,i)=>`<a href="#pillar-${i}" class="rm-pillar-nav-item"><span class="rm-pillar-nav-icon">${pl[0]}</span><span class="rm-pillar-nav-text"><span class="rm-pillar-nav-title">${pl[1]}</span><span class="rm-pillar-nav-count">${pl[3].reduce((n,c)=>n+c[2].length,0)} shipped</span></span></a>`).join('')}</div>
 ...
 <div class="rm-filter-chips">${flatCats.map((c,i)=>`<button type="button" class="rm-filter-chip" data-cat="${i}"><span class="rm-cat-icon">${c.icon}</span>${shortCatLabel(c)}</button>`).join('')}</div>
 ${(()=>{let i=-1;return shippedPillars.map((pl,pIdx)=>`<div class="rm-pillar" id="pillar-${pIdx}"><div class="rm-pillar-head"><h3><span class="rm-pillar-icon">${pl[0]}</span>${pl[1]}</h3><p>${pl[2]}</p></div>${pl[3].map(c=>{i++;const idx=i;const isDefault=idx===latestCatIndex;return `<details class="rm-cat" id="cat-${idx}"${isDefault?' open data-default-open':''}><summary><h4 class="rm-cat-label"><span class="rm-cat-icon">${c[0]}</span>${c[1]}</h4><span class="rm-cat-meta"><span class="rm-cat-count">${c[2].length}</span><span class="rm-chevron"></span></span></summary><div class="rm-cat-body"><div class="backlog-shipped-list">${c[2].map(s=>`<div class="backlog-shipped-item"><div class="mark">✓</div><div><h5 class="t">${s[0]}</h5><div class="d">${s[1]}</div></div></div>`).join('')}</div></div></details>`}).join('')}</div>`).join('')})()}
 </section>
...
// add risk for no benefit), hides non-matching items, collapses any
// category left with zero matches, and hides a whole pillar wrapper once
// every one of its categories is hidden. Clearing the box restores the
// default state (only the newest category open, every pillar visible).
// Filter chips and pillar-nav links are quick-jumps, not filters - they
// open and scroll to one category/pillar, same as the hero spotlight.
function bindRoadmapSearch(){
 ...
 const pillars=[...document.querySelectorAll('#shipped .rm-pillar')];
 ...
 input.oninput=()=>{
  ...
  if(!q){
   cats.forEach(cat=>{cat.style.display='';cat.open=cat.hasAttribute('data-default-open');cat.querySelectorAll('.backlog-shipped-item').forEach(item=>item.style.display='')});
   pillars.forEach(p=>{p.style.display=''});
   countEl.textContent='';
   return;
  }
  let matches=0;
  pillars.forEach(pillar=>{
   let pillarAny=false;
   pillar.querySelectorAll('details.rm-cat').forEach(cat=>{
    let any=false;
    ...
    cat.open=any;
    if(any)pillarAny=true;
   });
   pillar.style.display=pillarAny?'':'none';
  });
  countEl.textContent=matches?`${matches} match${matches===1?'':'es'}`:'No matches';
 };
}
function releasesPage(){document.title='Releases — Lanesra OS';
 setPageMeta('The full Lanesra OS changelog - every shipped release across the desktop app, the online demo and this website, compiled directly from the working codebase. See also the desktop-only installer tags on GitHub, cut less often than this list.');
 const releases=[['v0.62.0','September 2026','Access Control v1, extended: online demo mirror &amp; list/search/dashboard/report enforcement',['Two follow-ups to v0.61.0\'s Access Control v1, both requested as explicit next steps. First, the online demo now mirrors the real capability x Record Scope engine - the demo\'s pre-existing but illustrative-only role permission matrix (none/read/read-write per object) is replaced with the same shape the desktop edition enforces (five capabilities x four Record Scopes, with a <code>\'*\'</code> wildcard default), wired into a real client-side evaluator at the one seam that matters (reassigning a record\'s owner, both the bulk action and the record-edit form), plus a new Access Inspector tab. Second, and more consequential: list views, global search, dashboard record-list widgets and custom report counts/sums are now narrowed to each viewer\'s Access Role Read scope too - the one gap v0.61.0\'s single-record-only pass deliberately left open, closed with the identical evaluator (<code>access_service::filter_visible</code>/<code>filter_visible_mixed</code>) single-record enforcement already uses, not a second check.','The list-level filtering runs before truncation or aggregation, not after - a dashboard widget or a report\'s count/sum would otherwise silently under-fill or misreport by first taking the top N records irrespective of visibility, then filtering what\'s left. Threaded through 10 owned-object list commands (desktop + Team Workspace server), global search, `dashboard_widget_service::run`, and `custom_report_service::run`/`run_with_as_of`. Related-record lists and aggregate KPI dashboard tiles are the one deliberate remainder, named honestly rather than implied as done.','Shipped on the desktop edition (4 new Rust tests covering `filter_visible`, mixed-type search filtering, dashboard pre-truncation filtering, and report-count accuracy) and the online demo (Playwright-verified: role editor, Access Inspector allow/deny traces, bulk reassign, and a live block on a non-Administrator\'s reassignment attempt).']],['v0.61.0','September 2026','Access Control v1: capability-scoped Access Roles, Record Scopes &amp; an Access Inspector',['Phase 2 of the six-phase Enterprise Access Foundation, right behind Phase 1\'s Organization/Org Units/Work Teams/record ownership (see below). Replaces the placeholder "Assign requires the Administrator role" gate <code>ownership_service::require_assign_capability</code>\'s own doc comment named as exactly this phase\'s seam with a real capability-and-scope engine: named, composable <b>Access Roles</b> carrying a capability set (per-object create/read/update/delete/assign, defaulting to a <code>\'*\'</code> wildcard row so a role never needs re-seeding when a new custom object is added) and a <b>Record Scope</b> - Owner-only, Owner\'s Team, Owner\'s Org Unit and below, or Organization-wide, each a strict superset of the one before it. Every workspace lazily bootstraps two system roles the first time any check runs - <b>Full Access</b> (every capability, Organization scope) for everyone holding the legacy Administrator role, <b>Standard User</b> for everyone else - and every new user is assigned Standard User automatically, so there\'s no empty-permissions cliff on upgrade.','An <b>Access Inspector</b> answers "why can/can\'t this user see this record" from a real evaluation trace, not guesswork - <code>access_service::explain_access</code> is the single evaluator both enforcement and the Inspector call, so the explanation is provably the same decision that was actually enforced, not a second parallel guess. Extended past the original Phase 2 scope at the same time, following two explicit scope decisions: record-level Create/Read/Update/Delete gating now runs on every one of the 10 owned business objects (Companies, Contacts, Opportunities, Products, Quotes, Orders, Invoices, Contracts, Tasks) plus Custom Object records - checked against one specific record at a time (create/update/delete/single-record-read), deliberately <i>not</i> filtering list views, search, dashboards, reports or related-record lists, which stay unchanged. Standard User\'s default grant was deliberately seeded at Organization scope, not Owner, specifically so this introduces zero regression to today\'s already-open CRUD behavior - only the Assign capability was ever actually restricted before this shipped.','The dozen or so existing <code>require_admin</code>-style checks scattered across other subsystems (Organization Units, Custom Objects, Workflow Automation, Business Rules, Custom Fields, Custom Reports, Status Transitions, Numbering, the Workspace profile, Backup/Restore, Work Teams, Relationships, App Builder) were migrated onto the new engine <i>additively</i>, not replaced: the legacy Administrator role still always passes, completely unchanged, and a non-Administrator can now <i>additionally</i> act only if an Access Role explicitly grants Update on that exact object type - never falling back to a role\'s <code>\'*\'</code> wildcard default the way ordinary record CRUD does, since that fallback leaking into schema/automation/org-structure admin screens would be a real, unintended widening rather than "same as today." Zero regression risk for every existing workspace: nothing changes unless an admin deliberately grants a new capability to a non-Administrator.','Shipped on the desktop edition (Rust core, migration 0056, new <code>access_role_service</code>/<code>access_service</code>, a new Access Roles admin screen and Access Inspector under Settings → Access, 14 new Rust tests covering scope-level evaluation, multi-role folding, the CRUD no-regression guarantee, and the additive legacy-check migration). Field Access Policies, Capability Packs, Access Groups and Record Grants (Phase 3) are still tracked as a GitHub issue on the roadmap, not built yet (see Roadmap).']],['v0.60.0','September 2026','Enterprise Access Foundation: Organizations, Organization Units, Work Teams &amp; Record Ownership',['Phase 1 of a six-phase Enterprise Requirements Specification, scoped down to exactly what was asked for rather than attempting all six phases at once - an Organization (one per workspace) containing Organization Units in a materialized-path hierarchy (<code>/root-id/child-id/.../</code> + depth, so a subtree move is one bounded <code>UPDATE ... WHERE path LIKE ?</code>, not a second closure-table structure to keep in sync) and Work Teams with time-bounded memberships. Ending a membership never touches any record\'s ownership - a regression the new Rust test suite checks directly, and a real bug (a missing <code>ON DELETE CASCADE</code> on the team-membership foreign key, which blocked deleting a team forever once any membership had ever existed, even an ended one) was caught by that same suite before it shipped.','Every record across Companies, Contacts, Opportunities, Products, Quotes, Orders, Invoices, Contracts, Tasks and Custom Objects now carries a structured Owner (a user or a team, <code>record_owner_type</code>/<code>record_owner_id</code> - not free text) and an Owning Organization Unit, defaulted to the creating user and their org unit on create. An ownership-version counter and an <code>assigned_at</code> timestamp advance only on a real reassignment, never a no-op save - another off-by-one the test suite caught (the column defaulted to 1, so every brand-new record already looked "transferred once"; fixed to default 0, with the legacy-owner backfill itself counted as the one real assignment). Bulk dry-run/commit transfer reuses the existing <code>bulk_action_service::BulkActionResult</code> shape rather than inventing a parallel one, and every transfer writes one <code>audit_events</code> row, reusing the existing generic audit table instead of a new Security Activity feed.','The desktop admin panel gets three new screens under Settings → Access: Organization, Organization Units (a tree view with create/edit and a move-with-impact-preview showing descendant-unit and owned-record counts before you confirm), and Work Teams (CRUD plus membership add/end). Owner and Owning Organization Unit fields land on every record\'s create/edit form. The legacy <code>owner_user_id</code> column on the five tables that already had one is kept, read-only, as a rollback safety net rather than dropped - SQLite can\'t rename/drop a column safely without a full table rebuild. The "who can reassign" capability check is a deliberate placeholder requiring the existing Administrator role, marked in code as the exact seam Phase 2\'s real capability-and-scope Access Role engine replaces.','Shipped on the desktop edition first (Rust core, migrations 0051-0056, 25 new tests) and mirrored in full in the online demo as real client-side state against <code>data</code>/<code>localStorage</code>, the same convention every other demo feature already follows - its own Organization/Organization Units/Work Teams admin screens, the same Owner/Owning Org Unit pickers on every record form, and the same bulk-reassignment flow. Phases 2 through 6 of the spec - capability-scoped Access Roles, Field Access Policies, Record Grants, AI Action Levels, and Enterprise Identity/SSO - are tracked as GitHub issues on the roadmap, not built yet (see Roadmap).']],
  ['v0.59.0','September 2026','Connector Template Library, Phase 1: five curated pre-built connectors',['Import connector now opens on a template gallery instead of only a blank paste box - five hand-curated, hand-trimmed OpenAPI specs (OpenAI, Cohere, Slack, GitHub, SendGrid) an admin can start from, landing in the exact same preview/select-operations/import flow that already existed. Picking one calls a new <code>connector_template_service::get_spec</code> and pre-fills the wizard; nothing about <code>connector_service::preview_import</code>/<code>import</code> changed.','Every template was chosen for a confirmed real-world fit with this codebase\'s existing Connection auth model - <code>Authorization: Bearer &lt;token&gt;</code> plus a JSON request body - not assumed. Verifying actual API docs (rather than trusting memory) surfaced two real mismatches worth designing around: Stripe/Twilio\'s classic APIs use <code>application/x-www-form-urlencoded</code> bodies, which this parser degrades to "opaque" with a warning; Google Gemini authenticates via a <code>?key=</code> query parameter, which none of the 7 existing <code>auth_mode</code>s can inject. Both are deliberately not in this batch.','Every template\'s request-body schema is fully inline (no <code>$ref</code>), so every imported action is immediately eligible for the Integration Hub Tool Bridge (v0.58.0) the moment an admin opts a connector in, not just Workflow Automation. A new <code>connector_templates.rs</code> test file asserts this automatically for every template - a zero-warning parse and a non-empty, informative body schema on every action with one - rather than just claiming it in a doc comment.','Data warehouses (Snowflake, BigQuery, Databricks), the OAuth2-authorization-code SaaS group (HubSpot, Salesforce, Google Sheets, Notion, Airtable), and the Stripe/Twilio/Gemini cases above stay on the roadmap, each with the specific reason it\'s not in this phase stated plainly rather than left unstated.']],['v0.58.0','September 2026','Integration Hub Tool Bridge: Connector Actions as AI Agent tools',['An AI Agent\'s Actions checklist could already list and manage Connectors but never call one - closed with a real bridge from an imported OpenAPI operation straight to a typed LLM tool-call definition, reusing <code>connector_execution_service::execute</code> unchanged (the same function a Tauri "Test Action" button and Workflow Automation\'s own <code>call_connector_action</code> already call).','Fails closed on an ambiguous schema, per the roadmap\'s own stated requirement: <code>connector_service::import</code> now stores a request body\'s real, resolved JSON Schema in the previously-dead <code>request_schema_json</code> column whenever it\'s fully local (no unresolved <code>$ref</code>/<code>oneOf</code>/<code>anyOf</code>/<code>allOf</code>); a new <code>connector_tool_service</code> only turns a body-bearing action into a tool when that stored schema is also genuinely informative (a real <code>properties</code> map, not just a bare <code>{"type":"object"}</code>) - two separate fail-closed checks, not one.','Phased exactly as scoped: an admin opts a connector into agent-tool use and names the one Connection Reference its tools execute against; read-only (GET/HEAD/OPTIONS) actions need only that opt-in, while write-capable actions need a second, separate opt-in. Tool names self-describe by prefix (<code>connector_action:</code> vs. <code>connector_write_action:</code>), so granting even one write-capable action makes the whole agent Administrator-only - the identical rule the fixed record/admin tool catalogs already enforced, now covering connector-derived tools too.','New migration 0050 adds three columns to <code>integration_connectors</code> (<code>agent_tools_enabled</code>, <code>agent_write_tools_enabled</code>, <code>agent_reference_key</code>), mirroring the <code>is_active</code>-flag pattern already established for AI Agents/Skills rather than overloading <code>Connection.status</code>\'s existing test-result semantics. Desktop edition only, like the rest of the Integration Hub - the online demo simulates that layer client-side with no server to make a real call from, so there is nothing honest to mirror here.']],['v0.57.0','September 2026','Engine hardening: self-referential & polymorphic relationships, cross-record conditions, effective-dating',['Five platform-primitive gaps closed together, since each pays off retroactively across all twelve Industry Data Model packages rather than one vertical - see <code>reference_packages.rs</code>\'s own running gap list, now marked closed but kept as the historical record of what each one worked around before this.','A relationship definition\'s source and target can now be the same object type - a parent/child hierarchy - with <code>link()</code> guarding the one real constraint that shape needs: a record can\'t link to itself. A definition can also be flagged <code>target_is_polymorphic</code>, accepting any valid workspace type as its target chosen per link (a Document attached to a Policy, a Claim or a Matter) instead of one fixed type - read paths (related-record listing, metadata) fully support this; writing to the variable side is a stated follow-up, not silently wrong output.','A Business Rule or Workflow condition can now carry a <code>relationship_definition_id</code>, reading a field off the record linked through a many-to-one/one-to-one relationship instead of only the triggering record\'s own - resolved into a per-rule scratch copy of the evaluation context so a sibling rule\'s own related-field read never collides. <code>date_reached</code>/<code>due_overdue</code> workflow triggers now also work on any Custom Object\'s own date field, not just a fixed built-in set.','A new <code>effective_dating_service</code> answers whether a record\'s <i>currently stored</i> Valid From/Valid To window is in effect as of a given date - not true point-in-time reconstruction of a past field value, which this schema has no history table to support, stated plainly rather than faked - wired into the custom report builder\'s new "as of" filter.','Two related validation gaps found and fixed while adding tests, not just the five items themselves: condition authoring-time validation for both engines was checking <code>field_key</code> against the triggering record\'s own fields even when a relationship was selected, and <code>date_reached</code>/<code>due_overdue</code>\'s own validation still rejected every custom-object date field even after the runtime matching logic gained support for it - both made the affected feature uncreatable through the ordinary admin screens until fixed. 5 new Rust tests across the relationship, business-rule, workflow and custom-object test files, plus the full existing suite, 660 tests green. Desktop edition only - not mirrored in the online demo.']],['v0.56.0','September 2026','Property Management retrofitted onto Foundation (v1.1.0)',['The first business vertical actually rebuilt on a real package dependency rather than the synthetic manifest that first proved the mechanism works: <b>Property Management</b> now declares a real, enforced <code>dependencies</code> entry on <b>Lanesra Industry Foundation</b> - install now hard-fails without Foundation installed and active first.','Its Owner relationship (previously targeting the built-in Company) now targets Foundation\'s Party instead - a single primary owner, person or organization - and it gains four new objects the spec\'s own deeper Property Management section calls for: <b>Owner Interest</b> (a junction object carrying each co-owner\'s fractional ownership percentage, since a plain relationship can\'t itself carry a split across several owners), <b>Lease Amendment</b>, <b>Security Deposit</b> and <b>Violation / Notice</b>. <code>maintenance_request</code> also gains an optional relationship to Foundation\'s shared <code>service_case</code>, for workspaces that want one unified cross-industry case record on top of its own maintenance-specific fields.','Lease Amendment, Security Deposit and Violation are also mirrored in the online demo; Owner Interest is desktop-only, since it needs Foundation\'s Party object, which the demo has no dependency-checking to support. 14 tests across the package\'s own test file.']],['v0.55.0','September 2026','Lanesra Industry Foundation — a twelfth, shared cross-industry package',['Different in kind from the eleven business-vertical packages before it: <b>Party</b>, <b>Party Role</b>, <b>Party Relationship</b>, <b>Location</b>, <b>Asset</b>, <b>Agreement</b> and more - shared plumbing other packages can optionally declare a real dependency on and relate to, instead of each reinventing its own version of the same concept.','First real use of the manifest format\'s <code>dependencies</code> field: <code>industry_package_service::validate</code> now enforces a required dependency (installed, active, version-satisfying) before install or an update runs - previously declarable in a manifest but never actually checked.','Several Foundation objects (Document Record, External Identifier, Interaction, Financial Transaction, Data Quality Issue) want a polymorphic relationship target the engine didn\'t have yet at the time - degraded to a plain, unenforced pair of text fields for this release, called out as a named candidate for the engine-hardening pass that shipped a few releases later. Desktop edition only - not mirrored in the online demo, which has no dependency-checking of any kind for any package. 16 tests across the package\'s own test file.']],['v0.54.0','September 2026','Field Service, deepened past its original scope',['Checked against how ServiceTitan and Salesforce Field Service Lightning actually shape the rest of a real field service management business, beyond the original Work Order/Appointment/Asset/Warranty Claim core: a pre-authorization <b>Estimate</b> (with its own approve/decline lifecycle) a Work Order can now optionally point back at, a recurring <b>Maintenance Plan</b> an Asset can be covered by and a Work Order can be generated under, per-technician <b>Time Entries</b> for job costing and payroll, and an <code>is_callback</code> flag on Work Order marking a return visit for unresolved prior work.','Recurring Work Order generation on a schedule would need a <code>date_reached</code> trigger on a custom object\'s own date field - this engine didn\'t have one at the time (closed two releases later by engine hardening) - so which visits happen when stays a dispatcher decision for now; the plan only records the agreement\'s own terms and which asset/customer it covers.','Estimate and Maintenance Plan are also mirrored in the online demo, each trimmed to its own status field plus one or two more, the same ratio every other reduced-mirror object already uses; Estimate Line and Time Entry are left out of the demo entirely, both plain child-record objects (line items, labor hours) the demo already omits the equivalent of elsewhere. 14 tests across the package\'s own test file.']],['v0.53.0','September 2026','Policy Administration & Claims Management: eleventh industry reference package',['Insurance policy admin and claims, added on direct product request - carriers and MGAs weren\'t in the original ten-vertical spec - checked against real PAS/claims practice: Guidewire PolicyCenter/ClaimCenter, Duck Creek, and the ACORD data standards.','Objects for the policy admin side - <b>Policy</b>, <b>Insured Risk</b>, <b>Coverage</b> (optionally scoped to one specific insured risk, or applying at the policy level), <b>Endorsement</b> (the PAS term of art for a mid-term policy change) - and the claims side, mirroring a real claim-file shape: one <b>Claim</b> against a Policy, one or more <b>Claimant</b>s (first- or third-party), <b>Loss Reserve</b> tracked separately from <b>Claim Payment</b> (real claims reserving keeps Indemnity/Expense/Legal reserves distinct from what\'s actually been paid), one or more <b>Adjuster Assignment</b>s, and an optional <b>Subrogation Case</b> when a third party is responsible for the loss.','Business rules and workflows for the real lifecycle events a PAS/claims system enforces - policy date-order validation, claim intake and closure, endorsement processing - described in the App Catalog\'s plain-language Details preview the same way every other package\'s automation already is. Mirrored in the online demo, trimmed to that simpler engine the same way the other ten packages already are. 12 tests across the package\'s own test file.']],['v0.52.0','September 2026','AI & Agentic Layer: Vector Search - embeddings-based semantic search over records',['Real embeddings from the workspace\'s own already-configured provider (OpenAI-compatible or Google Gemini - Anthropic has no embeddings API) power a second, meaning-based ranking over Custom Object records (<code>semantic_search_records</code>) alongside the existing FTS5 <code>bm25()</code> lexical one (<code>search_custom_records</code>, unchanged). This reverses this project\'s own previously-stated "no vector database, bm25 is enough at this scale" position - a deliberate choice for this pass, stated plainly rather than walked back silently.','Still no vector database or native extension: embeddings are plain BLOBs in SQLite, ranked by brute-force cosine similarity in Rust. A synchronous SQL trigger can\'t call an embeddings provider, so a record\'s own create/update/archive only ever enqueues a reindex; the real provider call happens in the same drain tick that already handles AI agent runs.','New admin panel (Admin -> LLM & MCP -> Vector Search): an embedding-model override, live embedded/pending counts, and a "Reindex now" action.','Desktop-only - not mirrored in the online demo, since its AI is simulated via keyword matching rather than a real embeddings call.','6 new Rust tests: reindexing requires admin, Anthropic\'s own lack of an embeddings API surfaces one clear error, creating a record enqueues it and reindexing clears the queue, archiving a record deletes its embedding immediately, and semantic ranking by real cosine similarity - not keyword overlap - proven against a controlled embedding stub.']],['v0.51.0','September 2026','AI & Agentic Layer, Phase 7g: approval gates on consensus and peer-review pipelines',['Extends Phase 7e\'s human-in-the-loop approval gates to the two topologies 7e explicitly left out, at the one step each has a well-defined resume point for: a consensus pipeline\'s synthesizer step (pausing after every candidate has run, before it does, with the joined <code>candidate_outputs</code> as the resumable value an admin can edit) and a peer-review pipeline\'s reviewer step (pausing after every round\'s verdict, before it\'s checked for "APPROVED", so an admin can edit feedback or force approval by editing it to start with that word).','<code>validate_pipeline_input</code>, the two topologies\' own execution functions, and <code>approve_pending_step</code> are all topology-aware now - resuming dispatches to whichever function the paused run\'s pipeline topology actually needs, rather than assuming sequential.','Frontend: the "Needs approval" checkbox now appears only on the one step each topology actually supports it on, and switching a pipeline\'s topology clears it from any step that no longer qualifies rather than silently carrying forward a flag the backend would reject on save.','4 new Rust tests: a consensus run pausing before the synthesizer and resuming with the joined candidate outputs (editable), a peer-review run pausing after every round\'s verdict and completing once an approved verdict is itself approved, and forcing approval by editing a verdict to start with "APPROVED" without running another round.']],['v0.50.0','September 2026','AI & Agentic Layer: an agent context primer - relationship metadata and a platform overview tool',['<code>get_object_metadata</code> now includes a <code>relationships</code> list - every Custom Relationship an object participates in, from either direction, with the forward/reverse label and related object key already resolved to the correct side - and a new <code>get_related_records</code> tool follows one for a specific record (the same <code>relationship_service::related_records_for</code> the desktop UI\'s own "Related records" panel already calls). A relationship is a link, never a raw field on either side\'s own record, so schema-level and data-level visibility are two distinct, necessary tools.','A new <code>get_platform_overview</code> tool: a static explainer of how Lanesra OS\'s primitives compose (Custom Object -> Fields -> Relationships -> Business Rules -> Workflow Automation -> App -> Dashboard) plus a live per-workspace summary (object/relationship/app counts) - orientation an agent can call before proposing or building anything unfamiliar, instead of guessing at how the platform works.','An incidental, separate gap fixed along the way: the fixed "admin" chat assistant previously had no access to <code>record_tools()</code> at all - only a named Foundry agent could mix both catalogs. Its tool list is now <code>record_tools()</code> + <code>admin_tools()</code>, routed per call by which catalog a name actually belongs to.','3 new Rust tests: relationship metadata read correctly from both directions, <code>get_related_records</code> following a link either way, and a full tool round trip proving the platform overview - including its live counts - reaches the model.']],['v0.49.0','September 2026','AI & Agentic Layer, Phase 7f: push a run\'s OTLP trace to a configured collector',['Closes the gap Phase 7e deliberately left open: a workspace configures an OTLP collector endpoint (Admin -> LLM & MCP -> Gateway, its own dial alongside the System token budget), then pushes any run\'s trace to it with one click from that run\'s own history.','<code>push_run_trace_to_otlp</code> is the same "Test Connection"-style shape <code>webhook_service::attempt_delivery</code> already uses for a real outbound call - timed out, reporting the collector\'s real response back to the admin, rather than a delivery queue with a persisted history. A one-off admin-triggered push earns a synchronous result, not the retry/delivery-log infrastructure a real webhook subscription has.','Desktop-only, like the OTLP feature it extends - the online demo has no real trace data or outbound-push capability to mirror this against.','3 new Rust tests: the endpoint setting is admin-gated and a blank value clears it rather than storing an empty string, pushing with nothing configured fails with a clear message naming where to configure it, and pushing against a real stub collector proves the exact JSON sent matches the trace export\'s own output and surfaces a collector\'s real failure response back to the caller.']],['v0.48.0','September 2026','AI & Agentic Layer, Phase 7e: human-in-the-loop approval gates + OTLP tracing',['A sequential Pipeline step can now be flagged <code>requires_approval</code>. The run pauses right after that step (status <code>awaiting_approval</code>) instead of continuing automatically, and an Administrator later approves - optionally editing the step\'s output before it feeds the next step - or rejects the run outright, from anywhere: a real approval can come from an entirely separate request much later, so the run\'s own trigger input and resume point are persisted, not recomputed.','Real wall-clock <code>started_at</code>/<code>finished_at</code> timing per step, plus a new <code>run_to_otlp_json</code> rendering a finished run as a standard OTLP <code>resourceSpans</code> trace (one root span for the run, one child span per step, deterministic trace/span ids) - genuinely OTel-shaped, so it interops with a real collector once piped there manually.','Online demo mirror: the <code>requires_approval</code> checkbox\'s pause/resume is a synchronous <code>confirm()</code> dialog rather than a real persisted pause-and-resume-later state machine, since the demo has no separate later request an approval could arrive on. Per-step duration is shown too.','6 new Rust tests: a gated step pausing the run with a non-admin unable to act on it, approving unchanged resumes with the gated step\'s real output (verified in the real outbound request, not just the stored text), approving with an edit overrides it, rejecting stops the run and further action on it is refused, <code>requires_approval</code> rejected outside sequential (until Phase 7g), and the OTLP export producing a deterministic, byte-identical 3-span trace across repeated exports.']],['v0.47.0','September 2026','AI & Agentic Layer, Phase 7d: consensus + peer-review orchestration, and a real Evaluation Harness',['Two new Pipeline topologies alongside "sequential" (unchanged, still the default): "consensus" - every step but the last is a candidate, run independently against the same trigger input (never chained to each other, since there\'s no single "previous" among peers), and the last step synthesizes every candidate\'s answer via a new <code>{{candidate_outputs}}</code> placeholder; "peer_review" - exactly two steps, a drafter and a reviewer, looping (draft, critique, revise) until the reviewer\'s answer starts with "APPROVED", or 3 rounds pass without approval, failing clearly rather than looping forever.','A new Evaluation Harness (<code>ai_eval_service</code>): a named Suite of golden test Cases - an input plus a plain-English success criteria - run against one Agent or Pipeline, each graded by an LLM-as-judge call rather than a brittle exact-match comparison an open-ended agent response could never satisfy. A malformed judge reply is graded as failed, fail-closed rather than silently passing.','7 new Rust tests across two new files: step-count validation per topology, a real consensus run proving both candidates saw the same trigger input and the synthesizer\'s real request carried both real answers, peer review approving on round 1 and exhausting 3 rounds with round 2\'s draft genuinely carrying round 1\'s real feedback, and a full eval run grading one real pass and one real fail against the judge\'s real verdict.']],['v0.46.0','September 2026','AI & Agentic Layer, Phase 7c: agent guardrails + declarative export/import',['A new <code>guardrails_md</code> field per agent, injected into the system prompt as an explicit "Operational guardrails (must not be violated)" section - advisory, not independently enforced, the same real/advisory split this project already draws between Business Rules and an agent\'s own persona text.','One genuinely code-enforced guard alongside it: <code>run_agent_once</code>\'s tool-calling loop now tracks each <code>(tool_name, arguments)</code> pair across consecutive rounds and aborts with a clear error the moment the same call repeats 3 times in a row, rather than silently exhausting the round cap.','Declarative agent export/import: new <code>"ai_agent"</code>/<code>"ai_skill"</code> Solution component types, reusing the existing Solution/package manifest mechanism rather than inventing a parallel directory-of-files format - agents and skills are referenced by name across workspaces. An agent\'s memory seed, guardrails, actions, skills and delegates all round-trip through export/import into a second workspace; its Gateway routing policy and live token-usage history don\'t, since those are workspace-specific runtime state, not portable definition.','5 new Rust tests: <code>guardrails_md</code> persistence and its appearance in the system prompt, the loop guard tripping before the round cap, and a full Solution export round-trip recreating an agent - with its skill and delegate - in a second workspace.']],['v0.45.0','September 2026','AI & Agentic Layer, Phase 7b: the Context Layer — memory history & ranked search',['Real history for an agent\'s persistent Memory: the prior value is snapshotted before every overwrite - whether from the agent\'s own always-available <code>update_memory</code> tool or an admin\'s direct edit - so what an agent has revised about itself over time is auditable and reviewable from a new "Show memory history" panel, most-recent-first. No snapshot is written for a genuine no-op (the prior value was empty, or identical to the new one).','A new <code>search_records</code> tool gives agents ranked full-text search over Custom Object records, backed by a SQLite FTS5 index kept in sync via triggers on the underlying tables - <code>bm25()</code> lexical relevance, deliberately not embeddings or a vector database: this product doesn\'t have one and isn\'t adding one just for this. Scoped to Custom Objects for this pass; built-in entities and MCP exposure are named next steps, not silently skipped.','Both capabilities are a ranked/audited evolution of code that already existed - <code>search_service::global_search</code>\'s own unranked substring scan, and <code>update_memory</code>\'s previously-silent overwrite - not a parallel system bolted on beside them.','8 new Rust tests covering memory-history snapshotting across the agent-tool and admin-edit paths, ranked-search relevance and scoping, archived-record exclusion, and a full agent tool round-trip verified against the model\'s own next-turn request.']],['v0.44.0','September 2026','AI & Agentic Layer, Phase 7a: the Unified AI Gateway',['Named, admin-managed AI Providers beyond a single workspace-wide default, with per-agent model routing across primary/fallback/local-fallback tiers - a request automatically fails over on an error or a rate limit, all the way down to a fully local, air-gapped model if configured.','Real per-(workspace, agent, user, day) token-usage accounting, enforced against a daily budget at both the System (workspace) and Agent tier - a new Gateway health view shows today\'s usage and every failover event, with its reason.','DLP-driven forced air-gapping: an agent can name sensitive data classes (SSN, credit card number, bank account, phone, email) that route its request straight to the local-fallback tier regardless of the normal routing order, the moment a scan detects one in the request.','Google Gemini joins Anthropic and OpenAI-compatible as a third supported cloud provider. Every agent that had no routing configured keeps its exact pre-Gateway behavior - this phase is additive, not a breaking change to Phase 6a/6b. 15 new Rust tests alongside the full existing suite, all green.']],['v0.43.0','September 2026','AI & Agentic Layer, Phase 6b: Orchestration — Pipelines, Triggers & workflow-fired agent runs',['Pipelines chain agents into a deterministic, admin-authored, ordered sequence - each step\'s input can reference the previous step\'s real output or the run\'s own trigger input - complementary to Phase 6a\'s dynamic delegation, not redundant with it: a Pipeline is fixed and repeatable, delegation is the model\'s own improvised choice.','Triggers cover schedule (an interval, checked by the same background job scheduler Integration Jobs already uses) and an inbound authenticated webhook (<code>POST /api/v1/agents/:id/trigger</code>, Bearer-authenticated and scoped like every other REST API route) - both alongside the always-available manual Run.','A new "Run AI agent" Workflow Automation action fires a Pipeline or Agent from a record create/update/status-change/etc - enqueued, never run inline, since a workflow fires synchronously inside a record save. One shared drain processes every non-interactive trigger kind; an incidental latent bug (an equivalent drain for Connector Actions had never actually been wired into any poll loop) was found and fixed in the same pass.','Every run - manual, scheduled, webhook or workflow-triggered - writes to one unified run history, step by step, with correct triggered-by/source linkage. An admin-scoped agent or pipeline can never run unattended unless the actor genuinely resolves to a real Administrator - a deliberate security boundary, not an oversight.']],['v0.42.0','September 2026','AI & Agentic Layer, Phase 6a: AI Agent Foundry — Agents, Memory, Skills, Actions, Delegation',['A new admin section for named, reusable AI Agents: a persona layered over a per-tool Actions checklist (granting any admin-catalog tool makes an agent Administrator-only, computed at runtime), with persistent Memory, an attached Skills library, and delegation to other agents.','Memory is a living markdown document an agent reads every run and can revise itself via an always-available <code>update_memory</code> tool - the same role a <code>CLAUDE.md</code> plays for a coding agent. A Skill shows only its name and description up front; full instructions load only when the agent decides to use it.','Delegation lets a supervisor agent call named sub-agents and get their answer back, guarded against runaway recursion by a depth cap (a tool-error, not a crash, when hit) - hierarchy that emerges from what an admin configures, not a fixed org chart.','Chat with any agent from the main Assistant page, gated by whether its actions require Administrator. 8 new Rust tests, including a straight-line 5-agent delegation chain proving the depth guard actually stops the 5th agent from ever running.']],['v0.41.0','September 2026','AI & Agentic Layer, Phase 5: LLM Chat Assistant — records + admin, full coverage',['A conversational assistant for every user (a "records" mode with the same 7 tools MCP exposes) and, for Administrators, an "admin" mode with ~17 tool pairs covering Business Rules, Workflow Automation, Custom Objects/Fields/Relationships, Integration Hub, Dashboards, Apps, Reports, Saved Views, Users and Numbering - each a thin wrapper over that subsystem\'s own already-validated function.','A real tool-calling completion loop (<code>ai_service::complete_with_tools</code>) for both Anthropic and OpenAI-compatible providers, capped at 8 rounds as a runaway-cost guard, persisted through new <code>chat_conversations</code>/<code>chat_messages</code> tables.','One non-negotiable boundary, built in from the start: a Connection\'s or API client\'s secret is never read from a tool call\'s arguments, and is redacted a second time out of the raw provider turn before it\'s ever persisted to chat history.','A new "Assistant" main-nav item for every user and an "Admin Assistant" card on the Admin landing page; the online demo mirrors both as a labeled, keyword-matched simulation. 8 new Rust tests, including one that caught a real gap in the secret-redaction pass.']],['v0.40.0','September 2026','AI & Agentic Layer, Phase 4: Agent Actions - natural-language reporting',['Ask a plain-English question and the workspace\'s configured LLM translates it into the exact shape the existing Custom Report builder already produces - one object, one group-by field, count or sum - reusing the identical validation a human\'s manual report creation already goes through, not a new reporting engine.','The first feature to actually call an LLM for inference, via a new <code>ai_service::complete</code> primitive; a new "Ask a question" tab in Reports shows the resolved report shape and a live result table, with a one-click Save as report.','The first of four proposed Agent Actions (meeting-prep briefings, commitment capture and record-hygiene suggestions remain proposed, since each needs a live Activity channel to be genuinely useful). The online demo simulates it with real grouping logic against real demo data, clearly labeled as a simulation. 7 new Rust tests.']],['v0.39.0','August 2026','AI & Agentic Layer, Phase 3: Unified Activity Timeline',['A generic activities log - one entry per email, call or message, tied to a Company, Contact or Opportunity - queryable the same way any other record is. Distinct from the audit trail, which records what a user changed, not what happened around a record from the outside.','A real find while building it: the underlying <code>activities</code> table has existed since the very first migration but was never wired up to any model, repository or service - this release completes it instead of creating a colliding, differently-shaped table.','Ships with manual logging first - a complete, honest feature on its own - with an email/call-transcript/Slack auto-capture connector named as its own separately-tracked follow-on, not silently skipped. 9 new Rust tests; a new ActivityTimeline component wired into Companies, Contacts and Opportunities on desktop, and a genuinely functional (not simulated) mirror in the online demo.']],['v0.38.0','August 2026','AI & Agentic Layer, Phase 2: MCP Server & CLI',['A stateless <code>POST /mcp</code> Model Context Protocol server (Team Workspace mode) exposes the same generic, permission-checked object dispatcher the REST API already wraps: 7 tools - list_objects, get_object_metadata, list_records, get_record, create_record, update_record, archive_record.','Authenticated with the same scoped <code>{client_id}.{secret}</code> API-client bearer keys the REST API already uses - no new credential type - and every call is rate-limited and logged through the same unified execution log.','A companion <code>lanesra</code> CLI (clap + reqwest) gives humans and shell scripts one subcommand per REST route, configured via <code>--api-key</code>/<code>--base-url</code> or environment variables.','The online demo mirrors it as a labeled UI-only simulation - a real endpoint/tool list and a Test connection button that simulates the JSON-RPC handshake against real demo data. 17 new tests (10 server, 7 CLI) alongside the full existing suite.']],['v0.37.1','August 2026','LLM & MCP: own admin section, and an online demo mirror',['Relabeled the Admin category from "AI" to "LLM & MCP" and split it into LLM and MCP Server sub-tabs, giving the next phase\'s MCP server an obvious home instead of needing another admin reshuffle later.','Added a labeled, client-side simulation of the same screen to the online demo - same provider/model/key form, a simulated Test key result, no real key storage or provider call, exactly like the demo\'s existing Integrations tab.']],['v0.37.0','August 2026','AI & Agentic Layer, Phase 1: bring-your-own LLM key',['A workspace supplies its own Anthropic or OpenAI-compatible provider key - Lanesra never resells or proxies inference, the same posture every other Lanesra feature already takes. The key is stored exactly like any other secret (AES-256-GCM via the existing secret store), never returned by any read command, with a one-click Test key connectivity check.','Two provider modes: Anthropic (a real API call, minimal token cost) and openai_compatible (an admin-supplied base URL - covers OpenAI itself, or a self-hosted, fully offline OpenAI-compatible server).','New Admin → AI screen with an honest note that no AI feature in this product does anything until this is configured. 10 new Rust tests covering rotation, admin-gating, and both provider modes against a real local HTTP listener.']],['v0.36.0','August 2026','Saved Views & Bulk Actions — desktop and online demo',["Any list screen that already had per-field filtering - Companies, Contacts, Tasks, Opportunities, and every Custom Object\'s records - can now save its current filter/sort/group-by combination as a named view, Private or Shared, with an admin-settable object default. Selecting a view narrows and re-sorts the list live; updating or deleting a view, and setting the workspace default, are all one click from the same bar","A saved view also feeds a dashboard record-list widget directly: the admin builder can narrow any record-list widget to one saved view, so a dashboard tile shows only the subset that view\'s filters define instead of every record of that type","Bulk operations pair with the multi-select a saved view narrows to: update a status/stage (validated through the same Status Transition rules a single edit already goes through), reassign owner, add/remove tags, and archive - each offered only where the entity actually carries that field (Contacts have no owner field on either platform, so bulk reassign isn\'t offered there)","Desktop (Rust core): migration 0034 (<code>saved_views</code>), <code>saved_view_service</code> (CRUD, one-default-per-object enforcement, owner-or-admin edit/delete) and <code>bulk_action_service</code> (per-entity allowlists mirroring what each entity\'s model actually supports), plus 11 new Rust tests, all green alongside the full existing suite","Online demo: the same view/sort/group-by and bulk update-status/reassign-owner/export/delete operations against this demo\'s simpler status-and-owner field set, wired into the shared <code>tablePage</code> renderer every list screen already uses - without a Private/Shared distinction or a per-custom-field filter, since the demo has neither a signed-in-user concept nor desktop\'s per-field list filtering to begin with"]],['v0.35.0','August 2026','Integration Hub — Connections, Connectors, API Access, Webhooks, Data Exchange, Integration Jobs, Logs & Monitoring, desktop only',['A real Integration Hub ships on the desktop app (Admin → Integration Hub): encrypted-at-rest Connections (AES-256-GCM) for generic REST, SFTP, PostgreSQL, OData and SMTP, with a genuine Test Connection call against a real endpoint - not a client-side simulation like the online demo\'s existing Integrations tab, which stays exactly that and is relabeled to match this feature\'s terminology rather than implying parity it doesn\'t have','Connectors import an OpenAPI 3.x spec and derive reusable Actions, each callable as a new Workflow Automation \'Call Connector Action\' step. API Access issues hashed, scoped <code>{client_id}.{secret}</code> bearer keys against a real generic <code>/api/v1/objects/...</code> REST API (Team Workspace server only - a pure desktop install has no listening socket for an external caller to reach). Webhooks sign every delivery with HMAC-SHA256, retry with exponential backoff, and keep a full delivery history, with a one-click Test Delivery','Data Exchange generalizes the previous Companies/Contacts-only CSV import into an object-agnostic wizard (any built-in or Custom Object, auto-map-from-header, transforms, duplicate/upsert policy, dry-run preview with per-row results) plus CSV export and reusable Mappings - all routed through the same generic record-write path the REST API itself uses, so validation, business rules and permissions apply identically, not a second parallel code path','External Objects surface read-only, live records from a Connection; Integration Jobs add a recurring pull-sync on an interval with a checkpoint cursor, run by a real background scheduler thread on the Team Workspace server (desktop has no long-running process to host one, so a manual Run Now is the only way a desktop-hosted Job ever runs today - stated directly in the Jobs tab\'s own copy). A unified execution log spans API calls, webhook deliveries and import/export runs, with a real KPI Overview and a Server-Sent-Events endpoint for live streaming','Best-effort, explicitly-labeled coverage beyond that core scope: SFTP and PostgreSQL connectors proven against a real in-process SFTP server and a real local Postgres instance (the Postgres test is <code>#[ignore]</code>d by default since CI doesn\'t run <code>cargo test</code> at all here, so contributors without Postgres installed are unaffected), OAuth2 client-credentials and authorization-code token exchange against a local mock token endpoint, a minimal SMTP send action against a raw-socket SMTP double, and an OData v4 query/response layer against a mock server. mTLS is the one deliberate, named gap - the spec itself marks it Future']],['v0.34.0','August 2026','Named, scoped Solutions — the Dynamics-365-style build-in-test / export / import-in-prod workflow, desktop and online demo',['Deployment Management gains a real <b>Solutions</b> tab: a named, versioned, admin-picked <em>subset</em> of components - not everything Local Workspace owns. Create a solution, curate exactly the objects/fields/relationships/rules/workflows/screens/reports it needs from anything in the workspace (hand-built or installed), and Export it on its own into the same manifest shape <code>export_local_workspace</code> already produces','No new "environment" concept was needed: a Lanesra OS workspace - one desktop install, or one Team Workspace deployment - already is a standalone environment, the same way a D365 org is. "Build in test, export it, import it in prod" is two separate workspaces exchanging the exported file through the existing Admin → App Catalog → Import → Validate → Install pipeline, unmodified','Desktop (Rust core): new <code>solutions</code>/<code>solution_members</code> tables (migration 0031), <code>solution_service</code> for admin-gated create/rename/delete and add/remove-component, and <code>industry_package_service::export_solution</code>, which shares its manifest-building pass with <code>export_local_workspace</code> via a new <code>build_export_manifest</code> helper. A solution\'s <code>package_id</code> is fixed as <code>local.solution.&lt;id&gt;</code>, stable across repeated exports, so bumping its version and exporting again produces a second, listable Release of the same package - upgradeable in the target workspace via the existing update-with-diff pair, exactly like any other package','Online demo: a parallel <code>data.solutions</code> registry with the identical create/rename/delete/add-member/remove-member/export shape, sharing the same manifest-building helper its own <code>exportLocalWorkspace</code> was refactored to use - a Solution here is a real, complete export for backup/inspection/sharing, though (like <code>exportLocalWorkspace</code>) not re-importable within the demo itself, since the demo has no pathway to install a hand-authored manifest back in','10 new Rust core tests (create/rename/delete, membership add/remove, scoped export excluding uncurated components, round-trip export into a separate "prod" workspace, two sequential version-bumped exports listing as two Releases, admin-gating) alongside the full existing suite, all green']],['v0.33.0','August 2026','Component-tagging, Local Workspace & export — the rest of Deployment Management, desktop and online demo',['Component-tagging: every custom object, field, relationship, business rule and workflow now has a real owner - the desktop edition wires this into all 10 creation call sites plus the package installer (migration 0030, <code>solution_component_service</code>), and the online demo mirrors it independently across its own 7 hand-built creation sites plus <code>installReferencePackage</code>. The Deployment Management <b>Components</b> tab now shows everything in the workspace, hand-built or installed, not only what an install created','The Packaged/Custom distinction\'s Custom half, made real without a fake package row: a synthetic <b>Local Workspace</b> entry in Solution Packages aggregates every component still owned by the <code>local</code> publisher, on both platforms','A genuine <b>Export</b> action turns everything in Local Workspace into a downloadable, re-importable manifest. On desktop this reuses the exact <code>IndustryPackageManifest</code> shape <code>import_package</code>/<code>install</code> already understand - exporting one workspace\'s hand-built customizations and importing them into another was verified end to end. The online demo\'s export produces the same manifest shape for backup/inspection/sharing, documented as not re-importable within the demo itself, since the demo has no pathway to install a hand-authored manifest the way its fixed reference-package catalog installs','<b>Releases</b> (desktop only): every imported version of a package, oldest first - no new table, since each <code>app_packages</code> row was already an immutable per-version snapshot','<b>Update-with-diff</b> (desktop only), replacing "reinstalling an already-installed package_id is rejected outright": <code>plan_package_update</code> previews Added/Modified/Removed for objects and fields (the two component types with stable keys across versions) plus added-counts for relationships/rules/workflows/screen layouts/reports; <code>apply_package_update</code> applies it transactionally with a pre-update safety backup, updating matched objects/fields in place and never deleting anything a newer manifest drops. The online demo\'s reference-package catalog has no multi-version concept to diff against, so this stays desktop-only, called out explicitly in its own Solution Packages tab rather than faked','11 new Rust core tests (component-tagging, Local Workspace summary, export round-trip into a fresh workspace, update-with-diff plan/apply) alongside the full existing suite, all green']],['v0.32.0','August 2026','Deployment Management: Admin IA reshuffle, Publisher registry & namespace enforcement — desktop and online demo',['Admin no longer opens into one flat category set: the desktop edition\'s landing page is regrouped into 8 categories (Workspace, Access, Data Model, Experience, Automation, Apps, Analytics, Deployment Management) and the online demo into 9 (the same plus Integrations, demo-only) - pure regrouping of existing tools plus one new area','New <b>Deployment Management</b> area (both platforms) answers "what have I customized beyond what I installed": a <b>Solution Packages</b> tab reframes every installed reference package with its publisher, component and dependency counts; a <b>Components</b> tab lists every custom object/field/relationship/rule/workflow an install created, grouped and searchable by type; a <b>Dependencies</b> tab shows each package\'s declared dependencies with a satisfied/unsatisfied badge','Desktop: wired the existing but previously-dead <code>app_dependencies</code> table into <code>import_package</code>, so a manifest\'s declared dependencies are now actually recorded at import time, and added a workspace-wide artifact listing so Components isn\'t limited to one installed app at a time. Online demo: added the equivalent artifact tracking to <code>installReferencePackage</code>, which didn\'t exist in any form before this release','A real <b>Publisher registry</b>, not a placeholder: every workspace auto-seeds two publishers - <code>lanesra</code> (official, owns all ten bundled reference packages) and <code>local</code> (the implicit home for hand-built customizations) - and an admin can register more from a new Publishers tab, with the same key rules enforced independently on both platforms: lowercase, starts with a letter, 2-32 characters, digits/underscores only, <code>lanesra</code> and <code>local</code> reserved','Every package import now resolves its publisher from the text before its <code>package_id</code>\'s first dot and rejects the import if that publisher isn\'t registered, naming exactly which key needs registering - real, enforced namespace validation on both the desktop Rust core (migration 0029) and the online demo, not merely documented behavior','Packaged/Custom, the 5-layer extension model, update-with-diff and Releases/export remain proposed - this release is the registry and visibility layer under them, not the full spec']],['v0.31.0','August 2026','Online demo: full 10-package App Catalog parity',['The browser demo\'s Admin → App Catalog now mirrors all ten industry reference packages, not just the original two (Field Service, Property Management) it launched with in v0.28.0: Construction & Contractors, Professional Services, Dental/Clinic Practice Administration, Recruitment & Staffing, Real Estate Brokerage, Legal Practice, Nonprofit & Association Management and Auto Repair & Service Garage all install client-side into the demo\'s existing engine - custom objects, fields, relationships, business rules and workflows, tagged to the installing app the same way per-app scoped automation already tags them on desktop','Documented (not silently worked around) three structural limits of the demo\'s simpler engine hit while mirroring these: workflow rules only fire on an edit where a watched field changed, not on record creation; a workflow\'s "create a new record" action only supports the 9 built-in types, not a custom-object target; and the demo\'s condition engine has no date-to-date comparison operator, only numeric-coercing greater/less-than','<b>App Builder</b> also gained real feedback: toggling an object on or off now shows a confirmation toast ("Added Contact to Field Service") instead of saving with no visible confirmation - the actual defect behind a reported "can\'t add an existing entity to an installed app" bug (the save always worked; there was simply nothing telling the user it had)']],['v0.30.0','August 2026','Auto Repair & Service Garage: tenth and final industry reference package',['Completes the Industry Data Model\'s initial reference-package set: Vehicle, Repair Order, Repair Line, Repair Inspection, Service Recommendation and Vehicle Appointment objects, 30 fields, 10 relationships, 3 business rules (repair-completion, authorization, odometer validation) and 4 workflows (appointment check-in, repair authorized, repair completed, no-show)','Surfaced and documented two real platform limits while building it: a workflow\'s create_record action can only target Company or an active custom object - never a built-in document type like Invoice, since those need a required relational field a no-code action can\'t safely synthesize - and create_record\'s name template is static, not built from the triggering record\'s own fields. Both are now called out in the reference-package module\'s own docs rather than worked around silently','10 new Rust tests covering manifest install counts and every business rule and workflow end to end']],['v0.29.1','August 2026','Real Estate Brokerage, Legal Practice, and Nonprofit & Association Management reference packages',['Three more industry reference packages on the Industry Data Model foundation, each with its own object model, business rules and workflows: <b>Real Estate Brokerage</b> (Listings, Transactions, Showings), <b>Legal Practice</b> (Matters, Time Entries, Court Dates) and <b>Nonprofit & Association Management</b> (Members, Donations, Programs)','Brings the App Catalog to nine reference packages, one short of the full initial set']],['v0.29.0','August 2026','Per-app scoped automation',['Business rules, workflow automation and dashboards created as part of an installed app are now tagged with the App that owns them - both admin builders gained an App filter, so an installed reference package\'s automation stays visibly contained as more apps get installed instead of mixing into one flat, unscoped list','Shipped on the desktop edition (Rust core migration + models + repos + services, Tauri commands, React UI) and mirrored in the online demo']],['v0.28.2','August 2026','App Builder: object-change confirmation, and App Catalog split into its own Admin category',['Admin → Apps split into two categories - <b>App Builder</b> (assemble your own app from existing objects) and <b>App Catalog</b> (install a ready-made industry reference package) - instead of one crowded tab covering both very different workflows','Toggling an object on or off inside an App Builder app already saved and refreshed navigation correctly; it just gave no visible confirmation it had, which read as a bug when the newly-added object\'s nav icon was easy to miss on the icon-only mobile sidebar. Fixed with a toast']],['v0.28.1','August 2026','App Catalog: Construction & Contractors, Professional Services, Dental/Clinic Practice Administration, and Recruitment & Staffing reference packages',['Four more reference packages available from Admin → App Catalog, each a complete object/field/relationship/business-rule/workflow set reusing the workspace\'s existing Company/Contact/Task core rather than a parallel data model: <b>Construction & Contractors</b> (Projects, Change Orders, Site Visits), <b>Professional Services</b> (Engagements, Timesheets, Deliverables), <b>Dental/Clinic Practice Administration</b> (Patients, Appointments, Treatment Plans) and <b>Recruitment & Staffing</b> (Job Openings, Candidates, Placements)','Brings the App Catalog to six reference packages']],['v0.28.0','August 2026','Industry Data Model: package manifest, install pipeline & App Catalog, with the first two reference packages',['The largest item that was still "proposed" as of v0.27.0, now shipped: a versioned metadata package format - objects, fields, relationships, business rules, workflows, screens, reports and a dashboard - installed into an existing workspace from a new Admin → App Catalog. Install runs through pre-install validation (naming/numbering collisions, missing dependencies), an automatic safety backup, and a transactional install with rollback on failure; deactivating a package removes it from navigation without touching the business records it created','Ships with the first two reference packages: <b>Field Service</b> (Work Orders, Assets, Service Appointments) and <b>Property Management</b> (Properties, Units, Leases) - both fully reusing the workspace\'s existing Company/Contact/Task core rather than creating parallel "Field Service Customer" style duplicates','Shipped on the desktop edition first (Rust core: repo, install-orchestrator service, Tauri commands, server dispatch, React admin UI) and mirrored in the online demo\'s own App Catalog']],['v0.27.0','August 2026','App Builder: package objects, screens and a dashboard into one named app with role-based access — now enforced server-side, not just in the UI',['<b>App Builder</b> ships as a real feature, not just a name: an Administrator can group any set of objects - Custom Objects or built-ins - their published Screen/App Builder layouts, and one dashboard into a named, publishable App with its own icon, then grant it to specific roles or individual users as Viewer or Editor. Every published App also filters navigation and adds an entry to a sidebar App Switcher - switching into an app only shows the objects it claims, not the whole workspace','Access grants are now enforced everywhere a record is written, not only in the UI that shows or hides a button: once an object type is placed in at least one published App, creating, editing or archiving a record of that type requires at least Editor access to some App containing it. An Administrator always bypasses; a grant made to a specific user beats one made to their role; the strongest grant across every matching App wins. Objects never placed in an App are completely unaffected - existing workspaces that haven\'t adopted App Builder see no change in behavior','Status-lifecycle actions are covered by the same check: issuing or voiding an invoice, recording a payment, setting a quote or order\'s status, and converting a quote to an order or an order to an invoice all require write access to the entity type the action actually changes - a conversion command is gated on the document it creates, not the untouched source document it reads','The desktop app reflects this before a click ever reaches the server: a Viewer-only user sees every "New", "Edit", "Issue", "Void", "Record payment" and status/conversion button on a scoped object disabled with a tooltip explaining why, instead of submitting and hitting a permission error','Shipped on the desktop edition first (Rust core, Tauri commands, React UI) and mirrored in the online demo (Admin → Apps, the sidebar App Switcher, and an app-scoped dashboard)']],['v0.26.1','August 2026','Dashboard customization Phase 2/3: chart and record-list widgets, on desktop and in the online demo',['Extends Phase 1\'s KPI-tile-only widget catalog: a <b>chart widget</b> reuses the existing Custom Reports engine directly on a dashboard - pick a saved report (any object, grouped by its status/stage or a reportable custom field, counted or summed) and it renders live, the same report definition Admin → Reports already builds and runs elsewhere','A <b>record-list widget</b> shows a short list of an object\'s most recent records - or, for Tasks and Invoices, its soonest-due ones - each row jumping straight to that record on click, the same one-shot navigation every ID hyperlink elsewhere in the app already uses','Both widget types ship on the desktop app\'s Admin → Dashboards builder and are mirrored in the online demo\'s own dashboard builder, reusing the identical draft/publish/role-resolution model Phase 1 established - a dashboard using neither widget type behaves exactly as it did before this release']],['v0.26.0','August 2026','Screen/App Builder (full, 4 phases) and Dashboard customization Phase 1 - the two largest items on the roadmap, both shipped',['<b>Screen/App Builder</b>, the single largest item that was still "proposed" - a drag-and-drop layout builder for any object\'s create/edit form, on both the desktop app and the online demo, shipped in four phases: (1) named layouts with tabs of field sections, assigned to roles with a required Default fallback, and Draft → Publish; (2) multi-column sections (1-3 columns) with a per-field full-width span; (3) placing a custom relationship\'s related-records list on a specific tab, with anything unclaimed still showing in an always-visible spot; (4) the same published layout now also drives the record\'s read-only detail/Overview view, not just the edit form - a custom field an admin added is now visible there too, something no detail page could show before this','Desktop gained a real Screen layouts builder for the first time (it previously had none at all - the online demo\'s was the only one); the demo\'s builder gained the tabs/columns/related-lists/detail-view capabilities its Phase-1-only version didn\'t have','<b>Dashboard customization</b> Phase 1 (desktop): Admin → Dashboards lets an Administrator build multiple named dashboard layouts - each an ordered list of KPI-tile widgets - and assign them by role, with a required Default fallback, reusing the exact same draft/publish/role-resolution model as Screen/App Builder. No dashboard published yet falls back to exactly the dashboard that existed before this feature, unchanged. Chart and record-list widgets, and an online demo mirror, are next','See <a href="/platform">what you can build</a> with these primitives - they\'re the same building blocks a whole custom business app is made from, not just form customization']],['v0.25.0','August 2026','Admin landing page redesign, plus duplicate/version history/dependency warnings for Business Rules and Workflow Automation',['Admin no longer opens straight into a flat 12-tab row - clicking Admin now opens a categorized landing page (Workspace, Access, Customization, Automation, Integrations), and clicking an item inside a category opens straight into that builder with a breadcrumb (Dashboard > Admin > tool) back to the landing page. The sidebar Admin icon always resets to the landing page, the same as clicking Setup in Salesforce always reopens Setup Home','Business Rules and Workflow Automation both gained a Duplicate action - clones a rule/workflow as an inactive copy and opens it for review before it can affect anything live','Both builders also gained a bounded version history (last 10 saves): every edit snapshots the rule\'s prior state, viewable from a History button with a one-click Restore per version - restoring itself snapshots the state it\'s replacing, so a restore is never a dead end','Deactivating a custom field that an active business rule or workflow rule still reads or writes now shows a confirmation listing exactly which active rules reference it, instead of silently breaking them the next time they evaluate']],['v0.24.1','August 2026','Mobile layout fix for record detail pages, and a stale favicon fix',['The new Products/Quotes/Orders/Invoices/Contracts/Tasks record detail pages (shipped in v0.24.0) overflowed horizontally on a phone - reported directly from mobile testing against the live demo. The line-items table wasn\'t wrapped in the same `.table-wrap` (overflow:auto) pattern every other data table in the app already uses, and the surrounding grid/header layout sized to that table\'s intrinsic minimum width instead of the actual viewport - a classic CSS grid/flex "min-width:0" gap. Fixed both; Company/Contact 360 (unaffected by the bug) and every other detail page now match viewport width exactly on mobile, with no change to desktop-width layout','Fixed the browser tab favicon, which still drew a "B" glyph left over from when the product was called BusinessOS (renamed to Lanesra OS in v0.5.0) - it now draws an "L" matching the in-app sidebar mark']],['v0.24.0','August 2026','Multi-condition OR-groups and a full action palette for Business Rules & Workflow Automation, on desktop and in the online demo',['Both engines\' conditions gained one level of nested OR-grouping on top of the existing AND/OR: a rule can now express "A AND (B OR C)", not just a flat AND or a flat OR - the builders show a dashed "OR group" box with a "+ OR group" control alongside "+ Add condition"','Business rule actions expanded from require/hide/lock/set-default/set-value/block-save/show-message to the full palette: Show and Make editable (explicit counterparts to Hide/Lock, most useful together with a new "Hide by default" flag on a custom field), Clear field value, Restrict choices (narrows a select field\'s options while the rule matches), and a severity split of the old generic message into Show error/Show warning - "Effects" is renamed "Action" throughout both builders','A custom field can now be flagged "Hide by default" - left off every create/edit form unless a business rule\'s Show action currently targets it, enforced server-side on desktop (a hidden-by-default+required field can never block a save) and mirrored client-side in the demo','Workflow automation gained two new field-behavior actions - Set default value (only fills a field if currently empty) and Clear a field - and, in the online demo specifically, an optional "extra conditions" section (AND/OR, same OR-groups) evaluated once the trigger itself already fired, plus true multi-action workflows (previously one action per rule)','"Trigger approval" from the design mockup is deliberately not included in this round','Desktop: Rust core (migration 0020), 8 new tests, full workspace suite green; React admin screens for both builders rebuilt to match. Online demo: business rules and workflow automation rebuilt from single-condition/single-action to fully array-based, with an in-place upgrade of any rule saved before this release - existing rules keep evaluating exactly as before until edited through the new builder']],['v0.23.1','August 2026','Online demo parity fix: workflow self-updates and custom-object workflows',['Workflow automation gained the desktop edition\'s update_field action (the companion to update_related_record) - "when this record\'s field changes, also set another field on this same record" - e.g. "when Status becomes Customer, set Industry to Active." The new value can be a fixed value or copied live from another field on the same record','Fixed a bug where a workflow rule defined on an admin-defined Custom Object could be created in Admin → Workflow Automation but would silently never fire - execution was gated on a built-ins-only lookup table left over from before Custom Objects existed as a workflow-eligible entity; workflows on custom objects now run exactly like on any built-in entity','Both fixes verified against the exact reported scenario (Company status change auto-updating a second field on the same Company) and against a custom object end to end']],['v0.23.0','August 2026','Integrations admin section in the online demo',['A new Admin → Integrations section - another capability that doesn\'t exist on the desktop edition, built for the demo first, as a UI-only simulation: the static demo has no server, so nothing here makes a real network call or runs on a real schedule','Scheduled jobs: define a data Export/Import/Sync job against any built-in or custom object with a schedule (manual, hourly, daily, weekly) and format (CSV/JSON); Run now simulates it immediately, with a per-job history log of simulated runs and record counts','API endpoints: define and "expose" a GET or POST endpoint backed by any object, with API-key or public auth; Test call shows the exact request and a realistic JSON response built from your own current demo data, entirely local','External API connections: configure an outbound connection (base URL, method, none/API-key/bearer auth) this workspace would call; Test request logs a simulated response with a call history, clearly labeled as simulated since the demo can\'t make real outbound requests','With this, the online demo has closed every gap identified in this round of parity work: Custom Objects, Custom Relationships, Reports, a no-code Screen layouts designer, and now Integrations - all four built for the demo, two catching up to desktop and two (layouts, integrations) new to the product entirely']],['v0.22.0','August 2026','No-code Screen layouts in the online demo',['A new Admin → Screen layouts tab lets an admin drag-order any built-in or custom object\'s create/edit fields into named sections - a capability that doesn\'t exist on the desktop edition either, built for the demo first','A layout has a draft and a published copy: dragging fields between/within sections, renaming a section, or adding one only ever edits the draft - the live create/edit form keeps using the plain default field order until Publish copies the draft over, and Unpublish clears it straight back to that default','A Preview button renders the draft exactly as the live form would show it, without saving anything or touching the live workspace','A published layout never drops a field it doesn\'t recognize - any field missing from the layout (a new custom field added after publishing, or a stale key) is automatically appended to a trailing "Other fields" section so nothing can go missing from a form because of a layout edit','With this, the online demo\'s only remaining gap against the desktop edition\'s admin extensibility spec is the Integrations admin section - up next, also a demo-first UI-only build']],['v0.21.0','August 2026','Reports in the online demo',['The online demo has a new top-level Reports section with the same fixed report gallery the desktop edition ships: Revenue by month, Win rate by owner, Lost reasons, AR aging and Sales by owner, each with a date-range (or "as of") filter and a bar-chart table matching desktop\'s layout','Added a Custom Reports builder to the demo, mirroring desktop\'s admin report builder: pick any object - built-in or a Custom Object - group by its status/stage or an active, reportable custom field, and count records or sum a numeric custom field; only fields an admin flagged Reportable are offered, same as desktop','Added CSV export to every report in the demo (a new capability for the demo generally, self-contained via a Blob download - no server involved)','Two disclosed substitutions where this demo\'s simpler data model doesn\'t match desktop\'s: Revenue by month/Sales by owner group by each invoice\'s due date since the demo has no separate issue date, and AR aging uses each invoice\'s full total as its balance since the demo doesn\'t track partial payments - both called out in the report\'s own subtitle rather than silently faked','Added a Lost reason field to Opportunities so the Lost Reasons report has something real to report on','With this, the online demo\'s only remaining gaps against the desktop edition are two capabilities that don\'t exist on desktop either: a no-code UI layout designer and a UI-only Integrations admin section - both underway next']],['v0.20.0','August 2026','Custom Relationships in the online demo',['An Administrator can now connect any two object types - built-in or custom - from Admin → Relationships: a cardinality (many-to-one, one-to-one or many-to-many), a forward/reverse label pair, and a choice of what happens to a link when a linked record is deleted (Restrict blocks the delete, Archive drops the link and keeps both records)','Every record\'s edit form now shows a "Related records" panel listing every linked record across every applicable relationship, from either direction, with inline Link/Unlink - the same place desktop puts its related-records card, since most objects in this demo have an edit form but no separate detail page','Cardinality is enforced on link (a many-to-one or one-to-one side can\'t be linked twice), and a relationship can\'t connect an object type to itself - both match the desktop edition\'s validation exactly','Reports beyond the dashboard remain the one desktop-only capability left in the online demo\'s parity work']],['v0.19.0','August 2026','Custom Objects in the online demo',['The online demo caught up with one of the desktop edition\'s biggest capabilities: an Administrator can now define a whole new business object at runtime - Vendors, Assets, Projects - from Admin → Custom Objects, with its own icon, sidebar entry and record-number prefix/digit width, no code change','A custom object is a full citizen of the demo\'s admin subsystems exactly like a built-in entity: it gets its own tab in Custom Fields, Business Rules, Status Transitions and Workflow Automation, and its records go through the same create/edit/list screens, auto-numbering and delete-dependency checks as Companies or Contacts','A custom object can\'t be named the same as a built-in entity (rejected at creation, matching desktop); deleting its definition is blocked while any record still exists, while deactivating is always safe and reversible since it only hides the object from navigation and new-record creation','Reports beyond the dashboard and Custom Relationships between record types remain desktop-only for now - next up in the online demo\'s parity work']],['v0.18.1','August 2026','Online demo parity fixes',['Workflow "Create a new record" now offers all 9 built-in record types in the online demo (companies, contacts, opportunities, products, quotes, orders, invoices, contracts, tasks), not just 3 - matching the desktop edition\'s full creatable set, with company-dependent types offered only when the trigger record actually carries a company','Workflow "Update a related record" now walks the demo\'s foreign-key graph in both directions, not just downward: a Contact can update its own parent Company the same way a Company can update its Contacts, and every entity gets its linked Tasks (and vice versa) through the existing relatedType/relatedId link','Custom fields in the online demo gained the validation and capability settings the desktop edition already had: Required, Max length and Pattern/regex (text fields), Min/Max value (number fields), and Searchable/Filterable/Reportable flags - enforced with native HTML5 form validation and shown in the custom fields list']],['v0.18.0','August 2026','Status transitions, richer workflow actions, test mode, a rule-builder redesign, and Customer 360',['Added a Status Transition Editor: restrict which status/stage changes are allowed on any object with a fixed-schema field (companies, contacts, opportunities, products, quotes, orders, invoices, contracts, tasks) - each rule is one from → to move, with a wildcard "any status" starting point and its own active toggle; with no active rules a field stays fully unrestricted, and resaving the same status is never blocked','Workflow automation actions expanded beyond "create a task": a workflow can now create a new record (a company, opportunity or task) or update a field on a record related to the trigger through the demo\'s existing company/contact/opportunity/quote/order relationships','Added a Test rule / Test workflow dry-run mode to both Business Rules and Workflow Automation: fill in hypothetical values for an object and see exactly which active rules or workflows would match and what they would do, without creating, changing or sending anything','Redesigned the Business Rules and Workflow Automation builders to match the desktop edition\'s rule-builder layout: numbered Condition/Effect (or Trigger/Action) sections, a live-updating rule summary panel, and - for workflows - a visual Trigger → Action → End canvas that mirrors the form as you edit it; both builders gained full editing (not just create) and header-level Test/Activate-Deactivate/Save controls','Custom fields gained four more settings: an optional default value applied whenever a save leaves the field empty, a "require a unique value" check (rejected at definition time for yes/no fields, since they only have two possible values), placeholder text, and help text shown under the field on every record form','Added Customer 360 and Contact 360: clicking a company or contact name anywhere in the app now opens a dedicated detail page with its full field overview and every linked record - contacts, opportunities, quotes, orders, invoices, contracts and tasks - each one click away, replacing edit-modal-only access','Fixed a pre-existing bug surfaced while building the above: the admin panel\'s tab row no longer freezes on whichever tab was open first - switching tabs now correctly highlights the one you\'re on']],['v0.17.0','August 2026','More operators & field-to-field comparison',['Business rules and workflows gained four more comparison operators - starts with, ends with, is one of, is not one of - on top of is/is not/contains/is empty/is not empty/greater than/less than','A condition can now compare a field against another field\'s live value instead of only a fixed value - e.g. "require Flag when Notes equals Expected Notes" - with the same live-updating preview on the record form that a fixed-value condition already had','Windows desktop edition: the shared condition engine gained the same operators and field-to-field comparison, for both business rule conditions and workflow triggers']],['v0.16.0','August 2026','Business rules & workflows now work on any field, not just status',['Business rules can now condition on any built-in field - name, industry, value, close date, whatever the object has - not only the status/stage field, with a real comparison operator (is/is not, contains, is empty, greater than, less than) chosen per field, and their require/hide action can now target a built-in field too, not just a custom one','Workflow automation\'s field-changed trigger can now watch any built-in field the same way, so "when Industry changes to X" or "when Due date is set" can create a task and notify admins, not only "when status/stage reaches a value"','Windows desktop edition: the underlying business rules and workflow engines gained the same any-built-in-field support for both conditions/triggers and actions (require, hide, lock, set default, force value, and the workflow update-field action), writing through each entity\'s own validation so nothing bypasses existing rules']],['v0.15.0','August 2026','Custom relationships, richer business rules & workflow automation',['Added admin-defined custom relationships between any two record types (companies, contacts, custom objects, and more), with one-to-one/many-to-one/many-to-many cardinality and a choice of what happens to linked records on delete','Added a related-records view on record detail pages showing every linked record through those relationships','Replaced the business rules engine: rules can now combine multiple conditions with AND/OR, use 10 comparison operators (not just equals), and lock a field, set a default or exact value, block saving entirely, or show a message — not just require or hide','Replaced the workflow automation engine: triggers now include field changes and dates reached/overdue in addition to status changes, and actions include assigning the record\'s owner, creating a related record, and posting an in-app notification, on top of creating a task','Added an in-app notification center (bell icon with unread count) for workflow-triggered notifications','Added optional validation for custom fields — a min/max range for number fields, a max length and regex pattern for text fields — plus searchable/filterable/reportable capability flags','Added Windows task reminder notifications (native toast notifications via the desktop app\'s webview)','Added a session inactivity auto-lock (15 minutes idle) requiring the current user\'s password to resume','Updated the online demo: business rules now support an "is / is not" operator, and workflow rules can optionally post an admin notification, shown in a new notification bell']],['v0.14.0','August 2026','Admin-defined Custom Objects',['Added Custom Objects: an Administrator can define an entirely new record type (its own label, fields and ID/numbering format) without any code changes','Custom Objects automatically get their own navigation section, and are full citizens of the existing custom fields, business rules and custom report builder — no per-object code was needed for any of the three','A custom object can\'t be named the same as a built-in entity, and deleting its definition is blocked while records exist (deactivating it is always safe and non-destructive)']],['v0.13.0','August 2026','Admin panel: users, roles & flexible configuration everywhere',['Added an Admin panel with user & role management, moved out of the main navigation into one dedicated section','Added an editable business profile (name, phone, address, city, logo) shown across the workspace','Generalized custom fields from Companies/Contacts to every major object: Opportunities, Quotes, Orders, Invoices, Contracts, Products and Tasks','Generalized conditional business rules and workflow automation the same way, so any object with custom fields can use them','Added admin-configurable numbering: choose the prefix and digit width used for each object\'s auto-generated ID (e.g. "ACC-000001" or "ACC-ab0001")','Added a simple custom report builder: pick an object, group by any field including custom fields, and count or sum','Added a dashboard KPI picker so admins choose which tiles show, in what selection, for the whole workspace','Updated the online demo with a full working Admin panel — mirrors every feature above in the browser']],['v0.12.0','August 2026','Branding, reports, custom fields, business rules & workflow automation',['Added business branding (logo, editable business profile) shown on the print letterhead for quotes, orders and invoices','Added reports beyond the dashboard: revenue by month, win rate by owner, lost reasons, AR aging and sales by owner','Added admin-defined custom fields on Companies and Contacts (text, number, date, yes/no, select), enforced both client- and server-side','Added conditional business rules that require or hide a custom field based on a record\'s status','Added Phase 1 workflow automation: auto-create a follow-up task when an Opportunity\'s stage or an Invoice\'s status changes']],['v0.11.0','August 2026','PDF printing & CSV import/export',['Added a browser-native "Print / Save as PDF" preview for quotes, orders and invoices, with business letterhead, line items and totals','Added CSV export on every list screen','Added CSV import for Companies and Contacts, validated row by row through the same rules as the manual forms']],['v0.10.0','August 2026','Team Workspace, backup & restore',['Added Team Workspace mode — a small team shares one server over the local network from browser tabs, with per-user sessions','Added whole-workspace backup and restore as a single file, safe to run against a live database','Added self-service password change from a "My account" screen']],['v0.9.0','August 2026','Desktop edition foundation published',['Published the Windows desktop edition source: Tauri v2 + Rust + SQLite','Implemented the full sales lifecycle on desktop — Companies, Contacts, Products, Opportunities, Quotes, Orders and Invoices','Added quote-to-order and order-to-invoice conversion, atomic document numbering and local user authentication','No packaged installer yet — desktop is available to build and run from source']],['v0.8.0','August 2026','Interactive navigation & public pages',['Made dashboard KPIs clickable with filtered drill-downs','Added a global Quick Create menu','Added mobile navigation while keeping Try Online prominent','Replaced Journey with Principles and added Compare and Download pages','Marked desktop downloads as Coming Soon','Fixed desktop sidebar navigation']],['v0.7.0','August 2026','Trust & product transparency',['Added Roadmap, Changelog and creator attribution','Added Person JSON-LD and updated discovery files']],['v0.6.0','August 2026','Record numbering & search',['Added automatically generated identifiers','Rebuilt global search as one stable result panel','Added keyboard shortcuts and wider search coverage']],['v0.5.0','July 2026','Lanesra OS rebrand',['Renamed BusinessOS to Lanesra OS','Updated product branding, metadata and documentation']],['v0.4.0','July 2026','Relationship integrity',['Added opportunity-to-contact relationship','Removed opportunity-to-contract relationship','Added company-filtered relationship dropdowns']],['v0.3.0','July 2026','Flexible sales flow',['Made opportunities optional for quotes','Made quotes optional for orders','Added products, services and line-item quantities']],['v0.2.0','June 2026','Connected sales MVP',['Added quotes, orders, invoices, contracts and dashboards','Connected core entities using clean relationships']],['v0.1.0','May 2026','First working prototype',['Launched the first browser-based MVP with sample data']]];$('#app').innerHTML=`<div class="site-dark">${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Release history</div><h1>Releases</h1><p>Every meaningful improvement to Lanesra OS, with detailed per-version release notes — documented publicly.</p><div class="status-row"><span class="status-chip">Latest: v0.59.0</span><span class="muted">Early Access</span></div><div class="backlog-callout" id="desktop-installer"><h3>Desktop installer</h3><p><b>desktop-v0.13.0 is the latest tagged Windows installer</b> on <a href="https://github.com/vikram2409-eng/Lanesra-OS/releases" target="_blank" rel="noopener">GitHub Releases</a> — cut with everything through v0.45.0 below (the Context Layer), including the whole no-code platform, App Catalog, Deployment Management, Integration Hub, and the AI & Agentic Layer through Phase 7b. Everything from v0.46.0 (agent guardrails + declarative export/import) through v0.59.0 (Connector Template Library) below is merged to <code>main</code> but not yet in a tagged installer. The two numbering schemes are deliberately not 1:1: version numbers on this page track every meaningful improvement across the desktop app, the online demo and this website; <code>desktop-v*</code> tags on GitHub are cut only when there's a new installer build worth shipping, so they move less often. Full desktop feature list → <a href="/download">/download</a>, full desktop release-status detail → <a href="/roadmap#desktop">/roadmap#desktop</a>.</p></div><div class="rm-shipped-toolbar"><input type="search" id="relSearch" class="rm-search-input" placeholder="Search ${releases.length} releases… (e.g. &quot;MongoDB&quot;, &quot;roadmap&quot;)" aria-label="Search releases"><span id="relSearchCount" class="rm-search-count" aria-live="polite"></span></div></div></section><section class="section"><div class="container changelog-list" id="relList">${releases.map((r,i)=>`<article class="release" id="${r[0].replaceAll('.','-')}"><div class="release-meta"><span class="status-chip">${r[0]}</span><span>${r[1]}</span></div><div><h2>${r[2]}</h2><details class="rm-cat" ${i<3?'open':''} data-default-open="${i<3}"><summary>${r[3].length} change${r[3].length===1?'':'s'}</summary><ul>${r[3].map(x=>`<li>${x}</li>`).join('')}</ul></details></div></article>`).join('')}</div></section></main>${publicFooter()}</div>`;bindPublicNav();bindReleasesSearch()}
function bindReleasesSearch(){
 const input=document.getElementById('relSearch');
 const countEl=document.getElementById('relSearchCount');
 const items=[...document.querySelectorAll('#relList .release')];
 if(!input)return;
 input.oninput=()=>{
  const q=input.value.trim().toLowerCase();
  if(!q){
   items.forEach(item=>{item.style.display='';const d=item.querySelector('details.rm-cat');if(d)d.open=d.dataset.defaultOpen==='true'});
   countEl.textContent='';
   return;
  }
  let matches=0;
  items.forEach(item=>{
   ...
   item.style.display=hit?'':'none';
   const d=item.querySelector('details.rm-cat');
   if(hit&&d)d.open=true;
   if(hit)matches++;
  });
  countEl.textContent=`${matches} of ${items.length} releases`;
 };
}
function platformPage(){
 document.title='Platform — Lanesra OS';
 setPageMeta('The full Lanesra OS architecture: seven core no-code primitives (Custom Objects, Relationships, Screen/App Builder, Business Rules, Workflows, Reports, Dashboards) underneath a complete CRM, the AI & Agentic Layer (BYO LLM key, MCP server, AI Agent Foundry, orchestration), an 11-app Industry Data Model, Deployment Management and the Integration Hub - every claim here is shipped code, not a roadmap promise.');
 ...
 const industryApps=['Field Service','Property Management','Construction & Contractors','Professional Services','Practice Administration','Recruitment & Staffing','Real Estate Brokerage','Legal Practice','Nonprofit & Association Management','Auto Repair & Service Garage','Policy Administration & Claims Management'];
 const layers=[
  {icon:'◈',eyebrow:'Admin → Deployment Management',name:'Deployment Management',blurb:'Curate a named, versioned Solution from anything you\'ve built, export it, and import it into another workspace — a Publisher registry, component-tagging and update-with-diff included, so your customizations move between test and production the way a real software vendor would ship an update. Shipped on both the desktop app and the online demo.'},
  {icon:'⬡',eyebrow:'Admin → Integration Hub',name:'Integration Hub',blurb:'Connects that same workspace to everything else your business runs: encrypted Connections (REST, SFTP, PostgreSQL, MySQL, SQL Server, MongoDB, Redis, OData, SMTP), OpenAPI-imported Connectors - importable from a curated gallery of pre-built templates (AI models, data warehouses, and SaaS tools like Slack, GitHub, Stripe, HubSpot and more) or a hand-pasted spec - a generic REST API secured by scoped keys, HMAC-signed webhooks, a CSV data-exchange wizard, and scheduled Integration Jobs. A Tool Bridge turns an opted-in Connector Action into a real AI Agent Foundry tool, fail-closed on any ambiguous schema. Real on the desktop app and Team Workspace server; the online demo\'s version stays an explicitly labeled client-side simulation, since a static demo has no server to make a real call from.'},
  {icon:'🔐',eyebrow:'Admin → Access · Phases 1 &amp; 2 of 6',name:'Organization & Access Control',blurb:'An Organization Unit hierarchy (regions, divisions, teams — move one with an impact preview before it commits) and Work Teams with time-bounded memberships that never touch record ownership on their own. Every record across the CRM and your Custom Objects carries a structured Owner (a user or a team) and an Owning Organization Unit, individually or bulk-reassignable, with a full transfer history. Capability-scoped Access Roles, Record Scopes and an Access Inspector now gate record Assign and per-record CRUD, with an Administrator bypass that\'s never weakened. Field Access Policies and Enterprise Identity/SSO are next. Organization/Org Units/Work Teams/ownership are shipped on both the desktop app and the online demo; Access Roles and the Access Inspector are desktop edition first.'},
 ];
 const aiFoundryLayer={icon:'🏭',eyebrow:'Admin → AI Agent Foundry',name:'AI Agent Foundry',blurb:'Bring your own Anthropic, OpenAI-compatible or Google Gemini key - Lanesra never resells inference - then build on it: a conversational Chat Assistant with real tool access, and a full AI Agent Foundry where an Administrator defines named agents with a persona, a per-tool Actions checklist, persistent Memory, a shared Skills library, guardrails, and delegation to other agents. Chain agents into a deterministic Pipeline - sequential, consensus, or a peer-review loop - and fire it manually, on a schedule, from a webhook, or as a new Workflow Automation action, with human-in-the-loop approval gates and OTLP tracing on any run. A Context Layer gives agents ranked full-text search plus embeddings-based Vector Search over your records, and a real Evaluation Harness grades an agent\'s answers against golden test cases - every run lands in one unified history.'};
 const mcpLayer={icon:'⌁',eyebrow:'Admin → LLM & MCP → MCP Server',name:'MCP Server & CLI',blurb:'A stateless POST /mcp endpoint speaks the Model Context Protocol over JSON-RPC 2.0 - the same protocol Claude Desktop and most IDE coding agents use - securely exposing your Custom Objects, records and business logic to any MCP-capable client, authenticated with the same scoped API keys the REST API already uses. A companion CLI gives humans and shell scripts that same REST API access without an agent in the loop.'};
 $('#app').innerHTML=`<div class="site-dark">${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">The platform underneath the agents</div><h1>This isn't a CRM with AI bolted on. It's the platform your agents run on.</h1><p>Every agent in Lanesra's AI Agent Foundry — its tools, its memory, the records it reads and writes — runs directly against the same extensible data model every business object on this page is built from. Companies, Contacts, Opportunities, Quotes, Orders, Invoices, Contracts and Tasks aren't hardcoded, and neither is what an agent can see: an Administrator defines the objects, relationships and rules once, from a settings screen, and a human user and an AI agent operate against the identical model underneath — not a bolted-on chatbot with its own separate context pipeline to keep in sync.</p><div class="hero-actions"><a class="btn btn-primary" href="/demo">Try the live demo →</a><a class="btn btn-secondary" href="#examples">See what you could build ↓</a></div></div></section>
 <section id="ai-layer" class="section"><div class="container narrow"><div class="section-head" style="text-align:center;margin:0 auto 8px"><div class="eyebrow">Built for agents, not bolted on</div><h2>AI Agent Foundry & MCP.</h2><p class="muted">A full AI Agent Foundry, then a native MCP server & CLI for outside agents — both shipped, both running against the same data model below.</p></div><div class="platform-layers" style="max-width:820px;margin:0 auto">${[aiFoundryLayer,mcpLayer].map(l=>`<article class="platform-layer-card"><div class="platform-layer-icon">${l.icon}</div><div class="eyebrow">${l.eyebrow}</div><h3>${l.name}</h3><p>${l.blurb}</p></article>`).join('')}</div></div></section>
 <section class="section"><div class="container narrow"><div class="section-head" style="text-align:center;margin:0 auto 8px"><div class="eyebrow">Core CRM & No-Code Platform</div><h2>Seven primitives. One workspace.</h2><p class="muted">Every example below — and every tool an agent above can call — is assembled from these seven primitives. Nothing industry-specific is baked into the product itself; the Industry Data Model below is what you get when they're pre-assembled for you.</p></div><div class="platform-primitives">${primitives.map((p,i)=>`<span class="platform-primitive"><span class="n">${i+1}</span>${p}</span>${i<primitives.length-1?'<span class="platform-arrow">→</span>':''}`).join('')}</div></div></section>
 <section id="industry-data-model" class="section" style="background:var(--surface-alt,#f7f8fc)"><div class="container narrow"><div class="section-head" style="text-align:center;margin:0 auto 8px"><div class="eyebrow">Industry Data Model</div><h2>Eleven industries, pre-assembled.</h2><p class="muted">Each app below is the same seven primitives above, already modeled for a specific business — objects, relationships, layouts, rules, workflows and dashboards, packaged as a versioned Solution you install instead of build.</p></div><div class="platform-primitives">${industryApps.map((a,i)=>`<span class="platform-primitive"><span class="n">${i+1}</span>${a}</span>${i<industryApps.length-1?'<span class="platform-arrow">→</span>':''}`).join('')}</div><p class="muted" style="text-align:center;max-width:680px;margin:22px auto 0">Every app installs from <b>Admin → Apps → App Catalog</b> with pre-install validation, an automatic backup, and a transactional, rollback-safe install that reuses your existing Companies and Contacts rather than creating a parallel database. Each is a real, versioned <b>Solution</b> under the hood — the same package format <a href="/help/the-package-manifest">Deployment Management</a> uses for your own customizations, so an app you install is never a black box: open <a href="/help/install-an-industry-app">Admin → Deployment Management</a> afterward and you'll see exactly what components it added, ready to extend the same way you'd extend anything you built yourself.</p></div></section>
 <section id="examples" class="section"><div class="container"><div class="section-head"><div class="eyebrow">Make it real</div><h2>What this actually becomes.</h2><p class="muted">Four businesses that look nothing like a CRM, each modeled entirely from Admin → Custom Objects, Relationships, Screen layouts, Business rules, Workflow automation and Dashboards. Three of the four are no longer just examples — install them in a few clicks from Admin → App Catalog instead of building them by hand.</p></div><div class="app-examples">${apps.map(a=>`<article class="app-example-card"><div class="eyebrow">${a.eyebrow}${a.inCatalog?' · <span class="badge">In App Catalog</span>':''}</div><h3>${a.name}</h3><p class="tagline">${a.tagline}</p><ul class="example-detail">${a.details.map(d=>`<li>${d}</li>`).join('')}</ul><div class="built-from">${a.builtFrom.map(b=>`<span>${b}</span>`).join('')}</div>${a.inCatalog?'<p class="muted" style="margin-top:10px">Install it as-is from Admin → App Catalog, or use it as a starting point and customize freely.</p>':''}</article>`).join('')}</div></div></section>
 <section class="section"><div class="container"><div class="section-head" style="text-align:center;margin:0 auto 8px"><div class="eyebrow">Deployment Management, Integration Hub & Access Control</div><h2>Package it, connect it, govern it.</h2><p class="muted">Three more layers sit on top of the seven primitives and the Industry Data Model above — all shipped, all real, none of them a separate product.</p></div><div class="platform-layers">${layers.map(l=>`<article class="platform-layer-card"><div class="platform-layer-icon">${l.icon}</div><div class="eyebrow">${l.eyebrow}</div><h3>${l.name}</h3><p>${l.blurb}</p></article>`).join('')}</div><p class="muted" style="text-align:center;margin:22px auto 0;max-width:640px">A plain <b>App Builder</b> (Admin → Apps) still groups any set of objects into your own named, publishable application with its own icon, a sidebar App Switcher and access grants — nothing on this page requires a separate integration platform, ISV tooling, or AI subscription.</p></div></section>
 <section class="section"><div class="container narrow"><div class="honesty-note"><h3>What's real today, and what's next</h3><p>Every building block used above — Custom Objects, Relationships, Screen/App Builder, Business Rules, Workflow Automation, Dashboards, Reports, the App Catalog and Deployment Management — is shipped and working right now, in both the desktop app and the <a href="/demo">online demo</a>. Open Admin → Custom Objects and you can start building any of these examples yourself, this minute. The <a href="/roadmap">roadmap</a> tracks exactly what's still proposed, compiled from the working codebase, not a wishlist.</p></div></div></section>
 <section id="data-privacy" class="section"><div class="container narrow"><div class="honesty-note"><h3>Data & privacy</h3><p>A self-hosted Lanesra OS install stores everything in a single local SQLite database file you control - no account, no phone-home, no usage telemetry baked into the product itself, and nothing calls out to a model provider until you configure your own API key. The <a href="/demo">online demo</a> runs entirely in your browser (localStorage, reset anytime); this marketing site (lanesraos.com) does use privacy-respecting web analytics (PostHog) to understand visitor traffic, same as most product websites - that's a fact about this website, not about the software you'd run. See <a href="https://github.com/vikram2409-eng/Lanesra-OS/blob/main/SECURITY.md" target="_blank">SECURITY.md</a> for vulnerability reporting and the <a href="https://github.com/vikram2409-eng/Lanesra-OS/blob/main/LICENSE" target="_blank">MIT LICENSE</a> for what you're free to do with the code.</p></div></div></section>
...
function comparePage(){document.title='Compare — Lanesra OS';
 setPageMeta('How Lanesra OS compares to HubSpot, Zoho CRM, Salesforce and Microsoft Dynamics 365 on pricing, customization, AI agents, deployment and lock-in - sourced against each vendor\'s own current public pricing/docs, dated for verification, not assumed.');
 // Columns: HubSpot, Zoho CRM, Salesforce, Dynamics 365, Lanesra OS. Zoho
 // and Dynamics 365 replaced Excel (2026-09) - Excel isn't a real peer to
 // CRM platforms; researched facts behind each cell: Zoho CRM is cloud-only,
 // no-code is real for layouts (Canvas) but custom logic needs its Deluge
 // scripting language, and Zia AI is proprietary/tier-gated (Enterprise+
 // for Zia, Ultimate for the no-code Zia Agent Studio); Dynamics 365 Sales
 // runs on the Power Platform/Dataverse, giving it the deepest real no-code
 // depth of the four vendors but as separately-licensed Power Apps/Power
 // Automate, and Copilot is bundled but usage-metered beyond a base
 // allowance. Dynamics 365 also has a legacy on-premise deployment option
 // (Microsoft has been steering customers to cloud-first for years) -
 // noted, not treated as equivalent to a bundled self-hosted install.
 const groups=[
  ['Ownership & deployment',[
   ['Runs without internet','No','No','No','No (legacy on-prem exists)','Yes'],
   ['Self-owned business data','Cloud-hosted','Cloud-hosted','Cloud-hosted','Cloud-hosted','Yes'],
   ['Local database','No','No','No','No','Yes (desktop)'],
   ['Open source','No','No','No','No','Yes'],
   ['Mandatory subscription','Yes','Yes','Yes','Yes','No'],
  ]],
  ['Sales workflow',[
   ['Connected sales workflow','Limited','Yes','Advanced','Advanced','Yes'],
   ['Designed for small business','Yes','Yes','Enterprise','Enterprise','Yes'],
  ]],
  ['No-code customization',[
   ['Custom record types, no code','Limited, paid tiers','Modules, paid tier + scripting for logic','Yes, complex/paid','Yes, via Power Apps (separate license)','Yes, built in'],
   ['Custom screens & layouts by role','Limited','Canvas, no-code (paid tier)','Yes, complex/paid','Yes, via Power Apps (paid)','Yes, built in'],
   ['Custom business rules & workflow automation','Paid tiers','Workflow rules + Deluge scripting (paid tiers)','Yes, needs admin training','Power Automate (separate paid license)','Yes, built in'],
   ['Build a whole custom app on the platform','No','No (separate Zoho Creator product)','Yes, paid tiers','Yes, Power Apps (separate paid license)','Yes, built in'],
  ]],
  ['Apps, packaging & connections',[
   ['Ready-made industry apps to install, not just build','No','Marketplace/partner-built, not bundled','AppExchange, paid','Partner/ISV accelerators, several deprecated','Yes, 11 built in'],
   ['Package & promote customizations, test → production','No','Sandbox, paid tiers','Yes, paid tiers (Change Sets/DevOps Center)','Solutions/ALM via Power Platform, paid & complex','Yes, built in'],
   ['Native REST API, webhooks & scheduled sync','Paid tiers','Yes, webhooks from Professional tier up','Yes, paid tiers/limits','Yes, via Dataverse/Power Automate (paid)','Yes, built in'],
  ]],
  ['AI & Agentic Layer',[
   ['Bring your own LLM key, no markup or reselling','No, bundled AI credits','No, bundled Zia credits (paid tier)','No, bundled Einstein/Agentforce credits','No, bundled + metered Copilot credits','Yes, built in'],
   ['MCP server & CLI for outside AI agents','No','No','No','No','Yes, built in'],
   ['Conversational AI assistant with real tool access','Limited, paid tiers','Zia, paid tier (Enterprise+)','Yes, paid add-on (Agentforce)','Yes, Copilot (bundled base + metered)','Yes, built in'],
   ['Custom AI agents with memory, skills & delegation','No','Zia Agent Studio, top paid tier only','Yes, paid add-on (Agentforce)','Copilot Studio, separate paid product','Yes, built in'],
   ['Multi-agent orchestration & workflow-triggered runs','No','No formal orchestration','Yes, paid add-on','Copilot Studio + Power Automate, paid add-on','Yes, built in'],
  ]],
  ['Deployment & scale',[
   ['Concurrent multi-user writes','Yes, cloud multi-tenant','Yes, cloud multi-tenant','Yes, cloud multi-tenant','Yes, cloud multi-tenant','Yes (SQLite today; Postgres migration planned for scale)'],
   ['Multi-location / multi-geo remote access','Yes','Yes','Yes','Yes','Yes (self-hosted Team Workspace on your own domain)'],
   ['Self-hosted, open-source install','No','No','No','No (legacy on-prem, proprietary)','Yes'],
   ['Multi-tenant Cloud SaaS tier available today','Yes','Yes','Yes','Yes','Planned — see <a href="/roadmap">roadmap</a>'],
  ]],
 ];
 $('#app').innerHTML=`<div class="site-dark">${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Choose with context</div><h1>Where Lanesra fits.</h1><p>A factual comparison for small businesses deciding between cloud CRMs and a local-first open-source system.</p></div></section><section class="section"><div class="container compare-wrap"><table class="compare-table"><thead><tr><th>Capability</th><th>HubSpot</th><th>Zoho CRM</th><th>Salesforce</th><th>Dynamics 365</th><th class="lanesra-col">Lanesra OS</th></tr></thead><tbody>${groups.map(g=>`<tr class="compare-group-row"><td colspan="6">${g[0]}</td></tr>${g[1].map(r=>`<tr>${r.map((x,i)=>`<td class="${i===5?'lanesra-col':''}">${x}</td>`).join('')}</tr>`).join('')}`).join('')}</tbody></table><p class="compare-note">Comparisons are intentionally high-level and based on public vendor documentation as of September 2026. Product capabilities and commercial terms change often; review each vendor's current documentation before making a purchase decision.</p><details class="rm-cat"><summary>Sources &amp; methodology</summary><ul style="margin:0;padding:18px 24px;line-height:1.8"><li>Zoho CRM is cloud-only; its no-code layout builder (Canvas) is real, but custom business logic needs Zoho's own Deluge scripting language, and its Zia AI is proprietary and tier-gated (Enterprise+ for Zia itself, Ultimate for the no-code Zia Agent Studio).</li><li>Dynamics 365 Sales runs on Microsoft's Power Platform/Dataverse, giving it the deepest real no-code depth of the three cloud vendors compared here - but as separately-licensed Power Apps/Power Automate, not bundled. Copilot is included but usage-metered beyond a base allowance. A legacy on-premise deployment option exists but Microsoft has steered customers to cloud-first for years, so it isn't treated as equivalent to a bundled self-hosted install.</li><li>An Excel-based comparison was deliberately dropped (2026-09) in favor of Zoho CRM and Dynamics 365 - a spreadsheet isn't a real peer to purpose-built CRM platforms.</li><li>Every cell above reflects each vendor's own public pricing/docs at time of writing, not a live feed - verify current terms directly with the vendor before deciding.</li></ul></details></div></section></main>${publicFooter()}</div>`;bindPublicNav();bindCompareScrollCue()}
function bindCompareScrollCue(){
 const wrap=document.querySelector('.compare-wrap');
 if(!wrap)return;
 const update=()=>{wrap.classList.toggle('at-end',wrap.scrollLeft+wrap.clientWidth>=wrap.scrollWidth-4)};
 wrap.addEventListener('scroll',update);
 update();
}
function downloadPage(){document.title='Download — Lanesra OS';
 setPageMeta('Download Lanesra OS for Windows (Early Access, unsigned installer) - a self-hosted desktop edition with no cloud account, no internet requirement, and no activation. macOS and Linux support is planned. See what ships in the desktop build vs. the online demo.');
 $('#app').innerHTML=`<div class="site-dark">${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Local-first desktop edition</div><h1>Download Lanesra OS.</h1><p>The independent desktop edition runs locally with no cloud account or mandatory internet connection. It is in active early development, with an Early Access Windows installer now available.</p></div></section><section class="section"><div class="container download-grid"><article class="download-card featured"><span class="status-chip">Early access — installer available</span> <span class="muted" style="font-size:12px">desktop-v0.13.0 · <a href="/releases">what's new since →</a></span><h2>Windows</h2><p>Tauri + Rust + SQLite desktop app with the full sales lifecycle, Contracts, Tasks and user management working: Companies, Contacts, Products, Opportunities, Quotes, Orders, Invoices, Contracts and Tasks, an Admin panel for the whole no-code platform, an App Catalog of 11 install-ready industry apps, Deployment Management, an Integration Hub, and - bring your own LLM key - the full AI & Agentic Layer.</p><details class="rm-cat"><summary>What's included, in detail</summary><p>Team Workspace mode for small teams, backup and restore, PDF printing, CSV import/export, admin-defined Custom Objects and relationships between any record types, and an Admin panel covering branding, user roles, custom fields, richer conditional business rules, richer workflow automation with in-app notifications, configurable ID formats, a Screen/App Builder for every object's create/edit and detail layouts, named dashboard layouts assigned by role with chart and record-list widgets, App Builder — grouping objects, screens and a dashboard into a named, publishable app with server-enforced Viewer/Editor access — and an App Catalog of 11 install-ready industry apps (Field Service, Property Management, Construction & Contractors, Professional Services, Practice Administration, Recruitment & Staffing, Real Estate Brokerage, Legal Practice, Nonprofit & Association Management, Auto Repair & Service Garage, Policy Administration & Claims Management), each installed with pre-install validation, an automatic backup and a transactional, rollback-safe install. On top of that, Deployment Management (Publishers, versioned Solutions, component-tagging, export/import and update-with-diff) lets you package and promote your own customizations between workspaces, and an Integration Hub connects the workspace to everything else you run - encrypted Connections, OpenAPI Connectors, a generic REST API, HMAC-signed webhooks, CSV data exchange and scheduled Integration Jobs. On top of all of that, bring your own LLM key and the AI & Agentic Layer takes over: a native MCP server & CLI, a Chat Assistant with real tool access to records and the admin configuration surface, the AI Agent Foundry (named agents with persona, Memory, Skills, Actions and delegation), Orchestration (deterministic Pipelines fired manually, on a schedule, from a webhook, or by a new Workflow Automation action, with one unified run history), a Unified AI Gateway (multi-provider routing, automatic failover, token budgets, DLP-driven air-gapping), and a Context Layer (real memory-change history plus ranked full-text search over Custom Object records). Everything since - agent guardrails, declarative export/import, consensus/peer-review orchestration with a real Evaluation Harness, approval gates extended to every topology, OTLP collector push, an agent context primer and Vector Search - is merged to <code>main</code> but not yet in this tagged installer; see <a href="/roadmap#desktop">/roadmap#desktop</a> for the exact cutoff. Unsigned .exe and .msi installers are on GitHub Releases (Windows will warn on first run since they aren't code-signed yet).</p></details><a class="btn btn-primary" href="https://github.com/vikram2409-eng/Lanesra-OS/releases" target="_blank" rel="noopener">Download for Windows</a><a class="btn btn-secondary" href="https://github.com/vikram2409-eng/Lanesra-OS/tree/main/desktop" target="_blank" rel="noopener">View desktop source on GitHub</a></article><article class="download-card"><span class="status-chip">Planned</span><h2>macOS</h2><p>Apple silicon and Intel packaging will follow the Windows early-access release.</p><a class="btn btn-secondary" href="https://github.com/vikram2409-eng/Lanesra-OS/releases" target="_blank" rel="noopener">Watch releases on GitHub →</a></article><article class="download-card"><span class="status-chip">Planned</span><h2>Linux</h2><p>AppImage or Debian packaging is planned after the initial desktop release stabilizes.</p><a class="btn btn-secondary" href="https://github.com/vikram2409-eng/Lanesra-OS/releases" target="_blank" rel="noopener">Watch releases on GitHub →</a></article></div></section><section class="section maintenance"><div class="container narrow"><h2>What the desktop edition includes today</h2><div class="download-checks"><span>✓ No licence key</span><span>✓ No cloud account</span><span>✓ Standard SQLite database</span><span>✓ Offline from first launch</span><span>✓ Full sales lifecycle (quotes → orders → invoices)</span><span>✓ Contracts and tasks</span><span>✓ User management</span><span>✓ Team Workspace mode for small teams (Docker)</span><span>✓ Windows installer (unsigned, Early Access)</span><span>✓ Backup and restore</span><span>✓ Self-service password change</span><span>✓ PDF generation and printing</span><span>✓ CSV import and export</span><span>✓ Branding and print customization</span><span>✓ Reports, plus a custom report builder</span><span>✓ Custom fields & business rules on every object</span><span>✓ Workflow automation with in-app notifications</span><span>✓ Admin-defined Custom Objects</span><span>✓ Custom relationships between record types</span><span>✓ Screen/App Builder — layouts by role, Draft → Publish</span><span>✓ Dashboard customization — KPI, chart & record-list widgets by role</span><span>✓ App Builder — publish named apps with server-enforced access</span><span>✓ App Catalog — 11 install-ready industry apps</span><span>✓ Per-app scoped business rules, workflows & dashboards</span><span>✓ Deployment Management — Publishers, versioned Solutions, export/import</span><span>✓ Integration Hub — Connections, Connectors, REST API, webhooks, scheduled sync</span><span>✓ Windows task reminder notifications</span><span>✓ Session inactivity auto-lock</span><span>✓ Admin panel: user roles & configurable numbering</span><span>✓ Open-source code</span><span>○ Code-signed installer — planned</span></div><div class="hero-actions"><a class="btn btn-secondary" href="/roadmap#desktop">View desktop roadmap</a><a class="btn btn-secondary" href="https://github.com/vikram2409-eng/Lanesra-OS/tree/main/desktop" target="_blank" rel="noopener">View source on GitHub</a></div></div></section></main>${publicFooter()}</div>`;bindPublicNav()}
...
// Product Help: detailed, task-oriented admin user guides, one category at
// a time - starting with the Agentic AI Foundry (the admin panel's own
// "AI Agent Foundry" category, ai-agent-foundry). Static content shipped
// with the site, same delivery model as the roadmap/releases pages above -
// no CMS, no database. Deliberately duplicated (not imported) into the
// desktop admin panel's own desktop/src/lib/helpContent.ts, matching this
// codebase's established demo/desktop boundary convention.
// Shape: Category{key,label,icon,blurb,topics:Topic[]}
...
const HELP_CATEGORIES=[
 {key:'ai-agent-foundry',label:'Agentic AI Foundry',icon:'🏭',
  ...
  topics:[
   {slug:'connect-llm-provider',title:'Getting started: connect an LLM provider',
    summary:'Add your own Anthropic, OpenAI-compatible or Google Gemini key, understand what the MCP server and CLI are each for, and try the Chat Assistant.',
    sections:[
   ...
   {slug:'build-your-first-agent',title:'Build your first AI Agent',
    summary:'Persona, the Actions checklist, persistent Memory, the Skills library, Guardrails and delegation to sub-agents - end to end from Admin → AI Agent Foundry → AI Agents.',
    sections:[
     {heading:'Create the agent',bodyHtml:`<p>Go to <b>Admin → AI Agent Foundry → AI Agents → New Agent</b>. An agent needs a name, an icon (pick from the provided set), a short description, and a <b>system prompt</b> - the persona: who it is, what it's for, and the tone/scope it should stay inside. This prompt is the primary lever for behavior; everything else on this page (Actions, Memory, Skills, Guardrails, Delegation) adds capability around it.</p>`},
     {heading:'The Actions checklist',bodyHtml:`<p>Actions are the per-tool checklist this agent is allowed to call. They come in two catalogs:</p><ul><li><b>Record actions</b> (8): <code>list_objects</code>, <code>get_object_metadata</code>, <code>list_records</code>, <code>get_record</code>, <code>create_record</code>, <code>update_record</code>, <code>archive_record</code>, <code>search_records</code> - the same object model any user's own records already sit in.</li><li><b>Admin actions</b> (37): read/write pairs spanning Business Rules, Workflow Automation, Custom Objects/Fields/Relationships/Status Transitions, Integration Hub (Connections, Webhooks, Integration Jobs, API Clients), Dashboards, Apps, Custom Reports, Saved Views, Users, Numbering, the Workspace Profile, Connectors, and even AI Agents/Skills themselves.</li></ul><p class="help-note">Checking even one admin action makes the whole agent <b>Administrator-only</b> to chat with, computed automatically - there's no separate "trust level" field to also remember to set.</p>`},
     {heading:'Persistent Memory',bodyHtml:`<p>Every agent carries one Memory document - free text it reads on every run and can revise itself via its own <code>update_memory</code> tool call (an admin can also edit it directly, to seed it or correct it). The prior value is snapshotted before every real overwrite, viewable from the agent's own <b>Show memory history</b> panel, most-recent-first, so what an agent has taught itself over time is auditable. A save that leaves Memory empty or genuinely unchanged writes no new snapshot.</p>`},
     {heading:'The Skills library',bodyHtml:`<p>A Skill is a separate, reusable module: a short name and description (always visible to the agent up front, cheaply) plus full instructions that are only loaded into context when the agent decides a task actually calls for that skill. Build a library once under <b>Admin → AI Agent Foundry → AI Skills</b>, then attach whichever ones apply to each agent - the same skill can be shared across many agents instead of copy-pasted into each one's system prompt.</p>`},
     {heading:'Guardrails',bodyHtml:`<p>An agent's own <code>guardrails_md</code> field is plain text injected into its system prompt as an explicit operational-boundary statement - "never archive a record without explicit confirmation," for instance. Treat it as advisory, the same real/advisory split the rest of the product draws between a persona statement and a code-enforced rule. One guard on top of it <i>is</i> code-enforced and not optional: the tool-calling loop aborts with a clear error the moment the exact same tool call with the exact same arguments repeats three times in a row, so a confused agent can't spin forever against your data.</p>`},
     {heading:'Delegation to sub-agents',bodyHtml:`<p>An agent can name other agents as delegates - during a run it can call a named sub-agent, get its real answer back, and use that answer in its own response, the same way a supervisor might hand off a sub-task to a specialist. This is dynamic (the agent decides at runtime whether to delegate) and depth-guarded: a chain of delegate calls is capped at 4 levels deep, so a delegation loop can't recurse indefinitely. This is a different mechanism from a Pipeline (see the next article) - delegation is the agent's own runtime decision, a Pipeline is an admin-authored fixed sequence.</p>`},
   ...
   {slug:'unified-ai-gateway',title:'Unified AI Gateway: routing, failover, budgets & DLP',
    summary:'Named Providers beyond the default, per-agent primary/fallback/local-fallback routing, System → Agent token budgets, and DLP-driven forced air-gapping.',
    sections:[
   ...
   {slug:'context-layer-vector-search',title:'Context Layer & Vector Search',
    ...
    sections:[
     {heading:'Two ways an agent finds records',bodyHtml:`<p>Custom Object records can be searched two different ways, both exposed as agent tools:</p><table><thead><tr><th>Tool</th><th>How it ranks</th></tr></thead><tbody><tr><td><code>search_records</code></td><td>Lexical, keyword-based - a SQLite FTS5 index kept in sync by database triggers, ranked with <code>bm25()</code>. Fast, always on, no setup required.</td></tr><tr><td><code>semantic_search_records</code></td><td>Meaning-based - real embedding vectors compared by cosine similarity, finding a relevant record even when it shares none of the query's exact words. Requires Vector Search to be set up first (next section).</td></tr></tbody></table>`},
     {heading:'Setting up Vector Search — Admin → LLM & MCP → Gateway',bodyHtml:`<p>Vector Search is a card inside the <b>Gateway</b> sub-tab, not a standalone tab. It needs a provider that actually offers an embeddings endpoint: <b>OpenAI-compatible</b> or <b>Google Gemini</b> work; <b>Anthropic has no embeddings API</b>, so an Anthropic-only workspace can use lexical <code>search_records</code> but not semantic search until a second, embeddings-capable provider is configured.</p><ol><li>Optionally set an embedding-model override, if the default for your provider isn't what you want.</li><li>Click <b>Reindex now</b> to embed every existing Custom Object record.</li><li>The panel shows a live embedded/pending count as reindexing runs.</li></ol><p>After the initial reindex, a record's own create/update/archive automatically enqueues it for re-embedding - it never blocks the save on a live provider call. There's no bundled vector database or native extension: vectors are stored as plain BLOBs in SQLite, and cosine similarity is computed in Rust at query time.</p>`},
     {heading:'Memory change history',bodyHtml:`<p>Separately from record search, each AI Agent's own persistent Memory keeps a full change history - see <a href="/help/build-your-first-agent">Build your first AI Agent</a>'s Memory section for the "Show memory history" panel.</p>`},
     {heading:'The agent context primer',bodyHtml:`<p>Three tools help an agent (or you, testing one) understand how a workspace is actually built before it acts on something unfamiliar:</p><ul><li><code>get_object_metadata</code> - now includes every Custom Relationship an object participates in, from either direction, with the correct label and related object already resolved.</li><li><code>get_related_records</code> - follows one of those relationships for a specific record, the same lookup the UI's own "Related records" panel uses.</li><li><code>get_platform_overview</code> - explains how Lanesra OS's own primitives compose (Custom Object → Fields → Relationships → Business Rules → Workflow Automation → App → Dashboard) plus a live count of this workspace's own objects, relationships and apps.</li></ul>`},
    ]},
   {slug:'evaluation-harness',title:'Evaluation Harness: testing your agents',
    summary:'Write a Suite of golden test Cases with plain-English success criteria, understand LLM-as-judge grading, and why a malformed judge reply fails closed.',
    sections:[
     {heading:'Where this lives',bodyHtml:`<p><b>Admin → AI Agent Foundry → Evaluations</b> - a way to regression-test an Agent or Pipeline's actual behavior the way you'd test code, without a brittle exact-match assertion an open-ended LLM response could never reliably pass.</p>`},
     {heading:'Writing a Suite and its Cases',bodyHtml:`<p>Create a <b>Suite</b> naming which Agent or Pipeline it targets, then add <b>Cases</b> to it - each one an <code>input_text</code> (what you'd actually send it) and a <code>success_criteria</code> written in plain English ("the answer must cite a specific revenue figure and not just describe the report," for example, rather than a fixed expected string). Write several cases per suite covering both the behavior you want and the edge cases you're worried about.</p>`},
     {heading:'How grading actually works',bodyHtml:`<p>Running a suite executes every case's target Agent or Pipeline for real, then makes one additional LLM call - the judge - handing it the original input, your success criteria, and the actual output, and asking for a verdict. The harness reads only the <b>first non-empty line</b> of the judge's reply: it must read exactly <code>PASS</code> (case-insensitive) to count as a pass, with the rest of the reply kept as the recorded <i>reasoning</i>.</p>`},
     {heading:'Fails closed, not open',bodyHtml:`<p>Two failure paths are handled deliberately rather than silently: a case whose target run itself errors (a provider outage, a deleted agent) is recorded as failed without wasting a judge call on it, and a judge reply that doesn't parse as a clean PASS/FAIL - garbled, off-topic, empty - is graded as a <b>failure</b>, never as a silent pass. A flaky or confused judge can only make your suite too strict, never falsely green.</p>`},
     {heading:'Reading results',bodyHtml:`<p>Each run shows a per-case pass/fail with the judge's own reasoning text next to it, so a failure is immediately explainable - not just a red mark, but the judge's actual stated reason the response didn't meet your criteria.</p>`},
    ]},
   {slug:'extending-the-foundry',title:'Extending the Foundry',
    summary:'Use the MCP server from an external client, export Agents/Skills as part of a Solution, fire Pipelines from automation or webhooks, and the REST API underneath it all.',
    sections:[
     {heading:'Point an external AI client at your own workspace',bodyHtml:`<p>Any MCP-capable client - Claude Desktop, an IDE coding agent, a custom script using an MCP SDK - can be pointed at your Team Workspace's <code>POST /mcp</code> endpoint using an API client key from <b>Integration Hub → API Access</b> (see <a href="/help/connect-llm-provider">Getting started</a> for the exact scopes). Once connected, that external agent reads and writes your real records under the identical permission checks a person's own UI action already goes through - it's the same dispatcher, not a separate, looser path.</p>`},
 ...
 {key:'core-crm-no-code',label:'Core CRM & No-Code Platform',icon:'🧩',
  blurb:'Run the built-in sales lifecycle, then reshape the workspace itself - your own record types and relationships, admin-defined fields, conditional business rules, workflow automation, drag-and-drop screen layouts, and your own published app with its own dashboard.',
  topics:[
   {slug:'core-crm-sales-lifecycle',title:'The core CRM: companies, contacts & the sales lifecycle',
    summary:'The connected record model, the flexible Company → Opportunity → Quote → Order → Invoice path, and where to find things.',
    sections:[
     {heading:'The connected record model',bodyHtml:`<p>Everything in the core CRM hangs off a <b>Company</b>: its <b>Contacts</b>, its <b>Opportunities</b>, and every <b>Quote</b>, <b>Order</b>, <b>Invoice</b>, <b>Contract</b> and <b>Task</b> that references it. This isn't just cosmetic grouping - a Company's own detail page (its "360" view) surfaces every one of those linked records in one place, one click away, rather than leaving you to search for them separately.</p>`},
     {heading:'The flexible path: Company → Opportunity → Quote → Order → Invoice',bodyHtml:`<p>The full lifecycle is <b>Company → Contact → Opportunity → Quote → Order → Invoice</b>, with <b>Products &amp; Services</b> and <b>Contracts</b> attaching along the way and <b>Tasks</b> trackable against any of them. It doesn't have to start at the top: a <b>direct-quote</b> or <b>direct-order</b> shortcut lets you create a Quote or Order straight from a Company when there was no formal Opportunity to track - the full pipeline is there when you want it, not a mandatory gate.</p>`},
     {heading:'Line items & money math',bodyHtml:`<p>Quotes, Orders, Invoices and Contracts all carry real line items against Products &amp; Services, with totals computed from them. Money is tracked in integer cents internally, not floating-point decimals - the same class of bug that causes a spreadsheet's totals to drift by a cent over enough rows simply can't happen here.</p>`},
     {heading:'Data integrity, built in',bodyHtml:`<p>Two guards run automatically, not as something you configure: a warning when you're about to create a Company or Contact with a name or email that already exists (a duplicate check, not a hard block - you can still proceed deliberately), and gap-free, sequential document numbering per object - an admin sets the ID format for each entity (see the Custom Objects article for the exact same prefix/digit-width mechanism applied to your own record types).</p>`},
     {heading:'Finding things',bodyHtml:`<p>Every ID everywhere in the product - on a list, inside another record's related-records panel - is a hyperlink straight to that record. The Dashboard surfaces clickable, filterable KPIs; global search resolves a query straight to a matching record. Company and Contact both get a dedicated "360" detail page: full field overview plus every linked record, one click away, instead of edit-modal-only access.</p>`},
     {heading:'Reports & data safety',bodyHtml:`<p>Beyond the Dashboard, <b>Reports</b> gives you a fixed gallery (revenue by month, win rate, AR aging, sales by owner) plus a simple <b>Custom Reports</b> builder - pick any built-in or Custom Object, group by its status/stage or a field flagged Reportable, count or sum, export to CSV. For any Custom Object that carries its own active <b>Valid From</b> date field (the convention Property Management-style Lease/Owner Interest objects already use), a Custom Report also offers an <b>"as of" date</b> filter - was this record's stated validity window in effect on that date - a predicate over each record's currently-stored Valid From/Valid To values, not a reconstruction of what a field looked like on a past date. Whole-workspace backup/restore (a <code>.lanesra</code> file) and CSV import/export round out the data-safety story - see the CRM article's own admin screens for exactly where each lives.</p>`},
     {heading:'Next: make it yours',bodyHtml:`<p>Everything above is the fixed starting point. The rest of this category - <a href="/help/custom-objects">Custom Objects</a>, <a href="/help/custom-relationships">Custom Relationships</a>, <a href="/help/custom-fields">Custom Fields</a>, <a href="/help/business-rules">Business Rules</a>, <a href="/help/workflow-automation">Workflow Automation</a>, <a href="/help/screen-app-builder">Screen Builder</a> and <a href="/help/apps-dashboards">App Builder &amp; Dashboards</a> - is how an admin reshapes the workspace itself without writing code.</p>`},
    ]},
   {slug:'custom-objects',title:'Custom Objects: your own record types',
    summary:'Define a whole new business object at runtime - its own icon, ID format and sidebar entry - no code change.',
    sections:[
     {heading:'Why Custom Objects',bodyHtml:`<p>The 9 built-in entities (Companies, Contacts, Opportunities, Products, Quotes, Orders, Invoices, Contracts, Tasks) cover the common sales lifecycle, but a real business usually has something outside it - Vendors, Assets, Projects, whatever your organization actually runs. A Custom Object is a genuinely new record type, not a repurposed built-in one, and it plugs into every other admin tool (Custom Fields, Business Rules, Workflow Automation, the Report builder, Screen Builder) exactly like a built-in entity does.</p>`},
     {heading:'Create one — Admin → Data Model → Custom Objects',bodyHtml:`<p>New Custom Object needs four things:</p><table><thead><tr><th>Field</th><th>Example</th></tr></thead><tbody><tr><td>Singular name</td><td>Vendor</td></tr><tr><td>Plural name</td><td>Vendors</td></tr><tr><td>Icon</td><td>picked from a provided set</td></tr><tr><td>Record-number prefix &amp; digit width</td><td>prefix <code>VEN</code>, width 6 → IDs like <code>VEN-000001</code></td></tr></tbody></table><p>The prefix and digit width are the exact same numbering mechanism every built-in object already uses (see the CRM lifecycle article) - just admin-configurable per object here instead of fixed in code.</p>`},
     {heading:'What you get automatically',bodyHtml:`<p>Saving creates a new sidebar navigation entry with its own icon, and the object immediately works through the platform's existing machinery: add fields from <a href="/help/custom-fields">Custom Fields</a>, connect it to other objects from <a href="/help/custom-relationships">Custom Relationships</a>, drive its behavior with <a href="/help/business-rules">Business Rules</a> and <a href="/help/workflow-automation">Workflow Automation</a>, design its form with <a href="/help/screen-app-builder">Screen Builder</a>, and group it into your own app in <a href="/help/apps-dashboards">App Builder</a>. There's no separate, smaller feature set for a Custom Object versus a built-in one.</p>`},
     {heading:'Deactivate vs. delete',bodyHtml:`<p>Delete is blocked outright while any records exist under the object - there's no accidental data loss path. Deactivate is always safe and fully reversible: it hides the object from navigation without touching a single record, so you can retire something without committing to destroying its history.</p>`},
    ]},
   {slug:'custom-relationships',title:'Custom Relationships: connecting any two objects',
    summary:'One-to-one, many-to-one or many-to-many links between any two record types, built-in or custom, with automatic related-records panels.',
    sections:[
     {heading:'Connect any two objects — Admin → Data Model → Relationships',bodyHtml:`<p>Pick a <b>Source</b> (the "many"/owning side) and a <b>Target</b>, either of which can be a built-in entity or a Custom Object - a relationship doesn't care which kind either side is.</p><table><thead><tr><th>Cardinality</th><th>Meaning</th></tr></thead><tbody><tr><td>Many-to-one</td><td>Many source records, one target each</td></tr><tr><td>One-to-one</td><td>Exactly one on each side</td></tr><tr><td>Many-to-many</td><td>Either side can link to several of the other</td></tr></tbody></table>`},
     {heading:'Forward & reverse labels',bodyHtml:`<p>Set a <b>Forward label</b> (shown on the source record, e.g. "Primary Vendor") and a <b>Reverse label</b> (shown on the target record, e.g. "Companies supplied") - a relationship reads naturally from both directions instead of exposing its internal source/target framing to whoever's looking at either record.</p>`},
     {heading:'On delete: Restrict or Archive',bodyHtml:`<table><thead><tr><th>Behavior</th><th>What happens</th></tr></thead><tbody><tr><td>Restrict</td><td>Blocks archiving a linked record while the link exists - forces a deliberate unlink first</td></tr><tr><td>Archive</td><td>Drops the link automatically, keeps both records intact</td></tr></tbody></table>`},
     {heading:'Two more options',bodyHtml:`<p><b>Show as a related list on both records</b> puts an automatic related-records panel on each side's detail/edit view - the common case, and what most relationships want. <b>Source record should have a target linked</b> makes the link itself required, the way a required field works, enforced the same way.</p>`},
     {heading:'Self-referential relationships: parent/child hierarchies',bodyHtml:`<p>Source and Target don't have to be different object types - pick the same one on both sides to model a parent/child hierarchy on a single object, an Organization Unit's parent unit or an Asset's parent Asset. The two directions render as two distinct, correctly-labeled related-list groups (Forward label on one, Reverse label on the other) instead of merging into one confusing list, and the one real guard that shape needs is enforced automatically: a record can never link to itself.</p>`},
     {heading:'Polymorphic targets: "any record type"',bodyHtml:`<p>Check <b>Target can be any record type</b> and the Target picker disappears - any valid built-in or Custom Object type in the workspace becomes a legal target, chosen per link instead of fixed on the relationship itself (a Document Record attached to a Policy, a Claim, or a Matter). Linking shows a type picker before the record picker, and every related-records list already resolves each link's real type correctly. One limit, stated plainly: actions that write to the target side automatically (Workflow Automation's <b>Update a related record</b>, and <b>Create a new record</b>'s "link to" option) aren't supported against a polymorphic target's variable side yet - only reading related records back out is.</p>`},
     {heading:'Where it shows up',bodyHtml:`<p>Once saved, every linked record gets a "Related records" panel on its detail page with inline Link/Unlink, in both directions. This is also exactly the lookup an AI Agent's own <code>get_related_records</code> tool uses - see the Agentic AI Foundry category's <a href="/help/context-layer-vector-search">Context Layer &amp; Vector Search</a> article for the agent-facing side of the same relationship data.</p>`},
    ]},
   {slug:'custom-fields',title:'Custom Fields: types, validation & capability flags',
    summary:'Five field types, min/max/length/regex validation, and the Searchable/Filterable/Reportable flags that plug a field into the rest of the platform.',
    sections:[
     {heading:'Field types — Admin → Data Model → Custom fields',bodyHtml:`<table><thead><tr><th>Type</th><th>Notes</th></tr></thead><tbody><tr><td>Text</td><td>Optional max length and a regex pattern</td></tr><tr><td>Number</td><td>Optional minimum and maximum</td></tr><tr><td>Date</td><td>&nbsp;</td></tr><tr><td>Yes/No</td><td>Boolean</td></tr><tr><td>Select</td><td>A comma-separated option list you define</td></tr></tbody></table><p>Add a field to any built-in entity or Custom Object - the same builder, the same options, regardless of which.</p>`},
   ...
   {slug:'business-rules',title:'Business Rules: conditions, effects & status transitions',
    ...
    sections:[
     {heading:'The condition/effect model — Admin → Automation → Business rules',bodyHtml:`<p>A rule is IF (one or more conditions, AND or OR, plus one level of nested OR-groups) THEN (any number of effects) - evaluated live as a user fills out a form, and enforced again server-side on save, so a rule can never be bypassed by skipping the UI.</p>`},
     {heading:'Conditions: 12 operators',bodyHtml:`<p>equals · does not equal · contains · does not contain · starts with · ends with · is one of · is not one of · is empty · is not empty · is greater than · is less than - matched against any built-in or custom field, with <b>All conditions (AND)</b> or <b>Any condition (OR)</b> as the top-level match type.</p>`},
     {heading:'Reading a related record\'s field',bodyHtml:`<p>A condition doesn't have to read the triggering record's own field - pick <b>"Read value from"</b> a relationship first, then a field on the record linked through it, and the condition matches against <i>that</i> record's value instead. Only relationships where the triggering record has at most one linked record are offered (a many-to-one's "many" side, or either side of a one-to-one) - a many-to-many, or the "one" side of a many-to-one, has no single determinate related record to read, so those aren't eligible. Use this for the shape "the selected Asset must belong to the selected Site," or Property Management's "the Unit must not already have an overlapping active Lease"-style checks, where the missing piece was always reaching across the relationship, not the condition logic itself.</p>`},
     {heading:'Effects',bodyHtml:`<table><thead><tr><th>Group</th><th>Effects</th></tr></thead><tbody><tr><td>Validation</td><td>Require field · Block save (with a message) · Show error · Show warning</td></tr><tr><td>Field behavior</td><td>Show / Hide field · Make read-only / Make editable · Set field value · Clear field value · Set default value · Restrict choices</td></tr></tbody></table><p>A rule can carry effects from both groups at once - e.g. hide a field <i>and</i> clear its value when a condition stops matching.</p>`},
     {heading:'Priority & effective-date windows',bodyHtml:`<p>When two active rules disagree about the same target field, the higher-<b>priority</b> rule wins - a plain number you set per rule. An optional <b>effective start/end date</b> window lets a rule apply only during a specific period, without needing to remember to deactivate and reactivate it manually.</p>`},
     {heading:'Status Transitions: restricting stage changes',bodyHtml:`<p>A separate, narrower tool in the same Automation area: name which status/stage changes are allowed on an object, with a wildcard <b>"Any status"</b> starting point and a per-rule active toggle. No active rules leaves the field fully unrestricted; re-saving the same status is never blocked, so a transition rule can only narrow what's already possible, never trap an existing record.</p>`},
     {heading:'Test rule before activating',bodyHtml:`<p>A dry-run <b>Test rule</b> mode shows exactly what an active rule would do against hypothetical field values - which effects would fire, whether the save would be blocked - without touching real data, so you can verify a rule's logic before it affects anyone.</p>`},
    ]},
   {slug:'workflow-automation',title:'Workflow Automation: triggers & actions',
    summary:'7 trigger types, a growing library of actions from creating a task to running an AI Agent, and the same AND/OR condition engine Business Rules uses.',
    sections:[
     {heading:'Triggers — Admin → Automation → Workflow automation',bodyHtml:`<table><thead><tr><th>Trigger</th><th>Fires when</th></tr></thead><tbody><tr><td>Record created</td><td>A new record of this type is saved</td></tr><tr><td>Record updated</td><td>Any field on an existing record changes</td></tr><tr><td>Status/stage reaches</td><td>A specific status/stage value is set</td></tr><tr><td>Custom field changed</td><td>A specific field's value changes</td></tr><tr><td>Date reached</td><td>A date field reaches today (with an offset)</td></tr><tr><td>Due/overdue</td><td>A due-date field passes without completion</td></tr><tr><td>Recurring schedule</td><td>Every N days</td></tr></tbody></table><p>Date reached and Due/overdue both work on any Custom Object's own active date field, not only a fixed set of built-in ones - a Lease's Renewal Date or a Maintenance Plan's Next Visit Date qualifies exactly like an Invoice's due date does.</p>`},
     {heading:'Extra conditions',bodyHtml:`<p>Beyond the trigger itself, a workflow can carry the exact same AND/OR condition engine <a href="/help/business-rules">Business Rules</a> uses - including one level of nested OR-groups and, per condition, <b>"Read value from"</b> a relationship to match against a linked record's field instead of the triggering record's own (see Business Rules' own article for the exact eligibility rule) - so a trigger can be narrowed ("only when Status/stage reaches Closed Won <i>and</i> the deal size is over $10,000") without a separate mechanism to learn.</p>`},
     {heading:'Actions',bodyHtml:`<p>Create a task or a reminder (with a due-in/remind-in day offset and an assignee), update or clear a field on the triggering record (a fixed value or copied live from another field), set a field's default only if it's still empty, assign an owner, create a new record (optionally linked to the trigger record) or update a field on an already-linked one, send an in-app notification to the owner or all admins, or - the newest action - <b>Run AI agent</b>, firing an AI Agent or Pipeline run from an ordinary business event. See the Agentic AI Foundry category's <a href="/help/orchestration-pipelines">Orchestration</a> article for what that action actually triggers.</p>`},
     {heading:'Test workflow before activating',bodyHtml:`<p>The same dry-run philosophy as Business Rules: <b>Test workflow</b> shows what an active workflow would do against hypothetical values, without creating a real task, sending a real notification, or touching real data.</p>`},
     {heading:'The notification center',bodyHtml:`<p>An in-app notification a workflow sends lands in every recipient's own notification center, not just a database row nobody sees - the same place a task reminder toast surfaces from.</p>`},
    ]},
   {slug:'screen-app-builder',title:'Screen Builder: designing create/edit and detail layouts',
    summary:'Tabs of field sections, role-based assignment, and a draft-until-published model that also drives the read-only detail view.',
    sections:[
     {heading:'Tabs, sections & columns — Admin → Experience → Screen layouts',bodyHtml:`<p>A layout is a named set of <b>tabs</b>, each holding one or more field <b>sections</b>, each section laid out in 1-3 <b>columns</b> with any field able to span the full section width. Drag fields into a section from the available-fields list; anything no tab claims still shows in an always-visible spot rather than silently disappearing from the form.</p>`},
     {heading:'Assign by role, with a required Default',bodyHtml:`<p>A layout is assigned to one or more roles. Whichever roles a signed-in user has, the first published layout naming one of them wins; every object needs exactly one <b>Default</b> layout as the fallback for anyone whose roles no other published layout claims - the same resolution rule Dashboards uses below.</p>`},
     {heading:'Nothing changes until Publish',bodyHtml:`<p>Every edit here only ever touches the layout's <b>draft</b> - the live form users actually see keeps its last-published version until you click <b>Publish</b>. An "Unpublished changes" badge shows when the draft has diverged from what's live, and <b>Revert draft</b> discards unpublished edits back to the published version if you change your mind.</p>`},
     {heading:'Placing a related list on a tab',bodyHtml:`<p>A Custom Relationship's related-records list (see the Custom Relationships article) can be placed on a specific tab here instead of always appearing in its own fixed spot - so a Company's Vendors relationship, say, can live right next to the fields it's most relevant to.</p>`},
     {heading:'The same layout drives the detail view too',bodyHtml:`<p>A published layout isn't only the edit form - it also drives the record's read-only Overview/detail view, so a custom field you've placed on a tab is visible there too, not only while editing. <b>Preview</b> shows the draft rendered before you publish it, so you can check the result without affecting what anyone else sees yet.</p>`},
    ]},
   {slug:'apps-dashboards',title:'App Builder & Dashboards: publishing your own app',
    summary:'Group objects into a named app with its own access grants and sidebar switcher, and build role-assigned dashboards from KPI, chart and record-list widgets.',
    sections:[
     {heading:'App Builder — Admin → Apps',bodyHtml:`<p>An app is a named, iconed group of already-existing objects (built-in or Custom) plus an assigned Screen Builder layout scope and an optional dashboard - "Property Management," "Asset Tracking," whatever your organization actually runs. Every primitive an app assembles already exists elsewhere in Admin; App Builder's job is only to package and scope them, not to duplicate them.</p>`},
     {heading:'Draft, Publish, and access grants',bodyHtml:`<p>An app starts as a <b>Draft</b> and is invisible outside Admin until <b>Publish</b>. Once published, <b>Administrators always see every app</b>; everyone else needs an explicit grant - to a role or to one specific person - at <b>Viewer</b> or <b>Editor</b> level. With zero grants, a published app is genuinely invisible to everyone but Administrators, not just hidden from the sidebar.</p>`},
     {heading:'Editor is a real security boundary',bodyHtml:`<p>Viewer/Editor isn't just a UI hint: once an object is placed in a published app, every create/update/archive and status-lifecycle action on it requires at least Editor access to <i>some</i> app containing it, checked server-side on every command - not only by which button happens to be visible. The strongest grant across every matching app wins, and a grant to a specific person beats a grant to their role.</p>`},
     {heading:'Dashboards — Admin → Dashboards',bodyHtml:`<table><thead><tr><th>Widget</th><th>Shows</th></tr></thead><tbody><tr><td>KPI tile</td><td>A single number - the same KPI catalog the classic Dashboard KPI picker draws from</td></tr><tr><td>Chart</td><td>An existing saved Custom Report, rendered as a bar chart</td></tr><tr><td>Record list</td><td>An object's most recent (or soonest-due, for Tasks/Invoices) records - click one to jump straight to it</td></tr></tbody></table><p>Dashboards use the exact same named-layout / role-assignment / required-Default / draft-until-Publish model Screen Builder does - one mental model for both, not two to learn.</p>`},
     {heading:'App-scoped automation',bodyHtml:`<p>A Business Rule, Workflow, or Dashboard created while a specific app context is selected is tagged with the app that owns it, and both the Business Rules and Workflow Automation builders offer an App filter - so an installed or hand-built app's own automation stays visibly contained instead of mixing into one flat unscoped list as you build more apps.</p>`},
    ]},
   {slug:'workspace-essentials',title:'Workspace essentials: users, business profile, numbering, KPIs & the Admin Assistant',
    ...
    sections:[
     ...
     {heading:'The Admin Assistant — Admin → Admin Assistant',bodyHtml:`<p>A second, separate mode of the same chat component that powers the records-facing Assistant (see the Agentic AI Foundry category's first article for that side) - here scoped to the admin surface itself and restricted to Administrators. Ask it to build a business rule, a workflow, an integration connection, or anything else this Admin panel exposes, in plain English; it has its own tool set and system prompt server-side and can create these for real, the same as filling out the forms by hand, with its own separate conversation history from the records-mode assistant.</p>`},
    ]},
   ]},
 {key:'industry-data-model',label:'Industry Data Model',icon:'⬢',
  blurb:'Install a complete, ready-made industry app - or review one in plain language before you commit - built from a versioned package manifest that reuses your existing Companies and Contacts instead of creating a parallel database.',
  topics:[
   {slug:'install-an-industry-app',title:'Installing an industry app',
    summary:'Import a manifest or load a bundled starter, review it, install it, and see it in your Installed apps list - Admin → App Catalog.',
    sections:[
     {heading:'The flow: Import → Review → Install',bodyHtml:`<p><b>Admin → App Catalog</b> (under Apps) has three panels: <b>Import a package</b>, <b>Imported packages</b>, and <b>Installed apps</b>. Importing a package - pasting a manifest or clicking one of the 11 bundled starters - only adds it to this workspace's local catalog for review. <b>Nothing is created in your workspace until you click Install.</b></p>`},
     {heading:'The 11 bundled starters',bodyHtml:`<p>One click each loads a ready-to-review manifest for: Field Service, Property Management, Construction &amp; Contractors, Professional Services, Practice Administration, Recruitment &amp; Staffing, Real Estate Brokerage, Legal Practice, Nonprofit &amp; Association Management, Auto Repair &amp; Service Garage, and Policy Administration &amp; Claims Management. Loading a starter just fills the Import textarea with its JSON - you still choose whether to Import and then Install it. Field Service has since been deepened past its original scope (an Estimate a Work Order can point back to, a recurring Maintenance Plan) - both mirrored here alongside the original set.</p>`},
     {heading:'Lanesra Industry Foundation — shared plumbing, desktop only',bodyHtml:`<p>A twelfth package, different in kind from the eleven above: Party, Party Role, Party Relationship, Location, Asset, Agreement and more - shared cross-industry objects other packages can optionally declare a real, enforced dependency on and relate to, instead of each reinventing its own version. Property Management is the first package retrofitted onto it (v1.1.0): its Owner relationship now targets Foundation's Party instead of the built-in Company, plus four new objects (Owner Interest, Lease Amendment, Security Deposit, Violation / Notice). Foundation itself - and the parts of Property Management's retrofit that need it - is desktop edition only; this online demo has no dependency-checking of any kind for any package, so it isn't one of the starters above. Lease Amendment, Security Deposit and Violation are still mirrored here, since none of the three actually needs Foundation.</p>`},
     {heading:'Review before you commit',bodyHtml:`<p>Once imported, a package sits in the <b>Imported packages</b> table with its name, industry, version and minimum required Lanesra version. Click <b>Details</b> to open a full plain-language preview before installing - see <a href="/help/review-before-you-install">Reading a package's Details</a> for what that panel actually shows.</p>`},
     {heading:'Installing',bodyHtml:`<p>Click <b>Install</b> from either the table row or the open Details panel. A successful install moves the app into the <b>Installed apps</b> table with a <b>Status</b> badge, and - if the package defines one - a new sidebar entry via the same App Builder mechanism a hand-built app uses. See <a href="/help/after-install-what-changed">After install</a> for exactly what gets created and how to fit it to your team.</p>`},
     {heading:'Installing more than one package',bodyHtml:`<p>A package already marked <b>Installed</b> shows that badge instead of an Install button - re-installing the identical package is blocked, but nothing stops you from importing and installing several different industry packages side by side in the same workspace; each is tracked and can be deactivated independently.</p>`},
    ]},
   {slug:'the-package-manifest',title:'The package manifest',
    summary:'What a package actually contains - objects, fields, relationships, business rules, workflows, seed data and recommended permissions - all as one versioned JSON document.',
    sections:[
     {heading:'One JSON document, one workspace-shaped diff',bodyHtml:`<p>A package manifest is plain JSON: <code>format_version</code>, a globally-unique <code>package_id</code> (e.g. <code>lanesra.field_service</code>), a display <code>name</code>, an <code>industry</code> label, a semantic <code>version</code>, and a <code>min_lanesra_version</code> your workspace must meet. Everything else it defines is expressed in exactly the same shape the admin screens that create these things already use.</p>`},
     {heading:'What it can define',bodyHtml:`<table><thead><tr><th>Section</th><th>Same shape as</th></tr></thead><tbody><tr><td><code>objects</code></td><td>Custom Objects (key, singular/plural label, icon, ID prefix &amp; digit width)</td></tr><tr><td><code>fields</code></td><td>Custom Fields (entity, label, type, options, required)</td></tr><tr><td><code>relationships</code></td><td>Custom Relationships (source/target, cardinality, forward/reverse labels)</td></tr><tr><td><code>business_rules</code></td><td>Business Rules (conditions + actions)</td></tr><tr><td><code>workflows</code></td><td>Workflow Automation (trigger + conditions + actions)</td></tr><tr><td><code>seed_data</code></td><td>Starter records created at install time, if any</td></tr><tr><td><code>app</code></td><td>An App Builder definition - name, icon, description, and <b>recommended_permissions</b> (role → level pairs, informational only)</td></tr></tbody></table>`},
     {heading:'Why this matters',bodyHtml:`<p>Because a package is just data in the same shape the rest of the platform already understands, installing one doesn't create a parallel, special-cased "industry app" data model - the objects, fields and rules it creates show up in the exact same Admin screens (Custom Objects, Custom Fields, Business Rules...) as anything you'd build by hand, editable the same way afterward.</p>`},
    ]},
   {slug:'review-before-you-install',title:"Reading a package's Details",
    summary:'The four-part plain-language preview - what it builds, how it connects to your data, its automation, and fitting it to your organization - before you commit to installing.',
    sections:[
     {heading:'What this builds',bodyHtml:`<p>A full accounting of every object, field and relationship the package defines - the same object/field/relationship facts from the manifest, rendered as plain sentences instead of raw JSON.</p>`},
     {heading:'How it connects to your existing data',bodyHtml:`<p>Which of your built-in entities (Companies, Contacts, Opportunities...) the package's new objects link into, and how - so you can see up front that, say, a Property links to Company before you commit to installing it.</p>`},
     {heading:'Automation, in plain language',bodyHtml:`<p>Every business rule and workflow the package would create, described with the identical operator and action wording the real Business Rules and Workflow Automation builders use ("require Unit's Lease field", "when Lease's End Date is reached, create task..."), so what you read here reads identically once you open those screens after installing.</p>`},
     {heading:'Fitting this to your organization',bodyHtml:`<p>Practical, generated guidance: whether the package links into built-ins you should already have real records in before installing; whether it ships starter records or none; a reminder that object names, ID prefixes and picklist options are just this package's defaults - rename or adjust them from Custom Objects/Custom Fields after installing to match how your team actually talks about the work; how many recommended permission grants are listed for your review; and a plain statement that every rule/workflow only reacts to the record that changed - a check that spans multiple records (an overlap check, a running total) still needs a person to enforce for now.</p>`},
     {heading:'This preview costs nothing to open',bodyHtml:`<p>Details is generated entirely from the manifest you already imported - opening it doesn't touch your workspace, call a server, or commit to anything. Open it for every package you're considering, even ones you don't end up installing.</p>`},
    ]},
   {slug:'after-install-what-changed',title:'After install: what changed, and fitting it to your team',
    summary:'The per-install artifact breakdown, why recommended permissions are never auto-granted, and adjusting a package\'s defaults after installing.',
    sections:[
     {heading:'What this install created',bodyHtml:`<p>Click an installed app's name to expand its own detail panel: a count of every artifact it created, grouped by type - custom objects, custom fields, relationships, business rules, workflows, screen layouts, dashboards, reports, numbering overrides, seed records, and (if the package includes them) AI agents or skills.</p>`},
     {heading:'Recommended permissions are never auto-granted',bodyHtml:`<p>A package can list recommended role → access-level pairs, but installing <b>never changes anyone's access on its own</b> - these are shown purely for you to review and apply by hand from Users or the Apps admin screen, the same explicit-grant model every app on the platform already uses.</p>`},
     {heading:'It becomes a real, editable app',bodyHtml:`<p>If the package defines one, install also publishes a genuine App Builder app - its own icon, an entry in the sidebar App Switcher, the same Draft/Publish and Viewer/Editor access model any hand-built app uses (see the No-Code Platform category's <a href="/help/apps-dashboards">App Builder &amp; Dashboards</a> article). From here it's not a special "installed" mode - it's an ordinary app you can extend.</p>`},
     {heading:'Making it yours',bodyHtml:`<p>Object names, ID prefixes and picklist options are the package's own defaults, not permanent - rename a field's label, adjust an object's numbering, or add/remove select options from Custom Objects/Custom Fields exactly as you would for anything you built by hand. A rule or workflow the package created is a real Business Rule/Workflow row too - edit, deactivate, or delete it like any other.</p>`},
    ]},
 ...
 {key:'deployment-management',label:'Deployment Management',icon:'◈',
  blurb:'See everything installed, what it created and depends on, curate a named Solution of exactly what you want to ship, and register the Publishers who own each package namespace - the way a real software vendor packages and promotes an update.',
  topics:[
   {slug:'solution-packages-overview',title:"Solution Packages: what's installed, what it created",
    summary:'The Admin → Deployment Management landing tab - every installed app, its Publisher and component/dependency counts, plus your own hand-built Local Workspace.',
    sections:[
     {heading:'One table, two kinds of rows',bodyHtml:`<p><b>Admin → Deployment Management → Solution Packages</b> lists every installed app - name, <b>Publisher</b>, <b>Type</b> (always "Packaged"), installed <b>Version</b>, active/inactive <b>Status</b>, and a live count of the <b>Components</b> it created and the <b>Dependencies</b> it declares. Alongside those sits one synthetic row, <b>🧩 Local Workspace</b> ("local" publisher, type "Custom"), representing everything you've built by hand rather than installed - the Packaged/Custom split the whole Deployment Management area is organized around.</p>`},
     {heading:'Export your own customizations',bodyHtml:`<p>The Local Workspace row's <b>Export</b> button downloads everything the <code>local</code> publisher owns as one re-importable manifest file - the all-or-nothing counterpart to a <a href="/help/solutions-curate-and-promote">Solution</a>'s deliberately-curated subset. No fake package row is created for it; it's generated on demand from whatever Components currently belong to <code>local</code>.</p>`},
     {heading:'Where installs actually happen',bodyHtml:`<p>This tab is read-and-review only - installing, deactivating or reactivating an app itself happens from <b>Admin → App Catalog</b> (see the Industry Data Model Help category). Come here afterward to see the result: what it created, whether an update is available, and its place among everything else in the workspace.</p>`},
    ]},
   {slug:'releases-and-updates',title:'Releases and updating an installed package',
    summary:"Every imported version of a package, and update-with-diff's real per-field Added/Modified/Removed preview before you apply a newer one.",
    sections:[
     {heading:'Releases: every version you\'ve imported',bodyHtml:`<p>Click <b>Releases</b> on any installed app's row to see every version of that package_id you've ever imported, oldest first - each is simply an immutable snapshot already sitting in the workspace's own package history, not a separate feature to maintain.</p>`},
     {heading:'"Update available"',bodyHtml:`<p>Import a newer version of an already-installed package from <b>Admin → App Catalog</b> the same way you'd import any package, then return here - an <b>Update to v...</b> button appears automatically on that app's row, offering the newest imported version that isn't the one currently installed.</p>`},
     {heading:'Review before you apply',bodyHtml:`<p>Clicking it opens a real diff, not a guess: every object and field key gets an individual <b>Added</b>, <b>Modified</b> or <b>Removed</b> badge, computed by actually comparing the two manifests. Everything else the newer version adds - relationships, business rules, workflows, screen layouts, reports - shows as a summarized count ("3 new business rules") rather than a per-item breakdown, since those don't have the same stable per-key identity objects and fields do. No changes detected reads as exactly that: the versions are identical where it matters.</p>`},
     {heading:'Applying an update',bodyHtml:`<p><b>Apply update</b> runs inside the same validated, safety-backed, single-transaction machinery as an original install (see the Industry Data Model category's <a href="/help/deactivate-reactivate-safety">Deactivate, reactivate &amp; install safety</a> article) - this replaces what used to be a dead end where reinstalling an already-installed package_id was simply rejected outright.</p>`},
    ]},
   {slug:'solutions-curate-and-promote',title:'Solutions: curate exactly what ships',
    summary:'Name a versioned, deliberately-picked subset of components, export it, and import it into another workspace - build-in-test, promote-to-production.',
    sections:[
     {heading:'Not everything - exactly what you pick',bodyHtml:`<p><b>Admin → Deployment Management → Solutions → + New solution</b> asks only for a <b>Name</b> and an optional <b>Description</b>. Unlike Local Workspace's all-or-nothing Export, a Solution starts empty - you add exactly the objects, fields, rules, workflows and screens it needs from <b>Add a component</b> (filterable by type, publisher or source app), and remove any you change your mind about.</p>`},
     {heading:'Version it like a real release',bodyHtml:`<p>Open a Solution to edit its <b>Version</b> field directly. Bump it before each Export so repeated promotions of the same Solution become successive, listable releases - the identical Releases view any installed package already gets, reusing the same immutable-snapshot mechanism.</p>`},
     {heading:'Export, then import like any package',bodyHtml:`<p><b>Export</b> (disabled until the Solution has at least one component) downloads the same manifest-file format every package already uses. Promote it to another workspace through the ordinary <b>Admin → App Catalog</b> import flow there - a Solution is not a separate import mechanism, it's a normal package with a hand-curated component list. This is the whole "environment" story: two Lanesra OS workspaces already are two separate environments, so promoting a Solution to "prod" is just exporting from one and importing into the other.</p>`},
     {heading:'Deleting a Solution',bodyHtml:`<p>Delete removes the Solution definition itself - the curated list and its version history - without touching any of the actual components it referenced. Nothing you built or installed is at risk from deleting a Solution that merely pointed at it.</p>`},
    ]},
   {slug:'components-and-dependencies',title:'Components & Dependencies',
    summary:"Every custom object, field, relationship, rule, workflow, screen and report in the workspace and who owns it, plus every package dependency and whether it's satisfied.",
    sections:[
     {heading:'Components: everything, tagged with an owner',bodyHtml:`<p><b>Admin → Deployment Management → Components</b> lists every custom object, field, relationship, business rule, workflow, screen layout and report in the workspace - filterable by publisher, source app, type or id - whether an installed package created it or you built it by hand. A count-by-type summary sits at the top, and each row shows its <b>Publisher</b> (with a <b>Local</b> badge for hand-built ones) and its <b>Source</b> - the installed app that created it, or "Hand-built".</p>`},
     {heading:'Why this is one list',bodyHtml:`<p>This is the same underlying data both Solution Packages' component counts and a Solution's own "Add a component" picker draw from - one real inventory rather than several different views that could quietly drift apart.</p>`},
     {heading:'Dependencies: what a package needs',bodyHtml:`<p><b>Admin → Deployment Management → Dependencies</b> lists every dependency declared by any package you've imported - which package it depends on, the version constraint, whether it's Required or Optional, and a live <b>Satisfied</b>/<b>Unsatisfied</b> status computed against what's actually installed. Empty until a package that declares a dependency is imported.</p>`},
    ]},
   {slug:'publishers-and-namespaces',title:'Publishers: namespace ownership',
    ...
    sections:[
     {heading:'A package_id has an owner',bodyHtml:`<p>Every package's <code>package_id</code> is expected to be shaped <code>&lt;publisher-key&gt;.&lt;name&gt;</code> (the bundled reference packages all use <code>lanesra.*</code>). <b>Admin → Deployment Management → Publishers</b> is the registry of which keys are claimed - importing a package under a key nobody has registered is rejected outright, naming exactly what needs registering first rather than silently accepting a namespace collision.</p>`},
     {heading:'Registering one',bodyHtml:`<p><b>+ Register publisher</b> takes a <b>Key</b> (lowercased automatically, e.g. <code>acme</code>), a display <b>Name</b>, and an optional <b>Description</b>. Two publishers are seeded automatically in every workspace and can't be un-registered: <code>lanesra</code> (Official - owns every bundled reference package) and <code>local</code> (the implicit home for everything you build by hand) - so every existing install keeps working unchanged even in a workspace that's never registered a publisher of its own.</p>`},
     {heading:'What the table shows',bodyHtml:`<p>Each Publisher's row shows its key, name, description, and a live count of how many imported packages currently belong to it, plus <b>Official</b>/<b>Local</b> badges for the two seeded ones. Registering a publisher only unblocks importing packages under its key - it doesn't retroactively reassign anything you've already built to it (a named next step, not silently missing).</p>`},
    ]},
   ]},
 {key:'integration-hub',label:'Integration Hub',icon:'⬡',
  blurb:'Connect encrypted external systems, turn an OpenAPI spec into callable Workflow actions, expose your own REST API and outbound webhooks, move data with CSV or a recurring pull sync, and watch it all through one unified log.',
  topics:[
   {slug:'connections-and-overview',title:'Connections & the Integration Hub Overview',
    summary:'Encrypted connections to six kinds of external systems, Connection References for portable Solutions, and the health KPIs that greet you first.',
    sections:[
     {heading:'Overview: six numbers, one glance',bodyHtml:`<p><b>Admin → Integration Hub</b> opens on <b>Overview</b> - live KPI tiles for Active connections, Failed connections, API calls today, Failed webhooks today, Jobs running, and Jobs failed today, so a problem anywhere in the Hub is visible from the very first screen, before you go looking for it.</p>`},
     {heading:'Connections: six types, encrypted at rest',bodyHtml:`<p><b>Connections</b> holds generic REST, webhook, SFTP, PostgreSQL, OData and SMTP connections, each with a <b>Name</b>, <b>Type</b>, <b>Base URL</b>, and one of 7 auth modes - none, API key, basic, bearer, custom header, OAuth2 client-credentials, or OAuth2 authorization-code. Any secret is encrypted at rest (AES-256-GCM) and never returned in plaintext once saved - the row shows only whether one is set. <b>Test</b> makes one real call and records the result (latency on success, the actual error on failure) directly on the connection.</p>`},
     {heading:'Connection References: portable by design',bodyHtml:`<p>A Connection itself is workspace-specific - it holds a real base URL and a real secret, so it can never travel inside an exported Solution or package. A <b>Connection Reference</b> is the portable half: a logical name (a <b>Key</b> like <code>crm_primary</code>) and an expected type that a Solution or Workflow Automation binds to instead of a physical Connection. Export a Solution that uses one, and only the reference travels; the receiving workspace binds it to its own real Connection afterward from this same tab.</p>`},
    ]},
   {slug:'connectors-openapi-import',title:'Connectors: turn an OpenAPI spec into Workflow actions',
    summary:'Import an OpenAPI 3.x spec, pick which operations you need, and call any of them as a new "Call Connector Action" Workflow Automation step.',
    sections:[
     {heading:'Parse, then choose',bodyHtml:`<p><b>+ Import connector</b> takes a pasted OpenAPI 3.x spec (JSON or YAML) and <b>Parse spec</b> returns every operation it found, with any parsing warnings shown up front. Nothing is imported yet at this point - you're just previewing what's available.</p>`},
     {heading:'Select exactly what you need',bodyHtml:`<p>Every parsed operation starts checked; uncheck any you don't want. Name the connector, give it an optional description, and <b>Import N actions</b> creates one Connector with only the operations you selected - not the whole API surface whether you use it or not.</p>`},
     {heading:'A new Workflow Automation action',bodyHtml:`<p>Each imported action - its HTTP method and path template - becomes callable from Workflow Automation as <b>Call Connector Action</b>, the same way any other workflow action fires from a trigger. <b>Test</b> next to any action calls it for real right from this tab, before you ever wire it into a workflow.</p>`},
    ]},
   ...
   {slug:'data-exchange-and-external-objects',title:'Data Exchange & External Objects',
    summary:'CSV import/export and reusable Mappings through the same validated write path the REST API uses, plus read-only live records from an external system.',
    sections:[
     {heading:'CSV Import: the same path as everything else',bodyHtml:`<p>Pick a <b>Target object</b> (any built-in or active Custom Object), an <b>Operation</b> (insert, update or upsert - update/upsert need a <b>Match key</b>, the target field that identifies an existing record), and a <b>Duplicate policy</b> (skip, update matched, or create new). Paste CSV text, <b>Auto-map from header row</b> to seed the field mapping from column names, then adjust each mapping's <b>transform</b> (trim, uppercase, lowercase, concatenate, numeric, date, or none) by hand. <b>Preview (dry run)</b> runs the exact same import without writing anything; <b>Import</b> commits it - both go through the identical generic record-write path the REST API itself uses, so the same validation, Business Rules and permissions apply. Results show total/succeeded/failed/skipped-duplicate counts, with per-row errors for anything that failed.</p>`},
     {heading:'CSV Export & reusable Mappings',bodyHtml:`<p>CSV Export downloads any object's records as a file. <b>Mappings</b> saves a named field-mapping configuration so a recurring import from the same source system doesn't need re-mapping by hand every time.</p>`},
     {heading:'External Objects: read-only, live',bodyHtml:`<p><b>+ New external object</b> defines a live, read-only window into another system through a Connection: an <b>Object key</b>, <b>Display name</b>, the Connection to use, and a <b>Resource path</b> (e.g. <code>/widgets</code>). <b>Preview</b> fetches real records on demand and shows the raw JSON - nothing is copied into your workspace by defining one; it's a live lookup, and it's also what an <a href="/help/integration-jobs">Integration Job</a> pulls from on a schedule.</p>`},
    ]},
   {slug:'integration-jobs',title:'Integration Jobs: recurring pull sync',
    ...
    sections:[
     {heading:'What a Job actually does',bodyHtml:`<p><b>+ New job</b> pairs an External Object with a target Lanesra object and an interval in minutes. Each run tracks a checkpoint cursor, so a Job resumes from where it left off rather than re-pulling everything every time. This is pull-only - syncing a change back out to the external system isn't built; use a Webhook or Connector Action for the outbound direction instead.</p>`},
     {heading:'Where the schedule actually runs',bodyHtml:`<p>The background scheduler that fires Jobs automatically only runs where a Team Workspace server is hosting this workspace. A pure desktop install has no long-running process to host it, so <b>Run Now</b> is the only way a desktop-hosted Job ever executes - a real, named limit, not a hidden one.</p>`},
     {heading:'Watching runs',bodyHtml:`<p>Each Job's row shows its target, interval, status and last run outcome at a glance; <b>Runs</b> opens the full history for that Job specifically, separate from the Hub-wide log covered next.</p>`},
    ]},
   {slug:'logs-monitoring-settings',title:'Logs, Monitoring & Settings',
    ...
    sections:[
     {heading:'One log, every execution type',bodyHtml:`<p><b>Logs &amp; Monitoring</b> is a single execution log spanning API calls, webhook deliveries, and import/export runs - each row shows its type, direction, status, a records breakdown (Read/Written/Skipped/Failed), duration and timestamp. Filter by execution type, status, or a specific correlation id to trace one request's actual effects across every system it touched.</p>`},
     {heading:'Purging old logs',bodyHtml:`<p><b>Purge expired logs</b> removes anything past the retention window set in Settings on demand, reporting exactly how many rows it removed - you don't have to wait for a scheduled sweep to reclaim the space.</p>`},
     {heading:'Settings: rate limits & retention',bodyHtml:`<p>Workspace-wide controls: a per-client and a global <b>API rate limit</b> (requests per minute), <b>Log retention</b> and <b>File retention</b> in days, and an <b>Allow insecure (non-TLS) outbound connections</b> toggle for the rare case a Connection genuinely needs to reach a plain-HTTP endpoint. These apply across every tab in this category, not per-connection.</p>`},
    ]},
   ]},
  {key:'architecture',label:'Architecture & Extensibility',icon:'🏛',
   blurb:'How Lanesra OS runs as three different things from one shared core - a desktop app, a self-hosted Team Workspace, and a planned paid cloud SaaS. Written for anyone evaluating, deploying or extending the platform, not day-to-day feature use.',
   topics:[
    {slug:'multi-target-architecture',title:'Multi-target architecture: Desktop, Team Workspace & Cloud SaaS',
     summary:'One shared Rust core, three deployment shells - why storage, tenancy and hosting differ per target while the business logic never forks.',
     sections:[
      {heading:'One core, three shells',bodyHtml:`<p>The business logic - what a Business Rule does, how an AI Agent runs, what a Solution export contains - never forks across targets. Every difference between <b>Desktop</b> (Tauri, one local user, offline-first), <b>Team Workspace</b> (self-hosted, one organization's own server) and <b>Cloud SaaS</b> (Lanesra-operated, paid, many tenants) lives entirely in the thin shell wrapping the core: which storage engine it talks to, how a session is authenticated, how the process is deployed and operated.</p><p class="help-note">Already true today, not aspirational: Desktop and Team Workspace already share the same core without a fork. A SaaS shell extends the same pattern rather than inventing a new one.</p><p>The corollary: no service function in the core branches on "is this SaaS" or equivalent. Tenant isolation, billing gates and deployment-specific auth all belong in the layer wrapping the core for that target - never inside it.</p>`},
      {heading:'Storage matched to the real need',bodyHtml:`<p>Desktop stays on <b>SQLite</b> - a single local user has no concurrent-write problem to solve, and forcing a database server onto one person working offline would be pure regression. Team Workspace and Cloud SaaS both run <b>PostgreSQL</b>, since both need real multi-user write concurrency that SQLite's single-writer lock can't provide. Only two engines to ever support, not three, each matched to what actually needs it - a storage boundary in the core makes the engine a configuration choice at the shell level, not a fork of the business logic.</p>`},
      {heading:'Cloud SaaS multi-tenancy: one database per tenant',bodyHtml:`<div class="callout callout-info"><p>A tenant on the (planned) SaaS tier gets its own Postgres database, provisioned on signup - not a shared database with every tenant's rows filtered by <code>workspace_id</code> at query time. Two tenants' data is never in the same database to begin with, so a query-filtering bug can never leak one tenant's records into another's - the strongest tenant-isolation guarantee available.</p></div><p>This is also the cheapest path architecturally: the schema already scopes nearly every table by <code>workspace_id</code>, but the server already assumes exactly <em>one</em> workspace per connection. Database-per-tenant keeps that assumption completely unchanged; a shared-schema approach would mean rewriting it everywhere a query runs.</p>`},
      {heading:'Billing, tenant provisioning & feature parity stay outside the core',bodyHtml:`<p>Subscription state, payment processing, and tenant provisioning/de-provisioning live entirely in the SaaS control plane - a layer that operates many instances of the same open-source core from the outside, never visible to or reachable from Desktop or Team Workspace.</p><div class="callout callout-warning"><p><b>Never:</b> a license check, a feature flag tied to payment, or a phone-home call inside the shared core. Self-hosted stays self-hosted, with no exceptions for convenience.</p></div><p>What's open-source everywhere versus SaaS-only is a deliberate, written decision, not whatever turned out easiest to gate. The default assumption is full parity - the CRM, the no-code platform and the AI Agent Foundry all work identically on every target. SaaS-exclusive items are operational conveniences a paying customer is buying relief from running themselves (managed backups, SSO, a cross-workspace console), never core capability withheld from the self-hosted tiers.</p>`},
      {heading:'Postgres, two ways',bodyHtml:`<p>Team Workspace and Cloud SaaS both speak Postgres, but don't run it the same way. <b>Team Workspace</b> is one customer's own vanilla, self-hosted Postgres, bundled directly into Lanesra's own setup process - no separate database install step for the operator to perform by hand. <b>Cloud SaaS</b> is Lanesra operating potentially thousands of small, mostly-idle tenant databases, provisioned the instant someone signs up - a genuinely different operational shape, better served by a serverless-native Postgres platform that provisions a new tenant database in seconds and scales an idle one to zero, rather than vanilla self-managed instances.</p><p>This costs nothing at the core - it's still just a Postgres connection string either way - and it doesn't weaken the open-source commitment: that commitment covers what a self-hosting customer runs (Team Workspace), never Lanesra's own infrastructure choice for operating its own paid service.</p>`},
      {heading:'Moving between targets, and testing discipline',bodyHtml:`<p>Growing from a laptop into a self-hosted team, or later into a paid plan, rides the same whole-workspace <a href="/help/deactivate-reactivate-safety">backup/restore</a> mechanism and <a href="/help/solutions-curate-and-promote">Solution export/import</a> pipeline already built for data safety and packaging customizations - not a bespoke migration project invented per pair of targets.</p><p>Since Team Workspace and SaaS both run on Postgres while Desktop stays on SQLite, the same migration and service test suite runs against both engines in CI - a core change is only done once both pass, not whichever engine its author happened to have running locally. And since SaaS is the first target where one tenant reading another's data is even possible, it gets its own explicit discipline beyond that: an automated suite that actively attempts cross-tenant access on every release and asserts it always fails, rather than a property assumed from the schema looking about right.</p>`},
     ]},
   ]},
  {key:'organization-access-control',label:'Organization & Access Control',icon:'🔐',
   blurb:'Who reports to whom, who owns what, and who can act on it. Organizations, an Organization Unit hierarchy, Work Teams, a structured Owner + Owning Organization Unit on every record, and capability-scoped Access Roles with an Access Inspector - Phases 1 & 2 of a six-phase Access Foundation.',
   topics:[
    {slug:'organization-and-org-units',title:'Organizations & Organization Units',
     summary:'One Organization per workspace, an Organization Unit hierarchy you can create, edit and move - with an impact preview before any move actually commits.',
     sections:[
      {heading:'One Organization per workspace',bodyHtml:`<p>Every workspace has exactly one <b>Organization</b> - a name, legal name, code, status, default currency, locale and timezone - bootstrapped automatically the first time any Organization & Access Control screen is opened, so there's no setup step to remember before the rest of this category works. Edit it from <b>Admin → Organization</b>.</p>`},
      {heading:'Organization Units: a real hierarchy',bodyHtml:`<p><b>Admin → Organization Units</b> shows a tree - regions, divisions, teams, however your organization is actually structured - each unit with a name, a type, a manager, an effective-date window, and a parent unit. A workspace always has one root unit with no parent; every other unit sits somewhere beneath it. <b>+ New unit</b> creates a child of whichever unit you're viewing; <b>Edit</b> changes a unit's own fields without touching its position in the tree.</p>`},
      {heading:'Moving a unit: impact preview before you commit',bodyHtml:`<p><b>Move</b> on any non-root unit lets you pick a new parent. Before anything is written, a preview shows exactly what the move affects: how many descendant Organization Units come with it, and how many owned records (across every object type) sit somewhere in that subtree. Only after reviewing that preview do you confirm the move - it's never a silent, unreviewable reparenting.</p><p class="help-note">Internally, each unit stores its own path (e.g. <code>/root-id/child-id/</code>) and depth, so a move recomputes just that subtree's paths in one pass rather than touching the whole hierarchy or needing a second structure kept in sync alongside it.</p>`},
      {heading:'Deleting a unit',bodyHtml:`<p>A unit can't be deleted while it still has child units or owned records pointing at it - deactivate it instead (reversible) if you need it out of active use without breaking what still references it. This mirrors the same "block delete while dependents exist, allow deactivate/reactivate freely" pattern the rest of the admin surface already follows for Custom Objects, Business Rules and Workflow Automation.</p>`},
      {heading:'Next: teams and who owns what',bodyHtml:`<p>Continue to <a href="/help/work-teams-and-ownership">Work Teams &amp; Record Ownership</a> for how a person or a team actually ends up as a record's Owner.</p>`},
     ]},
    {slug:'work-teams-and-ownership',title:'Work Teams & Record Ownership',
     summary:'Time-bounded team memberships, and the Owner + Owning Organization Unit fields now on every record - changed individually or via bulk reassignment.',
     sections:[
      {heading:'Work Teams & memberships',bodyHtml:`<p><b>Admin → Work Teams</b> defines named teams - a type, a primary Organization Unit, an optional owning user, and whether the team itself <code>can_own_records</code> (most can). <b>Members</b> opens that team's membership list: add a user with a role-in-team and an optional effective-date window, or end an existing membership.</p><div class="callout callout-info"><p>Ending a membership never touches any record the departing member owns. Team membership and record ownership are deliberately independent - removing someone from a team is not a bulk-reassignment action, and never happens as a side effect of one.</p></div>`},
      {heading:'Record ownership: Owner + Owning Organization Unit',bodyHtml:`<p>Every Company, Contact, Opportunity, Product, Quote, Order, Invoice, Contract, Task and Custom Object record now carries two fields beyond its usual ones: an <b>Owner</b> (a single user, or a whole team - not free text) and an <b>Owning Organization Unit</b>. Both default automatically on create - Owner to whoever created the record, Owning Organization Unit to their own primary unit - so nothing is ever created ownerless.</p>`},
      {heading:'Changing an owner - one record or many',bodyHtml:`<p>Change a record's Owner or Owning Organization Unit right on its own edit form, the same as any other field. To reassign many records at once, select them on a list screen and use the bulk <b>Change Owner</b> action - pick the new Owner (user or team) and, optionally, a new Owning Organization Unit; anything left blank stays unchanged on every selected record. A <b>dry run</b> flags any selected record that isn't eligible before anything actually writes.</p>`},
      {heading:'Ownership history & the audit trail',bodyHtml:`<p>Each record carries its own ownership-version counter and an <code>assigned_at</code> timestamp, advancing only on a genuine reassignment - saving a record without actually changing its Owner never bumps either. Every real transfer also writes a row to the same audit trail every other create/update/archive already appears in, from the record's own <b>Activity</b> view.</p>`},
     ]},
    {slug:'ownership-modes',title:'Ownership Modes: how each object type is owned',
     summary:'USER_TEAM_OWNED, ORG_OWNED, PARENT_CONTROLLED and SYSTEM_OWNED - how a mode is decided for built-in objects versus Custom Objects.',
     sections:[
      {heading:'Four modes',bodyHtml:`<table><thead><tr><th>Mode</th><th>Means</th></tr></thead><tbody><tr><td><code>USER_TEAM_OWNED</code></td><td>The normal case - a specific user or team owns the record, changeable individually or in bulk. Every built-in object (Companies, Contacts, Opportunities, Products, Quotes, Orders, Invoices, Contracts, Tasks) uses this mode, and it's the default for a new Custom Object.</td></tr><tr><td><code>ORG_OWNED</code></td><td>The record belongs to the Organization Unit itself rather than to a specific person or team within it.</td></tr><tr><td><code>PARENT_CONTROLLED</code></td><td>A detail/child record inherits its owner from whatever parent record it belongs to, rather than carrying an independent one of its own. Defined in the data model; the runtime inheritance behavior is intentionally not built yet, since this codebase has no generic parent/child record concept to hang it off of beyond Custom Objects themselves - see What's next, below.</td></tr><tr><td><code>SYSTEM_OWNED</code></td><td>No individual owner applies - reserved for record types that are workspace infrastructure rather than business data a person is accountable for.</td></tr></tbody></table>`},
      {heading:'Built-in objects: fixed, not configurable',bodyHtml:`<p>The nine built-in object types are all <code>USER_TEAM_OWNED</code> and this isn't a per-workspace setting - it's a fixed fact about what each of those objects is, the same way their field list is fixed. This keeps ownership behavior predictable across every workspace rather than one more thing an admin has to get right.</p>`},
      {heading:'Custom Objects: a real per-object setting',bodyHtml:`<p>Every Custom Object has its own <code>ownershipMode</code>, defaulting to <code>USER_TEAM_OWNED</code> at creation - unlike the fixed built-ins, this is a genuine per-definition setting an admin could change, since a Custom Object modeling something like a shared reference table might legitimately want a different mode.</p>`},
     ]},
    {slug:'access-roles-and-record-scopes',title:'Access Roles & Record Scopes',
     summary:'Phase 2: named, composable Access Roles carrying a capability set and a Record Scope, replacing the Phase 1 Administrator-only placeholder for good.',
     sections:[
      {heading:'What an Access Role is',bodyHtml:`<p>An <b>Access Role</b> (<b>Admin → Access Roles</b>) is a named, reusable bundle of grants. Each grant targets one object type - a built-in object like Company, or a Custom Object - and carries five capability flags (<b>Create, Read, Update, Delete, Assign</b>) plus a <b>Record Scope</b> saying how far that capability reaches. A role can hold grants for several object types, and one special <code>*</code> ("default") grant applies to any object type the role doesn't otherwise mention - including a Custom Object created after the role was defined, with no re-editing needed.</p>`},
      {heading:'The four Record Scopes',bodyHtml:`<table><thead><tr><th>Scope</th><th>Covers</th></tr></thead><tbody><tr><td><b>Owner</b></td><td>Only records this specific user owns outright.</td></tr><tr><td><b>Team</b></td><td>Owner, plus records owned by a Work Team the user actively belongs to, or owned by another user who shares an active team membership with them.</td></tr><tr><td><b>Org Unit and below</b></td><td>Team, plus records whose Owning Organization Unit sits at or below the user's own primary Organization Unit in the hierarchy.</td></tr><tr><td><b>Organization</b></td><td>Every record in the workspace, regardless of owner or unit.</td></tr></tbody></table><p class="help-note">Each level is a strict superset of the one before it - granting Org Unit and below automatically covers Team and Owner too, so a role only ever needs to name the broadest level it should reach.</p>`},
      {heading:'Two system roles, seeded automatically',bodyHtml:`<p>The first time any Access Control check runs in a workspace, two roles are bootstrapped for you - neither needs manual setup. <b>Full Access</b> (every capability, Organization scope, every object) is assigned to everyone already holding the legacy Administrator role. <b>Standard User</b> is assigned to everyone else, and every newly created user going forward. Both are marked system roles and can't be deleted.</p><div class="callout callout-info"><p>Standard User's default grant is deliberately Create/Read/Update/Delete at <b>Organization</b> scope, not Owner - matching the fully open CRUD behavior this workspace already had before Access Roles existed. Only <b>Assign</b> (reassigning a record's owner) is withheld by default; that was the one capability actually restricted before this phase shipped.</p></div>`},
      {heading:'Custom roles: a real, narrower grant',bodyHtml:`<p>Create a custom role from <b>Admin → Access Roles → New role</b>, add one or more object-type grants with the capabilities and scope you intend, and assign it to specific users from the role's <b>Members</b> panel. A user can hold several roles at once - when roles disagree, the <b>broadest matching scope wins</b> for each capability, so a narrower custom role layered on top of Standard User's own Organization-scope default has no restrictive effect by itself. To genuinely narrow a user below Standard User's default, remove them from Standard User first.</p>`},
      {heading:'Where this is actually enforced',bodyHtml:`<p>Reassigning a record's owner (<b>Admin → Access Roles</b>' original seam) always requires an explicit Assign grant covering that record. Beyond that, Create/Read/Update/Delete is checked on Companies, Contacts, Opportunities, Products, Quotes, Orders, Invoices, Contracts, Tasks and Custom Object records - a create, an update, an archive, opening one specific record, and now also list screens, global search, dashboard record-list widgets and custom report counts/sums, all narrowed to the same Read scope before any truncation or aggregation happens. Related-record lists and aggregate KPI dashboard tiles are the one remaining gap - a deliberate, bounded scope decision, not an oversight.</p>`},
      {heading:'The dozen legacy admin checks: additive, not replaced',bodyHtml:`<p>Organization Units, Custom Objects, Workflow Automation, Business Rules, Custom Fields, Custom Reports, Status Transitions, Numbering, the Workspace profile, Backup/Restore, Work Teams, Relationships and App Builder each already had their own "Only an Administrator can..." check before this phase. Every one of them still works <i>exactly</i> as before - the legacy Administrator role always passes, unconditionally. A non-Administrator can now <i>additionally</i> act on one of these screens only with an explicit Update grant naming that exact object type (never through a role's <code>*</code> default, which would otherwise leak ordinary CRUD access into schema/automation/org-structure screens it was never meant to reach). Nothing changes for a workspace that hasn't deliberately granted one of these.</p>`},
     ]},
    {slug:'access-inspector',title:'The Access Inspector',
     summary:'Answer "why can or can\'t this user do that" from the exact evaluation trace enforcement itself uses - not a guess at what the rules probably mean.',
     sections:[
      {heading:'What it answers',bodyHtml:`<p><b>Admin → Access Roles → Inspector</b> takes a user, an object type, a capability, and optionally one specific record, and returns a real decision: allowed or denied, which of the user's roles produced the winning grant, what Record Scope that grant carries, and - for a specific record - whether that scope actually covers it.</p>`},
      {heading:'Why it\'s trustworthy',bodyHtml:`<p>The Inspector isn't a second, parallel explanation system that could drift out of sync with what's actually enforced - it calls the identical evaluator <code>require_capability</code> itself calls before allowing or blocking an action. The trace it shows you is the same computation that produced the real outcome, not a best-effort reconstruction of what the rules probably meant.</p>`},
      {heading:'Reading a trace',bodyHtml:`<p>Every Access Role the user holds is listed, whether it contributed to the final answer or not - including the ones that matched nothing, or matched but lost to a broader grant from another role. The one that won is called out explicitly, alongside a plain-language reason ("'Standard User' grants Update on 'Company' (Organization scope), which covers this record" / "The broadest grant found doesn't cover this specific record").</p>`},
      {heading:'Use it before granting, not just after a complaint',bodyHtml:`<p>Before creating a narrower custom role, run a quick Inspector check against a representative record to confirm it produces the scope you expect - multi-role folding always takes the broadest matching grant across every role a user holds, which can surprise you if they're still holding Standard User's own Organization-scope default alongside a new, narrower role.</p>`},
     ]},
    {slug:'access-foundation-whats-next',title:"What's next: Field Access Policies, AI Access & Enterprise Identity",
     summary:'Organization/Org Units/Work Teams/ownership and Access Roles/Record Scopes/the Access Inspector are Phases 1 & 2 of six. Phases 3-6 - Field Access Policies, Record Access Policies, AI Action Levels and SSO - are roadmapped, not built yet.',
     sections:[
      {heading:'Where this category stands',bodyHtml:`<p>What's documented above - Organizations, Organization Units, Work Teams, record ownership, and now Access Roles, Record Scopes and the Access Inspector - covers <b>Phases 1 and 2</b> of a six-phase Enterprise Requirements Specification, matching exactly what each phase of work set out to build rather than attempting all six phases in one pass.</p>`},
      {heading:'What Phase 2 deliberately still leaves open',bodyHtml:`<p>List views, global search, dashboard record-list widgets and custom report counts/sums are now narrowed to each viewer's Read scope too (see <a href="/help/access-roles-and-record-scopes">Access Roles &amp; Record Scopes</a>); related-record lists and aggregate KPI dashboard tiles remain unfiltered by design, the one deliberately bounded remainder. Field-level read/write policies, reusable capability bundles, cross-cutting Access Groups and one-off time-bounded Record Grants are all real gaps the next phase closes, not this one.</p>`},
      {heading:'Phases 3 through 6',bodyHtml:`<ul><li><b>v1.1</b> - Field Access Policies, reusable Capability Packs/Bundles, Access Groups, and Record Grants for one-off, time-bounded exceptions.</li><li><b>v1.2</b> - declarative Record Access Policies, and a stronger dedicated audit view over bulk reassignment specifically.</li><li><b>AI Access v1</b> - graded AI Action Levels per tool call, and a real Agent/Integration Identity distinct from the human who configured it.</li><li><b>Enterprise Identity v2</b> - OIDC/SAML single sign-on, SCIM-provisioned user lifecycle, and a first-class External User concept.</li></ul>`},
      {heading:'Track it',bodyHtml:`<p>Each phase has its own GitHub issue, and the <a href="/roadmap">Roadmap</a> page tracks them under Planned/Proposed alongside everything else being considered next.</p>`},
     ]},
   ]},
];
...
function helpHubPage(){
 ...
 input.addEventListener('input',()=>{
  ...
  document.getElementById('help-empty').hidden=anyVisible||!q;
 });
}
function helpTopicPage(slug){
...
else if(path==='/demo')appShell();else if(path==='/roadmap')roadmapPage();else if(path==='/releases')releasesPage();else if(path==='/platform'||path==='/build')platformPage();else if(path==='/compare')comparePage();else if(path==='/download')downloadPage();else if(path==='/help')helpHubPage();else if(path.startsWith('/help/'))helpTopicPage(path.slice(6));else landing();
