...
// customizations - not yet wired to Components-tab attribution here
// either, same gap the desktop edition's own doc comments call out).
...
function installReferencePackage(key){
 ...
 data.apps.push(app);
 data.installedApps.push({id:uid(),key,packageId:pkg.packageId,name:pkg.name,industry:pkg.industry,version:pkg.version,status:'active',appId:app.id,objectKeys:app.objectKeys,artifacts,installedAt:new Date().toISOString()});
...
const ARTIFACT_TYPE_LABELS={customObject:'custom object',customField:'custom field',relationship:'relationship',businessRule:'business rule',workflow:'workflow'};
...
function solutionsTab(body){
 const subTabs=[['packages','Solution Packages'],['components','Components'],['dependencies','Dependencies'],['publishers','Publishers']];
 body.innerHTML=`<div class="panel">
 <h3 style="margin-top:0">Solution Management</h3>
 <p class="muted" style="font-size:13px">Every industry app installed in this workspace, what it created, and who published it - read-only. Install, deactivate or reactivate an app from <b>Admin → App Catalog</b>; this is where you see the result.</p>
...
function renderSolutionsSubTab(){
 ...
 ({packages:solutionPackagesSubTab,components:solutionComponentsSubTab,dependencies:solutionDependenciesSubTab,publishers:solutionPublishersSubTab}[solutionsSubTab])(body);
}
function solutionPackagesSubTab(body){
 const installed=data.installedApps||[];
 body.innerHTML=`<div style="margin-top:16px">${installed.length?`<div class="table-wrap"><table class="table"><thead><tr><th>Name</th><th>Publisher</th><th>Type</th><th>Version</th><th>Status</th><th>Components</th><th>Dependencies</th></tr></thead><tbody>${installed.map(a=>{const publisher=publisherForPackageId(a.packageId);return `<tr><td>${REFERENCE_PACKAGES[a.key]?.appIcon||''} ${a.name}</td><td>${publisher?publisher.name:'—'}</td><td><span class="badge">Managed</span></td><td>${a.version}</td><td>${badgeMaybe(a.status==='active'?'Active':'Inactive')}</td><td>${(a.artifacts||[]).length}</td><td>0</td></tr>`}).join('')}</tbody></table></div>`:'<div class="empty">Nothing installed yet. Install a reference package from <b>Admin → App Catalog</b> to see it here.</div>'}</div>`;
}
function solutionComponentsSubTab(body){
 const all=(data.installedApps||[]).flatMap(a=>(a.artifacts||[]).map(art=>({...art,appName:a.name,version:a.version})));
 const byType={};
 all.forEach(a=>{byType[a.type]=(byType[a.type]||0)+1});
 body.innerHTML=`<div style="margin-top:16px">
 <p class="muted" style="font-size:13px">Every record any installed app created, across the whole workspace - what have you actually got beyond what you installed.</p>
 ...
 <input id="solutionsComponentFilter" placeholder="Filter by app, type or id..." style="margin-bottom:8px;width:100%;max-width:320px">
 <div class="table-wrap"><table class="table"><thead><tr><th>Type</th><th>Installed app</th><th>Created by version</th></tr></thead><tbody id="solutionsComponentRows">${all.map(a=>`<tr data-needle="${(a.appName+' '+artifactTypeLabel(a.type)+' '+a.id).toLowerCase()}"><td>${artifactTypeLabel(a.type)}</td><td>${a.appName}</td><td>${a.version}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">Nothing installed yet.</div>'}
 </div>`;
...
function roadmapPage(){
 ...
 const proposed=[
  ['Managed/Unmanaged packages, the 5-layer extension model, update-with-diff & Releases','The remaining, larger half of the Solution Packages design spec, now that the Admin IA reshuffle, read-only Solution visibility (Packages/Components/Dependencies) and a real Publisher registry with enforced namespace validation have shipped (see Shipped, below): a Managed/Unmanaged distinction on packages (today every install is implicitly managed — there\'s no way yet for an organization to package its own customizations for redistribution), a 5-layer extensibility model so a managed component can be safely extended without editing it destructively, update-with-diff (today reinstalling an already-installed package is rejected outright — no upgrade path), and immutable Releases with export/import as a portable <code>.lanesra</code> archive.','The full 37-section design spec covers this scope; it needs its own dedicated design pass before it\'s buildable — component-tagging (attributing hand-built objects to the <code>local</code> publisher) in particular has no obvious UX yet on either platform. Needs the user to scope it before work resumes.','App Catalog','L'],
 ...
 $('#app').innerHTML=`${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Built from the working codebase</div><h1>Roadmap & backlog.</h1><p>What's being built next, what's still waiting on a decision, and everything already shipped — compiled from the actual code (core/server/src-tauri/frontend) plus the online demo, not a wishlist. Every "Shipped" entry below is running code with tests.</p><div class="status-row"><span class="status-chip">Early Access v0.32.0</span><span class="muted">Last updated August 2026</span></div>
 <div class="backlog-callout" id="desktop"><h3>Release status</h3><p><b>desktop-v0.11.0 is the latest tagged release</b> (installers attached, Early Access/prerelease as intended). Everything below is merged to <code>main</code>. Full desktop feature list → <a href="/download">/download</a>.</p><p class="muted">Repo hygiene: a real MIT <code>LICENSE</code>, <code>CONTRIBUTING.md</code>, <code>CODE_OF_CONDUCT.md</code>, <code>SECURITY.md</code>, issue/PR templates, and a root README written for someone landing on the repo, not a deploy runbook.</p></div>
 </div></section>
 <section class="section"><div class="container narrow">

 <div class="rm-stats"><a class="rm-stat" href="#building-next"><span class="n">${planned.length}</span><span class="l">building next</span></a><a class="rm-stat" href="#deciding"><span class="n">${proposed.length}</span><span class="l">deciding</span></a><a class="rm-stat" href="#future-ideas"><span class="n">${futureIdeas.length}</span><span class="l">future ideas</span></a><a class="rm-stat done" href="#shipped"><span class="n">${shippedTotal}</span><span class="l">shipped</span></a></div>
 ...
 <section class="rm-section" id="deciding"><div class="rm-section-head"><div class="eyebrow">Proposed</div><h2>Big calls, waiting on a decision.</h2><p>Explicitly deferred, not forgotten — each needs a product or architecture decision before it's scoped enough to build. <b>Solution Packages, Publishers & Versioned Deployments</b> is the one worth deciding first: the Industry Data Model already turns App Builder into installable business apps (all 10 shipped — see <a href="/platform">/platform</a>); this is the step that adds real versioning, publishers and safe upgrades on top of it.</p></div><div class="rm-card-grid">${proposed.map(p=>`<div class="backlog-card"><div class="backlog-card-head"><h3>${p[0]}</h3><div class="backlog-card-tags"><span class="backlog-tag proposed-tag">proposed</span>${p[3]?`<span class="backlog-tag">${p[3]}</span>`:''}<span class="backlog-tag">size: ${p[4]}</span></div></div><p class="ask">${p[1]}</p><div class="backlog-solution"><div class="sol-label">Why it's still just proposed</div><ul><li>${p[2]}</li></ul></div></div>`).join('')}</div></section>
