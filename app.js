...
function roadmapPage(){
 ...
 setPageMeta('Everything shipped in Lanesra OS - CRM, no-code platform, App Catalog, Deployment Management, Integration Hub - what\'s being built next, and what\'s still proposed. Compiled directly from the working codebase, not a wishlist.');
 ...
 const latestCatIndex=flatCats.findIndex(c=>c.title==='Industry Data Model & App Catalog');
 ...
 const proposed=[
  ...
  ['MCP server for CRM records','Expose Companies, Contacts, Opportunities and every Custom Object as a Model Context Protocol server, alongside the existing REST API and CLI, so a user\'s own AI agent can read and write real records directly instead of only receiving an API key and a Swagger doc. Competitive context: Lightfield CRM\'s whole pitch is every record being "readable and writable over API, MCP and CLI" so agents get built on top of the CRM like infrastructure — Lanesra already has the generic REST API (hashed/scoped keys) and a scriptable desktop CLI from the Integration Hub work; MCP is the missing third leg of that same story, not a new one.','Needs a decision on tool surface (which entities/actions become MCP tools vs. stay REST-API-only) and, more importantly, on the write path: every MCP write has to run through the same Business Rules, Workflow Automation and permission checks a UI save does, never a shortcut around them. Also needs a host — the Team Workspace server is the natural fit; the desktop edition has no long-running server process to attach an MCP endpoint to today.','AI & Agents','M'],
  ['Structured interaction timeline (calls, email, chat) per record','Lightfield\'s core bet is a "customer world model" built by ingesting calls, email, Slack and LinkedIn into a live structured account picture that people and agents both work from. Lanesra already has the connected-record graph (Company → Contact → Opportunity → …) and an Integration Hub that pulls external data in — the gap is a first-class, timestamped activity timeline on each Company/Contact populated from a logged call or a connected mailbox/channel, instead of only the free-text Tasks a rep types by hand today.','Cuts directly against "Own your data" / "Offline first" unless scoped carefully: ingesting live email/Slack content means either a connector (fits the existing Integration Hub model) or a transcription/summarization step that most likely needs a cloud LLM call, which a self-hosted, no-mandatory-internet product can\'t assume every workspace wants running by default. Needs a decision on which channel ships first (email, since SMTP is already a supported Connection type) and a firm rule that any AI processing step is opt-in and bring-your-own-key, never a silent default.','AI & Agents','L'],
  ['Follow-up & commitment capture from logged activity','Turn a commitment made on a logged call or email ("I\'ll send the updated quote by Friday") into a Task automatically, reusing the Workflow Automation engine that already creates tasks on trigger conditions today, instead of relying on a rep to remember. One of the five agents Lightfield markets, alongside pipeline gen, meeting prep, record hygiene and reporting.','Depends on the interaction timeline item above shipping first — there\'s nothing to extract a commitment from until calls/emails exist as structured record data. Also needs a decision on the extraction approach itself: keyword/pattern rules (fits the existing rules engine, no external dependency, weaker accuracy) vs. an LLM call (the reverse trade-off, and the same opt-in/bring-your-own-key question as the timeline item).','AI & Agents','M'],
  ['AI-assisted record hygiene: fuzzy duplicates & staleness','Lanesra already warns on exact duplicate name/email at save time. Lightfield\'s "record hygiene" agent goes further — proactively surfacing likely-duplicate Companies/Contacts across the whole dataset and flagging Opportunities with no activity past a staleness threshold. A rules-based v1 fits Lanesra\'s existing model directly: a scheduled job doing fuzzy-match/staleness checks that raises a review queue, no LLM required.','Needs a decision on match approach (normalized phone/email/org-domain matching is a meaningfully bigger scope than the exact-match check that ships today) and on where the review queue lives — a new Admin screen, or the existing Notification Center.',null,'M'],
  ['Win/loss pattern mining across closed Opportunities','Shipped Reports already cover win rate by owner and lost reasons. Lightfield\'s "diagnoses weaknesses in your funnel and learns from your best sellers" pitch is the next layer on top: surfacing which activities or fields correlate with a win (e.g. opportunities with a call logged in the first 3 days close at a higher rate) instead of only aggregate win/loss counts.','Needs real usage volume and the interaction timeline item above before there\'s anything but stage/amount/owner to mine — correlation across a handful of fields on typical SMB deal counts is noise, not insight, without it. Revisit once both exist.',null,'L'],
  ['Guided Salesforce/legacy CRM import','Lightfield markets a concrete migration story — one customer moved ~10,000 contacts and ~2,600 opportunities off Salesforce. Lanesra\'s CSV import already runs Companies/Contacts through the same validated write path as the manual forms and the REST API; the gap is an importer that understands a source CRM\'s export shape specifically — standard-object field mapping, multi-object import in one pass that preserves the Company → Contact → Opportunity graph, and a dry-run preview, the same pattern the Data Exchange wizard already uses for Custom Objects.','Needs a decision on how much source-specific mapping to hand-curate (a Salesforce-standard-objects preset is the faster win) versus leaning entirely on Data Exchange\'s existing generic column-mapping wizard, which already handles any object but leaves the multi-object relationship-preservation piece unsolved.',null,'M'],
 ];
 const futureIdeas=['Projects and milestones','Inventory and suppliers','Recurring invoices','Customer portal','Plugin architecture','AI meeting-prep briefs','Prospect-discovery agent','Bring-your-own-key AI assistant'];
