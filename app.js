...
const seed = {
 ...
 fieldRules:[
  {id:'fr1',entity:'opportunities',fieldKey:'leadSource',triggerValue:'Won',effect:'require',operator:'equals'}
 ],
 workflowRules:[
  {id:'wf1',entity:'opportunities',toValue:'Won',taskTitle:'Kick off onboarding',daysOffset:2,notify:true},
  {id:'wf2',entity:'invoices',toValue:'Overdue',taskTitle:'Follow up on overdue invoice',daysOffset:0,notify:false}
 ],
 numberingOverrides:{},
 kpiPrefs:[],
 notifications:[]
};
const storeKey='lanesra-os-demo-v8';
...
function ensureAdminData(){
 ...
 (data.fieldRules||[]).forEach(r=>{if(!r.operator)r.operator='equals'});
 save();
}
...
function applyFieldRules(entityKey,form){
 ...
 function apply(){
  ...
  rules.forEach(r=>{
   ...
   const match=r.operator==='not_equals'?val!==r.triggerValue:val===r.triggerValue;
   if(r.effect==='hide'){
 ...
 if(wasEdit&&relatedTypeFor[key]&&obj[tf]!==undefined&&obj[tf]!==oldValue){
  (data.workflowRules||[]).filter(r=>r.entity===key&&r.toValue===obj[tf]).forEach(r=>{
   ...
   if(r.notify){
    ...
    data.notifications.unshift({id:uid(),message:`${entityLabel(key).replace(/s$/,'')} "${label}" reached ${tf}=${obj[tf]} — task "${r.taskTitle}" created`,createdAt:new Date().toISOString(),read:false});
   }
  });
 }
...
function rulesTab(body){
 ...
 body.innerHTML=`<div class="panel"><h3 style="margin-top:0">Business rules</h3><p class="muted">Require or hide a custom field based on whether the record's ${tf} is, or is not, a chosen value.</p>
 ${entityPills(keys,ruleEntity)}
 ${fieldsForEntity.length?`<div class="table-wrap"><table class="table"><thead><tr><th>When ${tf}</th><th>Then</th><th>Field</th><th>Actions</th></tr></thead><tbody>${list.map(r=>`<tr><td>${r.operator==='not_equals'?'is not':'is'} ${badgeMaybe(r.triggerValue)}</td><td>${r.effect==='require'?'Require':'Hide'}</td><td>${(data.customFields.find(f=>f.entity===r.entity&&f.key===r.fieldKey)||{}).label||r.fieldKey}</td><td><div class="actions"><button class="icon-btn" data-del-rule="${r.id}">Delete</button></div></td></tr>`).join('')}</tbody></table>${list.length?'':'<div class="empty">No business rules on '+entityLabel(ruleEntity)+' yet</div>'}</div><button class="btn btn-secondary" id="addRule" style="margin-top:14px">+ New rule</button>`:`<div class="empty">Add an active custom field on ${entityLabel(ruleEntity)} first — business rules apply to custom fields.</div>`}
 <p class="muted" style="margin-top:14px">This demo shows the core require/hide rule. The Windows desktop edition supports multi-condition AND/OR rules, 10 comparison operators, and lock/set-value/block-save/show-message actions — see the <a href="/roadmap">roadmap</a>.</p>
 </div>`;
...
function ruleModal(fieldsForEntity,tf){
 ...
 <div class="field"><label>When ${tf}</label><select name="operator"><option value="equals">is</option><option value="not_equals">is not</option></select></div>
 <div class="field"><label>Value</label><select name="triggerValue">${statusOpts.map(o=>`<option value="${o}">${o}</option>`).join('')}</select></div>
 ...
 $('[data-close]').onclick=closeModal;
 $('#ruleForm').onsubmit=e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(e.target).entries());data.fieldRules.push({id:uid(),entity:ruleEntity,fieldKey:fd.fieldKey,triggerValue:fd.triggerValue,effect:fd.effect,operator:fd.operator});save();closeModal();toast('Business rule added');renderView()};
}
function workflowTab(body){
 ...
 body.innerHTML=`<div class="panel"><h3 style="margin-top:0">Workflow automation</h3><p class="muted">Automatically create a follow-up task, and optionally notify admins, when a record's ${tf} changes to a chosen value.</p>
 ${entityPills(keys,wfEntity)}
 <div class="table-wrap"><table class="table"><thead><tr><th>When ${tf} becomes</th><th>Creates task</th><th>Due</th><th>Notifies admins</th><th>Actions</th></tr></thead><tbody>${list.map(r=>`<tr><td>${badgeMaybe(r.toValue)}</td><td>${r.taskTitle}</td><td>${r.daysOffset==0?'Same day':r.daysOffset+' day(s) later'}</td><td>${r.notify?'Yes':'No'}</td><td><div class="actions"><button class="icon-btn" data-del-wf="${r.id}">Delete</button></div></td></tr>`).join('')}</tbody></table>${list.length?'':'<div class="empty">No workflow rules on '+entityLabel(wfEntity)+' yet</div>'}</div>
 <button class="btn btn-secondary" id="addWf" style="margin-top:14px">+ New workflow rule</button>
 <p class="muted" style="margin-top:14px">This demo shows one trigger (a status/stage change) and two actions (create task, notify admins). The Windows desktop edition adds triggers for field changes and due dates, plus actions like assigning the record's owner and creating a related record — see the <a href="/roadmap">roadmap</a>.</p>
 </div>`;
...
function workflowModal(tf){
 ...
 $('[data-close]').onclick=closeModal;
 $('#wfForm').onsubmit=e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(e.target).entries());data.workflowRules.push({id:uid(),entity:wfEntity,toValue:fd.toValue,taskTitle:fd.taskTitle,daysOffset:Number(fd.daysOffset||0),notify:fd.notify==='true'});save();closeModal();toast('Workflow rule added');renderView()};
}
