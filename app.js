...
function roadmapPage(){
 ...
 const planned=[
  ...
  ['Connector Template Library: curated pre-built connectors','Every Connector today starts from a blank OpenAPI import - an admin pastes or uploads their own spec text before a single Action exists, with no pre-built option anywhere (a deliberate choice so far, not an oversight: the alternative considered and rejected was fabricating vendor-specific action packs no one had tested). A curated template gallery would let <b>Import connector</b> start from a ready-to-use spec instead, across three groups: data warehouses (Snowflake, BigQuery, Databricks - Redshift needs no new work at all, since it\'s wire-compatible with the existing PostgreSQL connection type already), other AI model APIs as callable Actions distinct from the Unified AI Gateway\'s own inference routing (OpenAI, Google Gemini, Cohere, Perplexity, ElevenLabs, Replicate), and common SaaS tools split by how they authenticate (Slack/GitHub/Stripe/SendGrid/Twilio on a simple API key or Bearer token; HubSpot/Salesforce/Google Sheets/Notion/Airtable on the <code>oauth2_authorization_code</code> flow Connections already support, once an admin has registered their own OAuth app with that vendor). Templates are deliberately hand-curated and trimmed to the endpoints people actually use, not a vendor\'s full official spec dumped in wholesale - a real Stripe or Salesforce spec leans on <code>$ref</code>/<code>oneOf</code> this parser doesn\'t resolve, which would silently exclude those actions from the Integration Hub Tool Bridge\'s own fail-closed schema gate; a trimmed spec with inline-resolved schemas imports clean and is usable as both a Workflow action and an agent tool immediately. Snowflake and BigQuery need their own auth-mode check before being added - both commonly use JWT/key-pair auth, which doesn\'t yet map onto the existing <code>auth_mode</code> enum (api_key/basic/bearer/custom_header/oauth2) the way the rest of this list does.','Integration Hub','L'],
