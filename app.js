...
const HOME_PRIMITIVES=[
 ...
 ['⇄','Integration Hub, Agent-Aware','Encrypted Connections, OpenAPI-imported Connectors, a generic REST API and HMAC-signed webhooks — the same Integration Hub your agents can already list and manage today. Turning a connector action into a callable agent tool is next on the roadmap.'],
];
...
function roadmapPage(){
 ...
 const planned=[
  ...
  ['Integration Hub Tool Bridge: connectors as agent tools','An AI Agent\'s Actions checklist is a fixed, hand-registered catalog (8 record tools, 37 admin action pairs) - nothing today lets an agent call a Connector Action built from an OpenAPI import, or fire an existing Workflow Automation action, as a tool of its own. Translating a Connector Action\'s own request/response schema into a standard LLM tool-call definition would give an agent real-world reach into whatever a workspace has already connected, without a line of custom integration code - the highest-leverage item here, since it\'s translation over infrastructure (Connectors, OpenAPI import, scoped auth) that already exists end to end. Phased for safety: <b>read-only Connector Actions exposed as tools first</b>; write-capable actions gated behind a separate, explicit per-connector opt-in (Administrator-only, the same gate the Actions checklist itself already uses for admin-catalog access) - not exposed by default just because a schema exists.','Integration Hub','L'],
