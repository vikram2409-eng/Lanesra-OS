...
function roadmapPage(){
 ...
 const shippedPillars=[
  ...
  ['✦','AI & Agentic Layer','Bring your own LLM key, then build on it — an MCP server for outside agents, a conversational assistant, and a full AI Agent Foundry with memory, skills, actions, delegation and orchestration.',[
 ...
 const latestCatIndex=flatCats.findIndex(c=>c.title==='AI Agent Foundry & Orchestration');
 ...
 <div class="backlog-callout" id="desktop"><h3>Release status</h3><p><b>desktop-v0.12.0 is the latest tagged release</b> (installers attached, Early Access/prerelease as intended), cut with the Integration Hub work (v0.35.0) — Saved Views & Bulk Actions (v0.36.0) through the LLM Chat Assistant (v0.41.0) have since merged to <code>main</code> but haven't been cut into a new installer tag yet; the AI Agent Foundry and Orchestration (v0.42.0–v0.43.0) are open pull requests, not yet merged. Everything below reflects the working codebase across <code>main</code> and those open PRs, regardless of installer or merge status. Full desktop feature list → <a href="/download">/download</a>, full release history → <a href="/releases">/releases</a>.</p><p class="muted">Repo hygiene: a real MIT <code>LICENSE</code>, <code>CONTRIBUTING.md</code>, <code>CODE_OF_CONDUCT.md</code>, <code>SECURITY.md</code>, issue/PR templates, and a root README written for someone landing on the repo, not a deploy runbook.</p></div>
 </div></section>
