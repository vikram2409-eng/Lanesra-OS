...
function roadmapPage(){
 ...
 <section class="rm-section" id="deciding"><div class="rm-section-head"><div class="eyebrow">Proposed</div><h2>Big calls, waiting on a decision.</h2><p>Explicitly deferred, not forgotten — each needs a product or architecture decision before it's scoped enough to build. <b>Industry Data Model</b> is the one worth deciding first: it's the step that turns App Builder from a manual assembly tool into installable business apps — see <a href="/platform">/platform</a>.</p></div><div class="rm-card-grid">${proposed.map(p=>`<div class="backlog-card"><div class="backlog-card-head"><h3>${p[0]}</h3><div class="backlog-card-tags"><span class="backlog-tag proposed-tag">proposed</span>${p[3]?`<span class="backlog-tag">${p[3]}</span>`:''}<span class="backlog-tag">size: ${p[4]}</span></div></div><p class="ask">${p[1]}</p><div class="backlog-solution"><div class="sol-label">Why it's still just proposed</div><ul><li>${p[2]}</li></ul></div></div>`).join('')}</div></section>
