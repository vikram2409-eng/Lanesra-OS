...
function runAiAgentPipelineNow(pipeline,triggerInput){
 ...
 const run={id:uid(),startedAt:new Date().toISOString(),triggerInput:triggerInput||'',steps,finalOutput};
...
function platformPage(){
 ...
 const aiLayer={icon:'🏭',eyebrow:'Admin → AI Agent Foundry',name:'AI & Agentic Layer',blurb:'Bring your own Anthropic or OpenAI-compatible key - Lanesra never resells inference - then build on it: an MCP server and CLI for outside agents, a conversational Chat Assistant with real tool access, and a full AI Agent Foundry where an Administrator defines named agents with a persona, a per-tool Actions checklist, persistent Memory, a shared Skills library, and delegation to other agents. Chain agents into a deterministic Pipeline and fire it manually, on a schedule, from a webhook, or as a new Workflow Automation action - every run lands in one unified history.'};
