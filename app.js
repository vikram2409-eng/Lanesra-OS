...
function platformPage(){
 ...
 const layers=[
  ...
  {icon:'🏭',eyebrow:'Admin → AI Agent Foundry',name:'AI & Agentic Layer',blurb:'Bring your own Anthropic or OpenAI-compatible key - Lanesra never resells inference - then build on it: an MCP server and CLI for outside agents, a conversational Chat Assistant with real tool access, and a full AI Agent Foundry where an Administrator defines named agents with a persona, a per-tool Actions checklist, persistent Memory, a shared Skills library, and delegation to other agents. Chain agents into a deterministic Pipeline and fire it manually, on a schedule, from a webhook, or as a new Workflow Automation action - every run lands in one unified history.'},
 ];
 $('#app').innerHTML=`<div class="site-dark">${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">One platform, any business</div><h1>Lanesra OS isn't only a CRM. It's what your CRM is built on.</h1><p>Most CRMs assume your business looks like everyone else's — until it doesn't, and a lease renewal, a case file or a maintenance schedule sends you back to a spreadsheet or a consultant's custom build. Companies, Contacts, Opportunities, Quotes, Orders, Invoices, Contracts and Tasks — the sales system you get out of the box — aren't hardcoded into Lanesra either. They're one app built from the same platform every workspace has underneath it, and an Administrator gets the identical building blocks, from a settings screen, to model whatever your organization actually runs on.</p><div class="hero-actions"><a class="btn btn-primary" href="/demo">Try the live demo →</a><a class="btn btn-secondary" href="#examples">See what you could build ↓</a></div></div></section>
