...
function roadmapPage(){
 ...
 const proposed=[
  ['Industry Data Model — installable business-app packages','A versioned metadata package — objects, fields, relationships, screens, views, business rules, workflows, dashboards, reports, permissions and numbering, with optional sample data — that installs a ready-to-use vertical app (Field Service, Property Management, Professional Services, Recruitment/Staffing, Contractor/Construction) into an existing workspace from an Admin → App Catalog. It is not a separate product or a separate Lanesra instance: install reuses the workspace\'s existing Customer, Contact, Product, Invoice and Task records rather than creating parallel "Property Customer" or "Recruitment Contact" duplicates, so Sales, Field Service and Property Management can coexist against the same customer master. Install runs through Pre-Install Validation (name/numbering collisions, missing dependencies), an automatic safety backup, and a transactional install with rollback on failure; Deactivate removes an app from navigation without touching the business records it created.','The largest single item ever proposed here — a package manifest format, an InstalledApp ownership/versioning registry that survives updates without clobbering an admin\'s own customizations to an installed app, and the transactional install/rollback machinery, all need building before a single reference app can ship. Sequencing needs a decision: the source spec proposes Field Service and Property Management first (each a self-contained reference data model — Work Order/Asset/Service Appointment for one, Property/Unit/Lease for the other), with a portable .lanesra-app package format and a publisher-facing "App Builder → Package App" export flow deferred to a later phase once the install/update/rollback story is proven.','App Builder','XL'],
 ...
 const sequence=[
  ['1 · Saved Views & Bulk Actions','Scoped and ready to build — a clear extension of primitives that already ship (the global search and per-field filter infrastructure), not an open product question.',true],
  ['2 · Decide: Industry Data Model, Approvals, APIs/Webhooks, richer report builder, Google/Microsoft sign-in, code signing','Everything left needs a product/architecture decision first — several independent scope/budget calls. Industry Data Model is the one worth deciding first: it\'s the packaging step that turns App Builder from a manual assembly tool into installable business apps — see /platform.',false],
 ];
 $('#app').innerHTML=`${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Built from the working codebase</div><h1>Roadmap & backlog.</h1><p>Where Lanesra OS stands today, what's being built next, and everything queued up after that — compiled from the actual code (core/server/src-tauri/frontend) plus the online demo, not a wishlist. Every "Shipped" line below is running code with tests.</p><div class="status-row"><span class="status-chip">Early Access v0.27.0</span><span class="muted">Last updated August 2026</span></div>
 ...
 <div class="backlog-stats"><div class="backlog-stat"><div class="n">${shipped.length}</div><div class="l">shipped epics</div></div><div class="backlog-stat"><div class="n">${planned.length}</div><div class="l">planned, scoped items</div></div><div class="backlog-stat"><div class="n">${proposed.length}</div><div class="l">proposed, awaiting a decision</div></div><div class="backlog-stat"><div class="n">${planned.length>0?1:0}</div><div class="l">up next</div></div></div>
 ...
 <section class="backlog-group"><div class="backlog-group-head"><h2>Up next</h2><span class="backlog-group-note">actively next in line, per the recommended sequencing below</span></div>${planned.length>0?`<p class="roadmap-row active">↻ ${planned[0][0]} — ${planned[0][1]}</p>`:`<p class="roadmap-row active">↻ Nothing scoped and ready right now — every remaining near-term item needs a product/architecture decision first. See "Proposed" below.</p>`}</section>

 ${planned.length>0?`<section class="backlog-group"><div class="backlog-group-head"><h2>Near-term backlog</h2><span class="backlog-group-note">scoped, not yet started</span></div>${planned.map(p=>`<div class="backlog-card"><div class="backlog-card-head"><h3>${p[0]}</h3><div class="backlog-card-tags"><span class="backlog-tag planned-tag">planned</span><span class="backlog-tag">${p[2]}</span><span class="backlog-tag">size: ${p[3]}</span></div></div><p class="ask">${p[1]}</p></div>`).join('')}</section>`:''}
