...
function roadmapPage(){
 ...
 const sequence=[
  ['1 · Dashboard customization Phase 2/3 + Saved Views/Bulk Actions','Both are scoped and ready to build — clear extensions of primitives that already ship (Custom Reports, the new filter/search infrastructure), not open product questions.',true],
  ['2 · Decide: App Builder, Approvals, APIs/Webhooks, richer report builder, Google/Microsoft sign-in, code signing','Everything left needs a product/architecture decision first — several independent scope/budget calls. App Builder is the one worth deciding first: it\'s the packaging step that turns everything already shipped into the platform story on /platform.',false],
 ];
 $('#app').innerHTML=`${publicNav()}<main class="page-site"><section class="page-hero"><div class="container narrow"><div class="eyebrow">Built from the working codebase</div><h1>Roadmap & backlog.</h1><p>Where Lanesra OS stands today, what's being built next, and everything queued up after that — compiled from the actual code (core/server/src-tauri/frontend) plus the online demo, not a wishlist. Every "Shipped" line below is running code with tests.</p><div class="status-row"><span class="status-chip">Early Access v0.26.0</span><span class="muted">Last updated August 2026</span></div>
