 ...
 if(wasEdit&&relatedTypeFor[key]&&before){
  ...
  (data.workflowRules||[]).filter(r=>r.entity===key).forEach(r=>{
   ...
   if(r.notify){
    ...
    data.notifications.unshift({id:uid(),message:`${entityLabel(key).replace(/s$/,'')} "${label}" — ${fieldLabelFor(key,wf)} ${OPERATOR_LABELS[r.operator||'equals']}${operatorNeedsValue(r.operator||'equals')?` ${describeComparand(key,r.compareField,r.toValue)}`:''} — task "${r.taskTitle}" created`,createdAt:new Date().toISOString(),read:false});
   }
  });
 }
...
function ruleModal(entityKey){
 ...
 <div id="condDynamic" style="display:contents">${conditionDynamicHtml(condFields,defaultField,'equals','',null)}</div>
 ...
 form.onsubmit=e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(form).entries());
  data.fieldRules.push({
   id:uid(),entity:entityKey,triggerField:fd.triggerField,operator:fd.operator,triggerValue:fd.triggerValue||'',
   compareField:fd.compareField||null,fieldKey:fd.targetField,effect:fd.effect,
  });
...
function workflowModal(entityKey){
 ...
 <div id="wfDynamic" style="display:contents">${conditionDynamicHtml(condFields,defaultField,'equals','',null).replaceAll('name="triggerValue"','name="toValue"')}</div>
 ...
 form.onsubmit=e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(form).entries());
  data.workflowRules.push({
   id:uid(),entity:entityKey,triggerField:fd.triggerField,operator:fd.operator,toValue:fd.toValue||'',
   compareField:fd.compareField||null,taskTitle:fd.taskTitle,daysOffset:Number(fd.daysOffset||0),notify:fd.notify==='true',
  });
