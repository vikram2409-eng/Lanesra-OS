...
function roadmapPage(){
 ...
 const proposed=[
  ['Solution Packages, Publishers & Versioned Deployments','A Dynamics-365-style Solutions layer on top of the now-shipped Industry Data Model: real Publishers with a namespaced key model (<code>lanesra.work_order</code>, <code>acme.inspection</code>, <code>local.customer_tier</code>), a Managed/Unmanaged distinction (every installed package today is implicitly managed — there\'s no way yet for an organization to package its own customizations), a 5-layer extensibility model so a managed component can be safely extended without editing it destructively, update-with-diff (today reinstalling an already-installed package is rejected outright — no upgrade path), and immutable Releases with export/import as a portable <code>.lanesra</code> archive. Paired with an 11-domain Admin information-architecture redesign (Workspace / Access / Data Model / Experience / Automation / Apps / Solution Management / Analytics / Integrations / Data Management / System) so "what have I customized beyond what I installed" finally has a real screen to answer it.','A full 37-section design spec exists and a Phase 1 plan (Admin IA reshuffle + a read-only Solution Management landing page over the existing app_packages/package_artifacts/app_dependencies data, zero new tables) was drafted but never approved — sequencing was agreed (Admin IA reshuffle before the deeper Publisher/versioning work) but building was paused for a different request. Needs the user to pick the plan back up before work resumes.','App Catalog','XL'],
