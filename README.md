...
- **Integration Hub** — AES-256-GCM-encrypted Connections (REST/SFTP/PostgreSQL/MySQL/SQL Server/OData/SMTP), OpenAPI-imported Connectors (a curated template gallery of 19 — AI models, data warehouses and SaaS tools including OpenAI, Google Gemini, Snowflake, Databricks, Slack, GitHub, HubSpot, Stripe, Twilio, Zendesk, Jira, PagerDuty, Datadog, Mailchimp, Shopify — to start from) usable as Workflow Automation actions or, once opted in, as AI Agent Foundry tools (read-only by default, write access behind a further explicit per-connector opt-in), a generic REST API with hashed/scoped API keys, HMAC-SHA256-signed Webhooks with retry, a generalized CSV data-exchange wizard, and scheduled Integration Jobs
